...
module.exports = {
  ...
  define: function(definition, cb) {
    ...
    this.describe(this.collection, function(err, existingAttributes) {
      ...
      self.adapter.define(self.collection, definition, cb);
    });
  },
  ...
  alter: function(attributes, cb) {
    ...
    if (this.adapter.alter) {
      this.adapter.alter(this.collection, attributes, cb);
      return;
    }
    ...
    if (this.adapter.addAttribute && this.adapter.removeAttribute) {
      ...
      // Alter the schema
      self.describe(self.collection, function afterDescribe (err, originalAttributes) {
