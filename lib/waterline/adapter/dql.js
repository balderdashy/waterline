...
module.exports = {
  ...
  create: function(values, cb) {
    ...
    // Strip nested create/update/upserts out before passing down to the adapter
    if (_.isObject(values)) {
      var collection = this.query.waterline.collections[this.collection];
      _.each(values, function (val,attrName){
        if (_isAssociation(attrName, collection)) {
...
/**
 * [_isAssociation description]
 * @param  {[type]}  attrName   [description]
 * @param  {[type]}  collection [description]
 * @return {Boolean}            [description]
 */
function _isAssociation(attrName, collection) {
  if (_.isObject(collection._attributes)) {
    var attrDef = collection._attributes[attrName];
    if (_.isObject(attrDef) && (attrDef.collection||attrDef.model)) {
      return true;
    }
  }
  return false;
