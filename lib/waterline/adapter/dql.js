...
module.exports = {
  ...
  distinct: function(attribute, criteria, cb) {
    if (!this.adapter.distinct) {
        cb(new Error("No distinct() method defined in adapter!"));
    } else {
        criteria = normalize.criteria(criteria);
        this.adapter.distinct(this.collection, attribute, criteria, cb);
    }
  },
