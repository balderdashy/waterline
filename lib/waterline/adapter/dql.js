...
module.exports = {
  ...
  find: function(criteria, cb) {
    ...
    this.findAll(criteria, function(err, models) {
      if (!models) return cb(err);
      if (models.length < 1) return cb(err);
      if (models.length > 1) return cb(new Error("More than one " + this.collection + " returned!"));
  ...
  count: function(criteria, cb) {
    ...
    if (!this.adapter.count) {
      this.findAll(this.collection, criteria, function(err, models) {
  ...
  update: function(criteria, values, cb) {
    ...
    this.updateAll(criteria, values, function (err, models) {
      if (!models) return cb(err);
      if (models.length < 1) return cb(err);
      if (models.length > 1) return cb(new Error("More than one " + this.collection + " returned!"));
  ...
  updateAll: function (criteria, values, cb) {
