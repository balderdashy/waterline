...
module.exports = function(cb) {
  ...
  self.describe(function afterDescribe(err, attrs) {
    ...
    // if it doesn't go ahead and define it, then get out
    if(!attrs) return self.define(cb);

    var oldAttrs = attrs;
    var newAttrs = self.query.waterline.collections[self.identity]._attributes;
    console.log('\n\n\nChecking for attrs that are in:', oldAttrs, '\n\nBUT NOT IN:',newAttrs);

    // If any attributes have been removed, prompt before continuing:
    var removedAttrs = [];
    _.each(oldAttrs, function (oldAttrDef, oldAttrName){
      // console.log('\n******\nsearching for '+oldAttrName+' in new attrs....');
      var match = _.find(newAttrs, function (newAttrDef, newAttrName) {
        var match =
        (newAttrDef.columnName && newAttrDef.columnName === oldAttrName) ||
        newAttrName === oldAttrName;

        console.log('newAttrDef.columnName && newAttrDef.columnName === oldAttrName ?',!!(newAttrDef.columnName && newAttrDef.columnName === oldAttrName));
        console.log('newAttrName (%s) === oldAttrName (%s) ?',newAttrName, oldAttrName, newAttrName === oldAttrName);
        return match;
      });

      if (!match) {
        oldAttrDef.name = oldAttrName;
        removedAttrs.push(oldAttrDef);
      }
    });
    console.log('\n\n\nRemoved attrs...', removedAttrs);

    // TODO: figure out why the result from `describe` is weird (at least in sails-disk)
    // It is as if it returns the new schema- not the actual physical schema in localDiskDb

    // if there are any removedAttrs, show a confirmation prompt before
    // continuing w/ the auto-migration
    (function (onwards){
      if (removedAttrs.length) {
        ...
        console.log(
          'Warning!\n'+
          'Your underlying datastore contains one or more fields/columns that  '+
          'are not referenced by the attributes in your `'+self.identity+'` model.\n'+
          'Since Waterline is configured with `migrate:alter`, if you proceed, your data\n'+
          'in those fields will be deleted.'
        );
        console.log();
        console.log('Should we proceed with the auto-migration?');
        console.log();
        console.log('** WARNING: DO NOT TYPE "y" IF YOU ARE WORKING WITH PRODUCTION DATA **');
        prompt.get(['y/n'], function (err, results) {
          if (err) return onwards(err);
          var wasConfirmedByUser = _.isString(results['y/n']) && results['y/n'].match(/y/);
          if (!wasConfirmedByUser) {
            return onwards(new Error('Auto-migration aborted. Please migrate your data manually and then try this again.'));
          }
          onwards();
        });
      }
      ...
      self.find({}, function (err, existingData) {
        ...
        async.each(uniqueAttrs, function (uniqueAttr, each_cb) {
          ...
          // TODO: handle any other db-enforced constraints the same way
          // (I don't think there are any, but leaving this here as a stub ~mike)
