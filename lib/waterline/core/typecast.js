...
Cast.prototype.run = function(values) {
  ...
  Object.keys(values).forEach(function(key) {
    ...
    switch(type) {
      ...
      case 'json':
        values[key] = self.json(values[key]);
        break;
    }
  });
...
Cast.prototype.integer = function integer(key, value) {
  ...
  // it. Needed for sails-mongo where the primary key value is set to integer by default.
  if(key === 'id' && utils.matchMongoId(value)) return value.toString();
...
Cast.prototype.json = function json(value) {
  try {
