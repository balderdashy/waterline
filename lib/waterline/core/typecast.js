...
Cast.prototype.run = function(values) {
  ...
  Object.keys(values).forEach(function(key) {
    ...
    // Set keys with `undefined` on their RHS to `null` instead.
    if (_.isUndefined(values[key])) {
      values[key] = null;
    }//>-

    // If RHS is null, or if `self._types` doesn't contain this key, or if SOMEHOW this RHS has gone missing...
    if (!_.has(self._types, key) || values[key] === null || !_.has(values, key)) {
      return;
    }//-•
