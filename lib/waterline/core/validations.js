...
Validator.prototype.validate = function (values, presentOnly, cb) {
    ...
    function validate(validation, cb, curValidation, values, errors) {
    ...
    function runValidation(validations, values, errors) {
        var attrs = Object.keys(validations);
        async.each(attrs, function (key, cb) {

            errors[key] = {};
            if (validations[key] instanceof Object && !validations[key].hasOwnProperty('type')) {
                return runValidation(validations[key], values[key], errors[key]);
            } else {
                return validate(key, cb, validations[key], values, errors);
            }

        }, function allValidationsChecked() {
    ...
    runValidation(self.validations, values, Errors);
