...
Validator.prototype.validate = function(values, presentOnly, cb) {
  ...
  function validate(validation, cb) {
    ...
    // If type is double, change to float for validation purposes.
    // Underlying validation package doesn't have double, but there
    // is no difference between double and float as far as javascript
    // is concerned.
    if(requirements.data && requirements.data.type && requirements.data.type === 'double')
      requirements.data.type = 'float';
