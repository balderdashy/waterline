...
Validator.prototype.validate = function(values, presentOnly, cb) {
  ...
  async.each(validations, function validate(validation, cb) {
    ...
    try {
      ...
      throw e;
      // Note, this should be handled differently so it does not crash
      // the process.  However, the following fails silently:
      // return cb(e);
    }
    ...
    async.each(Object.keys(requirements.data), function(key, next) {
      try {
