...
Schema.prototype.initialize = function(attrs, associations, hasSchema) {
  var key;
  ...
  // Build Schema Associations
  for(key in associations.models) {

    // Cache the association object
    var attr = _.clone(associations.models[key]);

    // Determine a columnName if one isn't specified
    var columnName = attr.model.columnName || attr.model.toLowerCase() + '_id';

    // Build the schema object
    var schemaValue = this.objectAttribute(attr);

    // Set type to integer if one isn't set
    schemaValue.type = schemaValue.type || 'integer';

    // Set a special foreign key flag
    schemaValue.foreignKey = true;

    // Set the schema value
    this.schema[columnName] = schemaValue;
  }
