...
Schema.prototype.initialize = function(attrs, hasSchema) {
  for(var key in attrs) {
    ...
    // Check if the user has specified an primary key,
    // if so set schema.pk to the name of the column
    if (typeof attrs[key].primaryKey !== 'undefined'
        && attrs[key].primaryKey === true) {
        this.schema.pk = key;
    }
  }

  // If the user hasn't specified an primary key, default to id
  if (typeof this.schema.pk === 'undefined') {
    this.schema.pk = 'id';
  }
...
Schema.prototype.addAutoAttributes = function() {
  ...
  // extend definition with autoUpdatedAt, autoCreatedAt
  ...
  // Add deletedAt, defaults to NULL (not deleted)
  if(this.context.softDelete && !this.schema.deletedAt) this.schema.deletedAt = { type: 'datetime', 'default': 'NULL' };
};
