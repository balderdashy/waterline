...
var Core = module.exports = function(options) {
  ...
  this._associations = schemaUtils.associations(this.attributes);
...
_.extend(Core.prototype, {
  ...
  _initialize: function(options) {
    ...
    // Add Associations to this._model.prototype
    Object.keys(this._associations.collections).forEach(function(key) {
      self._model.prototype[key] = new Association(self._associations.collections[key]);
    });

    // Initialize internal objects from attributes
    this._schema.initialize(this.attributes, this._associations, this.hasSchema);
    this._cast.initialize(this._schema.schema);
    this._validator.initialize(this.attributes, this.types);

    // Build Data Transformer
    this._transformer = new Transformer(this.attributes, this._associations.models);
