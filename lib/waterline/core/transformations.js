...
var Transformation = module.exports = function(attributes, associations) {

  // Hold an internal mapping of keys to transform
  this._transformations = {};

  // Hold an internal mapping of associations
  this._associations = _.clone(associations);

  attributes = _.extend(attributes, associations.models);
...
Transformation.prototype.initialize = function(attributes) {
  ...
  Object.keys(attributes).forEach(function(attr) {
    ...
    Object.keys(attributes[attr]).forEach(function(key) {
      ...
      // Currently just works with `columnName` and `model`
      if(key !== 'columnName' && key !== 'model') return;
      ...
      // Set association transformation
      if(key === 'model') {
        self._transformations[attr] = attributes[attr][key].toLowerCase() + '_id';
      }
