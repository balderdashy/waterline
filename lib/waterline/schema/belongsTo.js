...
function mapHasManyAssociation(attributes, schema) {
  ...
  Object.keys(attributes).forEach(function(attr) {
    ...
    var model = attributes[attr].collection.toLowerCase();
    var referenceAttr;

    // Figure out what to reference on the parent table and what type it is.
    Object.keys(schema[model].attributes).forEach(function(key) {
      if(schema[model].attributes[key].hasOwnProperty('foreignKey')) {
        referenceAttr = schema[model].attributes[key].columnName;
      }
    });
    ...
    attributes[attr].on = referenceAttr;
  });
