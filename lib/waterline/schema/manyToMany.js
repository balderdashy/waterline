  ...
  Object.keys(collection.attributes).forEach(function(attr) {
    ...
    tables.forEach(function(table) {
      if(table.tables.indexOf(identity) > -1 && table.tables.indexOf(collectionName) > -1) {
        ...
        collection.attributes[attr].references = table.identity.toLowerCase();
        collection.attributes[attr].on = fk ? fk : identity.toLowerCase() + '_id';
      }
    });
...
function buildJunctionTables(tableName, schema) {
  ...
  // Ensure tableName is always lowercased to prevent edge cases when people
  // use camel cased model names
  ...
  Object.keys(schema[tableName].attributes).forEach(function(key) {
    if(schema[tableName].attributes[key].hasOwnProperty('collection')) {
      tableCollections.push(schema[tableName].attributes[key].collection.toLowerCase());
    }
  });
  ...
  Object.keys(schema).forEach(function(key) {
    ...
    if(key.toLowerCase() === tableName) return;
...
function buildForeignKey(model, schema) {
  ...
  var columnName = model.toLowerCase() + '_' + referenceAttr.key.toLowerCase();
