...
function mapJunctionTables(identity, table, schema) {
  ...
  Object.keys(table.attributes).forEach(function(attr) {
    ...
    // Point original collections to the junction table
    var fk;

    Object.keys(junctionTable.attributes).forEach(function(key) {
      if(key.references === identity) fk = key;
    });

    table.attributes[attr].references = junctionTable.identity;
    table.attributes[attr].on = fk ? fk : identity + '_id';
  });
...
function buildJunctionTable(identity, schema) {
  ...
  Object.keys(schema).forEach(function(key) {
    ...
    Object.keys(table.attributes).forEach(function(attr) {
      if(!table.attributes[attr].collection) return;
      if(table.attributes[attr].collection.toLowerCase() !== identity) return;
      ...
      junctionTable.attributes[identity] = fk_identity;
