  ...
  findAndModify: function(criteria, values, options, cb) {
    ...
    this.find(criteria).exec(function(err, results) {
      ...
      if (results && results.length !== 0) {
        ...
        // you should normally only use mergeArrays if you are searching for a unique
        // indexed element.
        if (options.mergeArrays) {
          for (var i = 0; i < results.length; i++) {
            ...
            for (var j = 0; j < resultKeys.length; j++) {
              ...
              //before merging them
              if (Array.isArray(result[resultKeys[j]])) {
                if (resultKeys[j] in values && Array.isArray(values[resultKeys[j]])) {
                  //now update the values object
                  values[resultKeys[j]] = result[resultKeys[j]].concat(values[[resultKeys[j]]]);
                }
              }
            }
          }
