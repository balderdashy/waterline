  ...
  createEach: function(valuesList, cb) {
    ...
    async.each(valuesList, function(item, next) { _validate.call(self, item, next); }, function(err) {
      ...
      self.adapter.createEach(valuesList, function(err, values) {
        ...
        _afterCreate.call(self, values, function(err) {
          if(err) return cb(err);
...
function _validate(record, cb) {
  ...
  async.series([
    ...
    function(next) {
      ...
      async.eachSeries(self._callbacks.beforeCreate, runner, next);
    }
...
function _afterCreate(values, cb) {
  var self = this;

  async.each(values, function(record, next) {
    ...
    async.eachSeries(self._callbacks.afterCreate, runner, next);
  }, cb);
}
