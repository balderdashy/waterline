...
module.exports = {
  ...
  findOrCreateEach: function(criteria, valuesList, cb) {
    ...
    if(!_.isArray(criteria)) return usageError('No criteria specified!', usage, cb);
    if(!valuesList) return usageError('No valuesList specified!', usage, cb);
    if(!_.isArray(valuesList)) return usageError('Invalid valuesList specified (should be an array!)', usage, cb);
    ...
    async.each(valuesList, validateItem, function(err) {
      ...
      // Transform Values
      var transformedValues = _.map(valuesList, function(value) {
        // Transform values
        value = self._transformer.serialize(value);

        // Clean attributes
        return self._schema.cleanValues(value);
      });
      ...
      var transformedCriteria = _.map(criteria, self._transformer.serialize, self._transformer);
      ...
      self.adapter.findOrCreateEach(criteria, valuesList, function(err, values) {
        ...
        // Unserialize Values
        var unserializedValues = _.map(values, self._transformer.unserialize, self._transformer);
...
function _validateValues(valuesList) {
  for(var i=0; i < valuesList.length; i++) {
    if(valuesList[i] !== Object(valuesList[i])) {
      return 'Invalid valuesList specified (should be an array of valid values objects!)';
    }
  }
}
...
function _validate(record, cb) {
  ...
  _.forIn(self.attributes, function(attr, key) {
    if(!record[key] && record[key] !== false && _.has(attr, 'defaultsTo')) {
      var defaultsTo = attr.defaultsTo;
