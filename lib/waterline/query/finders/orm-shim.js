...
module.exports = function(obj){
  ...
  _.each(junctors, function eachJunctor (junctor) {
    ...
    // console.log('::\n'+util.inspect(junctorMetadata, false, null)+'\n-----------\n\n\n');
    _(junctorMetadata.attributes).each(function eachAttrInjunctor (attrDef, attrName) {
      ...
      _.each(childRelationWL1Schema.attributes, function eachChildAttribute (childAttrDef, childAttrName) {
        // console.log('Looking at `'+childAttrName+'`: '+util.inspect(childAttrDef, false, null));
      ...
      // EVERY FK on a junctor doesn't necessarily need to be referenced- just one of them.
      if (!wl1ReferencedAttrOnChildRelation) {
        return;
        // throw new Error('Could not match attribute on '+childRelation.identity+ ' which references junctor:'+junctor.identity);
      }
