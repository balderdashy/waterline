...
module.exports = {
  ...
  buildDynamicFinders: function() {
    ...
    // For each defined overviewed attribute create the corresponding function
    if (!_.isEmpty (this.overviewAttrs)) {
        _.each (_.isArray (this.overviewAttrs) ? this.overviewAttrs : _.keys (this.overviewAttrs),
                function (attrName) {
                    this.generateDynamicOverviewer(attrName, 'overviewBy*');
                }, this);
    }
  },
  ...
  generateDynamicOverviewer: function(attrName, method, dontCapitalize) {
    var self = this,
        criteria;
    ...
    // Assign this finder to the collection
    this[actualMethodName] = function dynamicMethod(options, cb, variations) {
      ...
      var usage = utils.capitalize(self.identity) + '.' + actualMethodName + '([options],callback,[variations])';

      // Statistical finders
      return self.overviewBy (attrName, options, cb, variations);
    };
  },
