...
module.exports = {
  ...
  buildDynamicFinders: function() {
    ...
    for(var attrName in this._schema.schema) {
      ...
      if(this._schema.schema[attrName].hasOwnProperty('foreignKey')) {
  ...
  generateAssociationFinders: function(attrName) {
    ...
    for(var key in this._associations.models) {
      var cache = this._associations.models[key];

      if(cache.hasOwnProperty('columnName') && cache.columnName === attrName) {
        name = key;
        model = cache.model;
        return;
      }
