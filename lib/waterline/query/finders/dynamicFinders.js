...
module.exports = {
  ...
  buildDynamicFinders: function() {
    ...
    for(var attrName in this._schema) {
  ...
  generateDynamicFinder: function(attrName, method, dontCapitalize) {
    ...
    this[actualMethodName] = function dynamicMethod(value, options, cb) {
      ...
      var usage = _.str.capitalize(self.identity) + '.' + actualMethodName + '(someValue,[options],callback)';
      ...
      // Aggregate finders
      if(method === 'findAllBy*' || method === 'findAllBy*In') return self.findAll(options, cb);
      if(method === 'findAllBy*Like') {
        criteria = _.extend(options, {
          where: {
            like: options.where
          }
        ...
        return self.findAll(criteria, cb);
      }
