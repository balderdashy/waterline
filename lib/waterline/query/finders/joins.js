...
Joins.prototype.modelOptions = function modelOptions() {
  ...
  joins = _.filter(this.joins, 'select');
  ...
  this.options.joins = joins.map(function(join) {
    ...
    var child = _(self.joins).filter(function(j) {
      return j.child !== join.parent;
    }).first();
...
Joins.prototype.makeModels = function makeModels() {
  ...
  if (!_.isArray(this.values)) return this.values;
...
Joins.prototype.modelize = function modelize(value) {
  ...
  _.forOwn(value, function(val, key) {
    ...
    if(Array.isArray(val)) {
      ...
      var records = _.map(val, function(val) {
        var collection,
            model;
...
Joins.prototype.checkForJoin = function checkForJoin(key) {
  ...
  var usedInJoin = _.some(this.joins, function(join) {
    if(join.alias === key) {
      relatedJoin = join;
      return true;
    }
