...
module.exports = {
  ...
  findOne: function(criteria, cb) {
    ...
    this._adapter.findOne(criteria, function(err, values) {
      ...
      // If this was a join, we will have an array here
      // if it wasn't a join lets just use the first item
      if((criteria.joins || criteria.join) && Array.isArray(values)) {
        var _values = [];
        ...
        values = self._transformer.group(_values);
      } else {
