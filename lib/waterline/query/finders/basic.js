...
var buildStageTwoQuery = require('../../utils/build-stage-two-query');
...
module.exports = {
  ...
  findOne: function(criteria, cb, metaContainer) {
    ...
    // If no criteria is selected, be sure to include all the values so any
    // populates get selected correctly.
    if(!criteria.select) {
      criteria.select = _.map(this._schema.schema, function(val, key) {
        return key;
      });
    }
  ...
  find: function(criteria, options, cb, metaContainer) {
    ...
    //  ╠╩╗║ ║║║   ║║  └─┐ │ ├─┤│ ┬├┤    │ ││││ │  │─┼┐│ │├┤ ├┬┘└┬┘
    //  ╚═╝╚═╝╩╩═╝═╩╝  └─┘ ┴ ┴ ┴└─┘└─┘   ┴ └┴┘└─┘  └─┘└└─┘└─┘┴└─ ┴
    // Ensures normalized format for query objects.
    var originalModelDefinitions = _.reduce(this.waterline.collections, function(result, val, key) {
      result[key] = val.attributes;
      return result;
    }, {});

    // Build up the skeleton for the stage one query
    var stageOneQueryObj = {
      method: 'find',
      using: this.identity,
      criteria: criteria,
    ...
    var queryObj;
    try {
      queryObj = buildStageTwoQuery(stageOneQueryObj, originalModelDefinitions);
    } catch (e) {
      return cb(e);
    }
