...
module.exports = {
  ...
  findOne: function(criteria, cb) {
    ...
    operations.run(function(err, values) {
      ...
      Object.keys(self._schema.schema).forEach(function(key) {
        if(_.has(self._schema.schema[key], 'primaryKey') && self._schema.schema[key].primaryKey) {
          primaryKey = key;
        }
      });
      ...
      Integrator(values.cache, criteria.joins, primaryKey, function(err, results) {
        ...
        criteria.joins.forEach(function(join) {
          if(!_.has(join, 'alias')) return;

          // Check for `OR` criteria
          if(_.has(tmpCriteria, 'or')) {
            tmpCriteria.or.forEach(function(search) {
              if(!_.has(search, join.alias)) return;
              delete search[join.alias];
            });
            return;
          }

          if(!_.has(tmpCriteria, join.alias)) return;
  ...
  find: function(criteria, options, cb) {
    ...
    var usage = _.capitalize(this.identity) + '.find([criteria],[options]).exec(callback|switchback)';
    ...
    operations.run(function(err, values) {
      ...
      Integrator(values.cache, criteria.joins, primaryKey, function(err, results) {
        ...
        criteria.joins.forEach(function(join) {
          ...
          if(_.has(tmpCriteria, join.alias)) delete tmpCriteria[join.alias];
        });
