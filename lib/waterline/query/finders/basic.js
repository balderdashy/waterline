...
module.exports = {
  ...
  findOne: function(criteria, cb) {
    ...
    operations.run(function(err, values) {
      // console.log('execed the findOne() query (probably for the save() query).., error?',err);
      ...
      // function.
      if(!criteria.joins) {
        // console.log('!criteria.joins  ---->   cache=',values.cache);
      ...
      if(values.combined) {
        // console.log('values.combined  ---->   values=',values);
      ...
      // console.log('about to hit integrator using...  ---->  ', values.cache, criteria.joins, primaryKey);

      // Perform in-memory joins
      Integrator(values.cache, criteria.joins, primaryKey, function(err, results) {
        // console.log('back from integrator ---->  ', 'err?',err,'results?',results);
