  ...
  findOne: function(criteria, cb) {
    ...
    this._adapter.findOne(criteria, function(err, values) {
      ...
      Object.keys(values[0]).forEach(function(key) {
        ...
        if(modelOptions.joins.indexOf(key) < 0) return;
        ...
        if(Array.isArray(values[0][key])) {
          ...
          values[0][key].forEach(function(val) {
            var model = new self.waterline.collections[key]._model(val);
            models.push(model);
          });
        ...
        values[0][key] = new self.waterline.collections[key]._model(values[0][key], { showJoins: true });
      });
        ...
        Object.keys(value).forEach(function(key) {
          ...
          if(Array.isArray(value[key])) {
            ...
            value[key].forEach(function(val) {
              var model = new self.waterline.collections[key]._model(val, { showJoins: false });
              records.push(model);
            });
          ...
          value[key] = new self.waterline.collections[key]._model(value[key], { showJoins: false });
        });
