 ...
 *
 * For each Waterline Collection set an empty array of values into the cahce.
 *
 * @api private
 */
...
Operations.prototype._runChildOperations = function _runChildOperations(intermediateResults, opt, cb) {
  ...
  if(!hasOwnProperty(opt, 'parent')) {
    return self._runOperation(opt.collection._adapter, opt.method, opt.criteria, cb);
  }
  ...
  res.forEach(function(result) {
    parents.push(result[opt.join.children[0].parentKey]);
  });

  var criteria = {};
  criteria[opt.join.parentKey] = parents;

  self._runOperation(opt.collection._adapter, opt.method, criteria, cb);
};
