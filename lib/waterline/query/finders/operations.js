...
var forgeStageThreeQuery = require('../../utils/forge-stage-three-query');
...
var Operations = module.exports = function operationBuilder(context, queryObj) {
  ...
  var stageThreeQuery = forgeStageThreeQuery({
    ...
    schema: context.waterline.schema,
  ...
  // Use a placeholder for the waterline schema attached to the context
  this.schema = context.waterline.schema;

  // Use a placeholder for the current collection identity
  this.currentIdentity = context.identity;

  // Use a placeholder for the current collection's internal adapter
  this.internalAdapter = context.adapter;
...
Operations.prototype.seedCache = function seedCache() {
  var cache = {};
  _.each(this.schema, function(val, collectionName) {
...
Operations.prototype.createParentOperation = function createParentOperation(connections) {
  ...
  // Deterine if the adapter supports native joins
  var nativeJoin = this.internalAdapter.hasJoin();
...
Operations.prototype.runOperation = function runOperation(operation, cb) {
  ...
  // Run the operation
  collection.adapter[queryObj.method](queryObj, this.metaContainer, cb);
};
...
Operations.prototype.buildChildOpts = function buildChildOpts(parentResults) {
  ...
  _.each(this.operations, function(item) {
    ...
    if (item.join.criteria) {
      ...
      _tmpCriteria = normalizeCriteria(_tmpCriteria);
...
Operations.prototype.findCollectionPK = function findCollectionPK(collectionName) {
  ...
  var attributes = collection._attributes;
  var pk;

  _.each(attributes, function(val, attributeName) {
    if (_.has(val, 'primaryKey') && val.primaryKey) {
      pk = val.columnName || attributeName;
    }
  });
