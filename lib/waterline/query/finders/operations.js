...
Operations.prototype._seedCache = function _seedCache() {
  ...
  _.forOwn(this.context.waterline.schema, function(val, key) {
    this.cache[key] = [];
  }, this);
};
...
Operations.prototype._stageOperations = function _stageOperations(connections) {
  ...
  _.forOwn(connections, function(conn, connection) {
    ...
    conn.joins.forEach(function(join, idx) {
      ...
      var child = _.some(localOpts, function(localOpt) {
        if(localOpt.join.child === join.parent) {
...
Operations.prototype._runOperation = function _runOperation(collectionName, method, criteria, cb) {
  ...
  if(!_.has(this.context.waterline.collections, collectionName)) {
...
Operations.prototype._buildChildOpts = function _buildChildOpts(parentResults, cb) {
  ...
  async.each(this.operations, function(item, next) {
    ...
    var parents = _.transform(parentResults, function(parents, result) {

      if(!_.has(result, item.join.parentKey)) return;
      if(result[item.join.parentKey] == null) return;
    ...
    if(_.has(_tmpCriteria, 'skip') || _.has(_tmpCriteria, 'limit')) {
      parents.forEach(function(parent) {
        ...
        if(_.has(_tmpCriteria, 'skip')) tmpCriteria.skip = _tmpCriteria.skip;
        if(_.has(_tmpCriteria, 'limit')) tmpCriteria.limit = _tmpCriteria.limit;
...
Operations.prototype._collectChildResults = function _collectChildResults(opts, cb) {
  ...
  if(_.size(opts) === 0) return cb(null, {});
...
Operations.prototype._runChildOperations = function _runChildOperations(intermediateResults, opt, cb) {
  ...
  var parents = _.map(res, opt.join.parentKey);
...
Operations.prototype._findCollectionPK = function _findCollectionPK(collectionName) {
  ...
  _.forIn(this.context.waterline.collections[collectionName]._attributes, function(attr, attribute) {
