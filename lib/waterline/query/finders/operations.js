...
Operations.prototype.run = function run(cb) {
  ...
  var adapters = {};
  _.keys(this.context.waterline.collections).forEach(function(adapterName) {
    adapters[adapterName] = self.context.waterline.collections[adapterName].adapter;
  });

  var obj = {
    criteria: _.cloneDeep(this.criteria),
    from: this.context.identity,
    method: this.parent,
    waterline: this.context.waterline,
    adapters: adapters,
    context: this.context
  };

  // Send the criteria down to the shim
  shim(obj, function(err, shimResults) {
    if (err) return cb(err);
    return cb(err, { combined: shimResults.preCombined||self.preCombined, cache: shimResults.cache });
  });
