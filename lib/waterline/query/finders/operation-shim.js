    ...
    if (joins[0].criteria) {
      ...
      var populateCriteria = {
        ...
        where: (function _buildPopulateWhere(){
          return _.merge(joins[0].criteria.where||{}, _.omit(joins[0].criteria, function (val,key) {
            return key==='skip'||key==='limit'||key==='sort'||key==='where';
          }));
        })(),

        limit: (function _buildPopulateLimit() {
          return joins[0].criteria.limit || undefined;
        })(),

        skip: (function _buildPopulateSkip() {
          return joins[0].criteria.skip || undefined;
        })(),

        sort: (function _buildPopulateSort() {
          return joins[0].criteria.sort || undefined;
        })()

      };

      // Merge in populate criteria
      _.merge(criteria.select[joins[0].alias], populateCriteria);
    }
  }
