...
module.exports = {
  ...
  join: function(collection, fk, pk, cb) {
    ...
    this._adapter.adapter.join(this.identity, collection, fk, pk, cb);
  },
  ...
  create: function(values, cb) {
    ...
    for(var key in this.attributes) {
      if(!values[key] && self.attributes[key].defaultsTo) {
        values[key] = self.attributes[key].defaultsTo;
      }
    }
    ...
    this._validator.validate(values, function(err) {
      ...
      self._adapter.adapter.create(self.collection, values, function(err, values) {
        if(err) return cb(err);
  ...
  update: function(options, newValues, cb) {
    ...
    this._validator.validate(newValues, function(err) {
      ...
      if(self.autoUpdatedAt) newValues.updated_at = new Date();
      ...
      // Pass to adapter
      self._adapter.adapter.update(self.collection, options, newValues, function(err, values) {
        if(err) return cb(err);
  ...
  updateWhere: this.update,
  updateAll: this.update,
  ...
  destroy: function(criteria, cb) {
    ...
    // Pass to adapter
    this._adapter.adapter.destroy(this.identity, criteria, cb);
  },

  destroyWhere: this.destroy,
  ...
  count: function(criteria, options, cb) {
    ...
    this._adapter.adapter.count(this.identity, criteria, cb);
  }
