...
module.exports = {
  ...
  create: function(values, cb) {
    ...
    async.series([
      ...
      // Automatically add updatedAt, createdAt and deletedAt (if enabled)
      if(self.autoCreatedAt) values.createdAt = new Date();
      if(self.autoUpdatedAt) values.updatedAt = new Date();
      if(self.softDelete) values.deletedAt = null;
  ...
  update: function(options, newValues, cb) {
    ...
    // Normalize criteria
    options = normalize.criteria(options, self._schema.schema);
  ...
  destroy: function(criteria, cb) {
    ...
    // Normalize criteria
    criteria = normalize.criteria(criteria, self._schema.schema);
    ...
    async.eachSeries(this._callbacks.beforeDestroy, runner, function(err) {
      ...
      // If softDelete is off
      if (!self.softDelete) {
        // Delete the record
        self._adapter.destroy(criteria, after);
      } else {
        // Update the record instead of deleting it
        self._adapter.update(criteria, {softDelete: new Date()}, after);
      }
    });
  },
  ...
  count: function(criteria, options, cb) {
    ...
    criteria = normalize.criteria(criteria, this._schema.schema);
