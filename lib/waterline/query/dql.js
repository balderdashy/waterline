...
module.exports = {
  ...
  /**
   * Finds a set of distinct values for a given attribute
   *
  ...
  distinct: function(attribute, criteria, options, cb) {
    var usage = utils.capitalize(this.identity) + '.distinct(attribute,[criteria],[options],callback)';
    ...
    // Return Deferred or pass to adapter
    if(typeof cb !== 'function') {
      return new Deferred(this, this.distinct, attribute, criteria);
    }
    ...
    if(_.isFunction(attribute) || _.isFunction(criteria) || _.isFunction(options)) {
      return usageError('Invalid options specified!', usage, cb);
    }
    ...
    this._adapter.distinct(attribute, criteria, cb);
  },
