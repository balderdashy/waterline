    ...
    if(attributes[pathElement]){
      ...
      if (childModel) {
        ...
        if(attributes[pathElement].collection){
          var parentPk = _.find(_.keys(attributes),function(attr){
          ...
          if(schema[junctionName] && schema[junctionName].junctionTable && schema[parent].attributes[pathElement].on){
            ...
            var manyToManyAttr = schema[parent].attributes[pathElement];
            // Find the other key in the junction table
            ...
            joins.push({
              parent: parent,
              parentKey: attributes[parentPk].columnName || parentPk,
              child: junctionName,
          ...
          // One To Many 
            ...
            joins.push({
              ...
              parentKey: attributes[parentPk].columnName || parentPk,
              child: childModel,
              childKey: childAttributes[attributes[pathElement].via].columnName || attributes[pathElement].via,
              alias: pathElement,
              removeParentKey: false,
              model: false,
              collection: true,
        ...
        // One to One or Many To One 
          ...
          joins.push({
            parent: parent,
            parentKey: attributes[pathElement].columnName || pathElement,
            child: childModel,
      ...
      parent = childModel;
      attributes = collections[childModel]._attributes;
      parentPath += '.' + pathElement;
        
      }else{
        throw 'The path "'+path+'" is not correct, the attribute "'+pathElement+'" is not a collection nor a model !';
      }
    }else{
      throw 'The path "'+path+'" is not correct, the attribute "'+pathElement+'" does not exist !';
    }
  }
  if (joins.length) {
    criteria.where = criteria.where || {};
    joins[joins.length - 1].criteria = criteria;
  }
  this._criteria.joins = Array.prototype.concat(this._criteria.joins || [], joins);
...
/**
 * verify if a join object exists
 */
Deferred.prototype.existJoin = function(parent, alias, path){
  if(!this._criteria.joins || !this._criteria.joins.length) return false;
  for(i in this._criteria.joins){
    var join = this._criteria.joins[i];
    if ( join.alias === alias && parent === this._context.identity && !join.path){
      join.path = path;
      return true;
    }
    if( join.alias === alias && path === join.path){
      return true;
    }
  }
  return false;
...
Deferred.prototype.execDeep = function(cb, deepJoins, cursor){
  ...
  if(!cursor){
    ...
    joins.forEach(function(join){
      var attributes = collections[join.child]._attributes;
      var childPk = _.find(_.keys(attributes),function(attr){
        return attributes[attr].primaryKey;
      });
      subPk[join.alias] = childPk;
    });

    var rootCriteria = _.cloneDeep(this._criteria);
    rootCriteria.joins = joins;
    
    var deferred = new Deferred(this._context,this._method, rootCriteria);
    deferred.exec(function(err, res){
      if(err) return cb(err);
      cursor = new DeepCursor(self._context.identity, subPk, res);
      deferred.execDeep(cb, self._criteria.joins, cursor);
    });
    ...
    async.each(this._criteria.joins, function(join, next) {
      ...
      var pathCursor = cursor.getChildPath(join.path + '.' + join.alias, subPk);
      
      //Retrieve only the attributes needed in the nextLayerParents to enchain layers (primary key and foreign key if exists)
      var select = _.pluck(_.filter(joins,function(join){
        return !join.junctionTable;
      }),'parentKey');
      select.push(pathCursor.upPks[join.alias]);
      
      var parents = pathCursor.getParents(); 
      
      var where = {};
      where[pathCursor.upPks[join.alias]] = parents;
      var criteria = {select : _.uniq(select), where : where, joins : joins};
      ...
      deferred.exec(function(err, newLevel){
        ...
        deferred.execDeep(next, deepJoins, pathCursor);
      });
    }, function(err){
      if(err) return cb(err);
 ...
 */
var getCurrentJoins = function(collectionName, joins, path){
  var currentJoins = _.filter(joins,function(join){
      return join.parent === collectionName && !join.junctionTable && path === join.path;
  });
...
Cursor.prototype.refs = {};

Cursor.prototype.index = function (pk,dest){
  if (!this.refs[pk]) {
    this.refs[pk] = [dest];
  }
  else {
    this.refs[pk].push(dest);
  }
  
};

Cursor.prototype.extend = function extend(pk,object){
  this.refs[pk].forEach(function(ref){
    _.extend(ref,object);
  });
};

Cursor.prototype.empty = function empty(){
  var self = this;
