...
Deferred.prototype.populate = function(keyName, criteria) {
  ...
  try {
    ...
    _.forOwn(this._context._attributes, function(attribute, key) {
      if(_.has(attribute, 'primaryKey') && attribute.primaryKey) {
        pk = attribute.columnName || key;
      }
    });
    ...
    join = {
      ...
      model: _.has(parentKey, 'model') ? true : false,
      collection: _.has(parentKey, 'collection') ? true : false
    };

    // Build select object to use in the integrator
    var select = [];
    _.forOwn(this._context.waterline.schema[attr.references].attributes, function(obj, key) {
      select.push(_.has(obj, 'columnName') ? obj.columnName : key);
    });
    ...
    if(this._context.waterline.schema[attr.references].junctionTable && _.has(attr, 'on')) {
      ...
      _.forOwn(reference, function(attribute, key) {
        if(!_.has(attribute, 'references')) {
          delete reference[key];
        }
        else if (_.has(attribute, 'columnName')) {
          if (attribute.columnName === attr.on) delete reference[key];
        }
        else if (key !== attr.on) {
          delete reference[key];
        }
      ...
      if(ref) {
        ...
        var selects = [];
        _.forOwn(this._context.waterline.schema[reference[ref].references].attributes, function(expanded, attr) {
          selects.push(expanded.columnName || attr);
        });
...
Deferred.prototype.where = function(criteria) {
  ...
  if (Array.isArray(criteria) && _.all(criteria, _.isObject)) {
  ...
  _.forOwn(criteria.where, function(val, key) {
    where[key] = val;
  });
...
Deferred.prototype.sort = function(criteria) {
  ...
  _.forOwn(criteria.sort, function(val, key) {
    sort[key] = val;
  });
