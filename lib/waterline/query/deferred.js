  ...
  for(var i = 0; i<splittedPath.length; i++){
    ...
    if(attributes[pathElement]){
      ...
      if (childModel) {
        ...
        if(parentPath === '.'){// if it's the path root, add joins to be treateds
          this._criteria.joins = Array.prototype.concat(this._criteria.joins || [], joins);
        }
...
Deferred.prototype.exec = function(cb) {
  ...
  if(this.deepJoins){
    args = [this._criteria, function(err,res){
      return self.processDeep(err, res, cb);
    }];
  }
...
Deferred.prototype.processDeep = function(err,res,cb){
  ...
  var self = this;
  var collections = this._context.waterline.collections;
  async.eachSeries(this._criteria.joins, function(join, next) {/* for every fetched population, perform its children populations */
    
      var parent = join.child;/* diving in deep, child become parent */
      var parents = getChildren(res, join.alias);/* retrieving last performed population records */
      if(!parents.length) return cb(null, res);
      
      var nextLayerJoins = getNextLayerJoins(join.child, self.deepJoins, join.path + join.alias);/* retrieving children joins of the current performed join */
      ...
      var criteria = {where : where, joins : nextLayerJoins};
      
      var childModel = new Deferred(collections[parent],self._method,criteria);
      childModel.deepJoins = self.deepJoins;
      childModel.exec(function(err,children){
 ...
 */
var getNextLayerJoins = function(modelName, joins, path){
  path += '.';
  var nextLayerJoins = _.filter(joins,function(dj){/* retreive the next joins (OneToMany, OneToOne or manyToOne */ 
      return dj.parent === modelName && !dj.junctionTable && path === dj.path;
  });
  ...
  nextLayerJoins.forEach(function(flJoin){/* flJoin => firstLayerJoin */
    var sl = _.filter(joins,function(dj){
        return (flJoin.child === dj.parent) && dj.junctionTable && dj.path === flJoin.path;
    }); 
