...
Deferred.prototype.execDeep = function(cb, deepJoins, res){
  ...
  if(!res){
    ...
    async.each(this._criteria.joins, function(join, next) {
      ...
      var lastLevelChildren = getChildren(res, join.alias);
      if(!lastLevelChildren.length) return cb(null, res);
      
      var joins = getCurrentJoins(join.child, deepJoins, join.path +'.'+ join.alias);
      if(!joins.length) return next();
      
      // finding the last level children pk
      ...
      var where = {};
      where[childPk] = _.uniq(_.pluck(lastLevelChildren, childPk));
      ...
      deferred.exec(function(err, newLevelParents){
        if(err) return next(err);
        var cursor = new Cursor;
        
        for (i in lastLevelChildren) {
          var child = lastLevelChildren[i];
          cursor.index(child[childPk], child);
        }
        
        for (j in newLevelParents) {
          var parent = newLevelParents[j];
          cursor.extend(parent[childPk], parent);
        }

        deferred.execDeep(next, deepJoins, newLevelParents);
      });
    }, function(err){
      if(err) return cb(err);
      return cb(null, res);
