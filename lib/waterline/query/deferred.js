    ...
    if(_.keys(this._criteria.paths[currentPath].children).indexOf(currentAlias) === -1){
      ...
      if(parentAttributes[currentAlias].collection){
        ...
        // Find the other key in the junction table
          ...
          this._criteria.paths[currentPath].children[currentAlias] = { collectionName : childModel, primaryKey : childPk, select : join1.parentKey };
        }
        // One To Many 
        else {
          var join = {
          ...
          this._criteria.paths[currentPath].children[currentAlias] = { collectionName : childModel, primaryKey : childPk, select : join.parentKey };
        }
      }
...
Deferred.prototype.execDeep = function(cb, cursor){
  ...
  if(!cursor){
    ...
    async.each(_.keys(previousChildren), function(alias, next) {
      ...
      var select = [currentPk];
      _.keys(currentChildren).forEach(function(alias){
        if(currentChildren[alias].select !== currentPk){
          select.push(currentChildren[alias].select);
        }
      });
