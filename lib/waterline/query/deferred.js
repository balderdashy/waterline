...
Deferred.prototype.populatePath = function(path, criteria) {
  ...
  if(!this._criteria.paths) this._criteria.paths = {};
  if(this._criteria.joins){
    ...
    for(i in this._criteria.joins){
      ...
      this._criteria.paths[currentPath].children[join.alias] = {model : join.child, pk : getPk(collections[join.child]._attributes)};
    }
  }
  for(var i = 0; i<pathChunks.length; i++){
    var currentAlias = pathChunks[i];
    if(parentAttributes[currentAlias]){
      ...
      if (childModel) {
        ...
        if(!this._criteria.paths[currentPath] || _.keys(this._criteria.paths[currentPath].children).indexOf(currentAlias) === -1){
          var childAttributes = collections[childModel].attributes;
          var parentPk = getPk(parentAttributes);
          var childPk = getPk(childAttributes);
          ...
          if(parentAttributes[currentAlias].collection){
            ...
            if(schema[junctionName] && schema[junctionName].junctionTable && schema[parent].attributes[currentAlias].on){
              ...
              if(!this._criteria.paths[currentPath]) this._criteria.paths[currentPath] = { joins : [], children : {}};
              this._criteria.paths[currentPath].joins.push(join1);
              this._criteria.paths[currentPath].joins.push(join2);
              this._criteria.paths[currentPath].children[currentAlias] = { model : childModel, pk : childPk };
            }
            // One To Many 
            else {
              var join = {
        ...
        throw 'The path "'+path+'" is not correct, the attribute "'+currentAlias+'" is not a collection nor a model !';
      }
    }else{
      throw 'The path "'+path+'" is not correct, the attribute "'+currentAlias+'" does not exist !';
    }
  }
...
Deferred.prototype.execDeep = function(cb, cursor){
  ...
  if(!cursor){
    ...
    cb = normalize.callback(cb);
    var joins = this._criteria.paths[this._context.identity].joins;
    var rootPathChildren = this._criteria.paths[this._context.identity].children;
    
    var subPk = {};
    _.keys(rootPathChildren).forEach(function (alias) {
      subPk[alias] = rootPathChildren[alias].pk;
    });
    ...
    var populatedChildren = self._criteria.paths[cursor.path].children;
    
    async.each(_.keys(populatedChildren), function(alias, next) {
      
      var previousPath = cursor.path;
      var currentPath = previousPath +'.'+ alias;
      var currentModel = populatedChildren[alias].model;
      ...
      var currentChildren = self._criteria.paths[currentPath].children;
      var subPk = {};
      _.keys(currentChildren).forEach(function (alias) {
        subPk[alias] = currentChildren[alias].pk;
      });

      var pathCursor = cursor.getChildPath(currentPath, subPk);
      ...
      select.push(pathCursor.upPks[alias]);
      
      var where = {};
      where[pathCursor.upPks[alias]] = _.uniq(pathCursor.getParents()); 
...
var getPk = function(attributes){
  return _.find(_.keys(attributes),function(attr){
