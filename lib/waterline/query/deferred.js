...
var Q = require('q'),
    ...
    redis = require('redis-fantastico'),
    async = require('async'),
...
Deferred.prototype.exec = function(cb) {
  ...
  if (self._cache) {
    ...
    var boundData = self._method.__bindData__;
    ...
    var identifier = crypto.createHash('md5').update(
      ...
      + boundData ? '' : boundData[0], // Get the function, bound behind lodash
    ...
    async.waterfall([
      ...
      async.apply(redis.getSlave().GET, cacheKey),
      ...
      function (results, callback) {
        redis.getMaster().SETEX([cacheKey, self._cache, JSON.stringify(results[1])], function (err) {
          if (err) throw err;
        });
