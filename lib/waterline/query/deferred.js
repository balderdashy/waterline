...
Deferred.prototype.populate = function(keyName) {
  var self = this,
      ...
      attr = this._context._attributes[keyName],
      pk = 'id',
      join;
  ...
  join = {
    ...
    collection: attr.references,
    pk: pk,
    fk: attr.on,
    select: true
  };

  if(this._context.tables[attr.references].junctionTable) join.select = false;

  joins.push(join);

  // If a junction table is used add an additional join to get the data
  if(this._context.tables[attr.references].junctionTable) {
    var reference = _.clone(this._context.tables[attr.references].attributes);
    delete reference[attr.on];

    join = {
      parent: attr.references,
      collection: attr.collection,
      pk: Object.keys(reference)[0],
      fk: reference[Object.keys(reference)[0]].on,
      select: true
    };
