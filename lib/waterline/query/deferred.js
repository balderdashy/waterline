...
Deferred.prototype.populatePath = function(path) {
  ...
  var pathInfos = verifyPath(collections, this._context.identity, splittedPath);
  if(!pathInfos) throw 'The path "'+path+'" is not correct, at least one attribute is not a model neither a collection !';
  this.pathToPopulate = {
    pathInfos : pathInfos,
    results : []
  };
  return this.populate(pathInfos[0].attributeToPopulate);
};
...
Deferred.prototype.processPath = function(infoIndex, resultIndex,cb){
  var pathInfos = this.pathToPopulate.pathInfos;
  if(infoIndex >= pathInfos.length) return cb(null ,this.pathToPopulate.results[0]);
  var self = this;
  var previousResults = this.pathToPopulate.results[resultIndex - 1];
  var info = pathInfos[infoIndex];
  var previousInfo = pathInfos[infoIndex - 1];
  var modelName = info.modelName;
  var collections = this._context.waterline.collections;
  var Model = collections[modelName];
  var parents = getParentsList(previousResults, previousInfo.attributeToPopulate, previousInfo.type);
  var parentPkValues = _.uniq(_.pluck(parents, info.type === 'collection'? info.parentPkAttr:info.parentFkAttr));
  
  if(info.jonctionModel){/* ManyToMany (Through) via a jonction model */
    var Where = {};
    Where[info.jonctionParentFk] = parentPkValues;
    var JonctionModel = collections[info.jonctionModel];
    JonctionModel.find(Where).populate(info.jonctionChildFk).exec(function(err,result){
      if(err) return cb(err);
      parents.forEach(function(parent){
        var associationRecords = _.filter(result,function(record){
          return parent[info.parentPkAttr] === record[info.jonctionParentFk];
        });
        var children = _.pluck(associationRecords,info.jonctionChildFk);
        parent[info.attributeToPopulate] = _.uniq(_.reduce(children, function(memo,item){
          return memo.concat(item);
        }),info.childPkAttr);
      });
      self.pathToPopulate.results.push(parents);
      self.processPath(infoIndex + 1, resultIndex + 1, cb);
    });
    
  }else{/* OneToMany or ManyToOne or OneToOne */
    var where = {};
    where[info.type === 'collection'? info.parentFkAttr : info.parentPkAttr] = parentPkValues;
    var query = Model.find({where : where});
    if(infoIndex +1 < pathInfos.length){
      query = query.populate(pathInfos[infoIndex + 1].attributeToPopulate);
    }
    query.exec(function(err,result){
      if(err) return cb(err);
      parents.forEach(function(parent){ 
        if(info.type === 'model' && _.isObject(parent[info.parentFkAttr])) return;/* this means that the reference is already populated */
        var children = _.filter(result,function(record){
          return parent[info.type === 'collection'? info.parentPkAttr:info.parentFkAttr] === record[info.type === 'collection'? info.parentFkAttr : info.parentPkAttr];
        });
        var toPopulate = info.attributeToPopulate.toString();
        if(children) parent[toPopulate] = info.type === 'collection'? children : children[0];
      });
      self.pathToPopulate.results.push(result);
      self.processPath(infoIndex + 2, resultIndex + 1, cb);
    }); 
  }
};

Deferred.prototype.execPopulatePath = function(err, firstLayer,cb) {
  if(err) return cb(err);
  this.pathToPopulate.results.push(firstLayer);
  this.processPath(1,1,cb);
};
...
Deferred.prototype.exec = function(cb) {
  ...
  var args = [this._criteria, function(err,res){
      if(self.pathToPopulate) return self.execPopulatePath(err,res,cb);
      return cb(err,res);
  }];
...
var verifyPath = function (collections ,modelName, splittedPath, modelsInfo) {
  if (!splittedPath.length)
    return modelsInfo;
  if (!modelsInfo)
    modelsInfo = [];
  var attributes = collections[modelName]._attributes;
  var pathElement = splittedPath[0];
  if(attributes[pathElement]){
    var parentPkAttr;
    var parentFkAttr;
    var childModel = attributes[pathElement].model || attributes[pathElement].collection;
    if (childModel) {
      var childAttributes = collections[childModel]._attributes;
      /* if it's a collection, the parent is the current model, the fk is the attribute name of parent model in the collection model*/
      if(attributes[pathElement].collection){
        parentPkAttr = _.find(_.keys(attributes),function(attr){
          return attributes[attr].primaryKey;
        });
        parentFkAttr = attributes[pathElement].via;
        
      }/* if it's a model, the parent is the model to populate, the fk is the attribute name of the parent model in the current model*/
      else{
        parentPkAttr = _.find(_.keys(childAttributes),function(attr){
          return childAttributes[attr].primaryKey;
        });
        parentFkAttr = pathElement;
      }
      var info = {
        modelName: childModel,
        type: attributes[pathElement].model ? 'model' : 'collection',
        attributeToPopulate: pathElement,/* name of the attrbiute to populate */
        parentPkAttr: parentPkAttr,/* name of the primary key attribute as described above */
        parentFkAttr: parentFkAttr/* name of the foreign key attribute as described above */
      };
      if(attributes[pathElement].collection && attributes[pathElement].through){
        var hasManyThrough = _.find(_.keys(childAttributes),function(attr){
          return childAttributes[attr].collection === modelName &&  childAttributes[attr].through === attributes[pathElement].through;
        });
        if(hasManyThrough){
          info.jonctionChildFk = childAttributes[hasManyThrough].via;/* Fk of parent in jonction */
          info.jonctionParentFk = attributes[pathElement].via;/* Fk of child in jonction */
          info.childPkAttr = _.find(_.keys(childAttributes),function(attr){
            return childAttributes[attr].primaryKey;
          });
          info.jonctionModel = childAttributes[hasManyThrough].through;/* name of the jonction table */
          delete info.parentFkAttr;/* replaced by jonctionParentFk in manyToMany through cases */
        }
      }
      modelsInfo.push(info);
      splittedPath.shift();
      return verifyPath(collections, childModel, splittedPath, modelsInfo);
    }
    return false;
  }
  return false;
...
var getParentsList = function(previousResults, toPopulateName, type){
