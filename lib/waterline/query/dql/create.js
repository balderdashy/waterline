...
function createBelongsTo(valuesObject, cb) {
  ...
  async.each(valuesObject.associations.models, function(item, next) {
    ...
    if(!_.isPlainObject(valuesObject.originalValues[item])) return next();
    ...
    model.create(valuesObject.originalValues[item]).exec(function(err, val) {
...
function createValues(valuesObject, cb) {
  ...
  self.adapter.create(valuesObject.values, function(err, values) {
    ...
    nestedModels.create.call(self, parentModel, valuesObject.originalValues, valuesObject.associations.collections, function(err) {
      if(err) return cb(err);
