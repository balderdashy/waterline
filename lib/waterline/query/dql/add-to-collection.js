...
module.exports = function addToCollection(targetRecordIds, collectionAttrName, associatedIds, done, metaContainer) {
  ...
  try {
    ...
    switch (e.code) {
      ...
      case 'E_INVALID_TARGET_RECORD_IDS':
        return done(new Error('Usage error: The target record ids (i.e. first argument) passed to `.addToCollection()` should be the ID (or IDs) of target records whose collection will be modified.\nDetails: '+e.message));

      case 'E_INVALID_COLLECTION_ATTR_NAME':
        return done(new Error('Usage error: The collection attr name (i.e. second argument) to `addToCollection()` should be the name of a collection association from this model.\nDetails: '+e.message));

      case 'E_INVALID_ASSOCIATED_IDS':
        return done(new Error('Usage error: The associated ids (i.e. third argument) passed to `.addToCollection()` should be the ID (or IDs) of associated records to add.\nDetails: '+e.message));
