...
module.exports = function find(criteria, options, cb, metaContainer) {
  ...
  if (_.isString(criteria) || _.isNumber(criteria)) {
    var tmpCriteria = {};
    tmpCriteria[this.primaryKey] = criteria;
  ...
  operations.run(function(err, values) {
    ...
    Integrator(values.cache, query.joins, primaryKey, function(err, results) {
      ...
      results.forEach(function(res) {
        ...
        query.joins.forEach(function(join) {
          console.log('JOIN', require('util').inspect(join, false, null));
          if (!join.criteria) {
            return;
          }
          var c = normalizeCriteria(join.criteria, join.identity, self.waterline);
