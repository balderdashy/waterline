...
module.exports = function populate (options) {
  ...
  return _.map(parentRows, function _insertJoinedResults (parentRow) {
    ...
    parentRow[alias] =
    _.transform(associatedChildRows, function (memo, childRow) {

      // Ignore child rows without an appropriate foreign key
      // to an instance in the REAL child collection.
      if (!childRow[childNamespace + childPK] && !childRow[childPK]) return;
      ...
      if (select) {
        ...
        childRow = _.transform(childRow, function (childRow, attrValue, attrName) {
