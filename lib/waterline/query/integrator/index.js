 ...
 * 
 * (would be an invalid attribute on a collection/model in userspace)
 * 
 * @type {String}
 */
var FOLD_KEY = ':::<-:::';
...
module.exports = function integrate(cache, joinInstructions, cb) {  
  ...
  var parentKey = joinInstructions[0].parentKey;
  console.log('\n\nPARENT::',parent, '\n', 'PARENT KEY::',parentKey);

  // Group the joinInstructions array by alias, so that `populateInstructions` 
  // contains a list of join instructions for each populated association.
