...
toObject.prototype.makeObject = function() {
  ...
  Object.keys(this.proto.associations).forEach(function(association) {
    ...
    if(self.proto._properties && self.proto._properties.joins) {

      // Build up a join key name based on the attribute's model/collection name
      var joinsName = association;
      if(self.proto.associations[association].model) joinsName = self.proto.associations[association].model.toLowerCase();
      if(self.proto.associations[association].collection) joinsName = self.proto.associations[association].collection.toLowerCase();

      // Check if the join was used
      if(self.proto._properties.joins.indexOf(joinsName) < 0) return;
    } else {
      return;
    }
    ...
    if(Array.isArray(self.object[association])) {
      ...
      self.object[association].forEach(function(item) {
        if(!hasOwnProperty(item, 'toJSON')) return;
