...
module.exports = function(context, proto, cb) {
  ...
  async.auto({
    ...
    // Otherwise there is no need and they can be removed from the proto.
    compareModelValues: function(next) {
      ...
      async.each(modelKeys, function(key, nextKey) {
        ...
        if(_.isEqual(currentVal, previousVal)) {
          delete proto[key];
        }
    ...
    updateRecord: ['compareModelValues', function(next) {
      new updateInstance(context, proto, function(err, data) {
        next(err, data);
      });
    }],
    ...
    deferred.resolve(proto.toObject());
