...
module.exports = function(context, proto, options, cb) {
  ...
  async.auto({
    ...
    persistRecord: ['compareModelValues', function(next) {
      ...
      if (primaryKey === null || primaryKey === undefined) {
        context.create(data, function(e, result) {
          ...
          new updateInstance(context, result.toObject(), mutatedModels, function(err, data) {
            next(err, data);
          });
        });
      } else {
    ...
    addAssociations: ['buildAssociationOperations', 'persistRecord', function(next, results) {
  ...
  function(err, results) {
    ...
    if (!results.persistRecord.length) {
    ...
    var obj = results.persistRecord[0].toObject();
