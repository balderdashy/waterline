...
module.exports = function(collection, records, cb) {
  ...
  function createNewRecord(associatedCollection, attribute, values, callback) {
    ...
    if(junctionTable) insertCollection = collection.waterline.collections[attribute.collection];
  ...
  function createManyToMany(associatedCollection, attribute, associationPK, callback) {
    ...
    // Grab the associated collection's primaryKey
    Object.keys(collection.waterline.schema[attribute.collection].attributes).forEach(function(key) {
      var attr = collection.waterline.schema[attribute.collection].attributes[key];
      if(attr.hasOwnProperty('collection') && attr.collection === collection.identity) {
