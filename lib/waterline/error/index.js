...
function duckType(err) {
  ...
  if (/* _isValidationError(err) || */ _isConstraintViolation(err)) {
    ...
    // validation errors.
    const er = new WLValidationError(err);
    if (er.details === 'Invalid attributes sent to undefined:\n • 0\n' && er.rawMessage) {
        er.code = err.code;
        er.details = er.reason = er.rawMessage = err.reason;
        er.text = JSON.stringify({
            code: er.code,
            message: '[Error (' + er.code + ')]: ' + er.details
        });
    }
    return er;
  }
