...
function WLError(err) {
  ...
  if ( _isValidationError(this.originalError) ) {
    ...
    this.prefix = ERRORTYPES.E_VALIDATION.prefix;
  }
  ...
  // i.e. constraint violation reported by adapter, like `unique`
    ...
    this.prefix = ERRORTYPES.E_CONSTRAINT.prefix;
  }
  ...
  // i.e. reported by adapter via `waterline-errors`
    ...
    this.prefix = ERRORTYPES.E_ADAPTER.prefix;
  ...
  //  bug. Or a bug in WL core.)
    ...
    this.prefix = ERRORTYPES.E_UNKNOWN.prefix;
  }
...
WLError.prototype.toString = function () {
    ...
    if (_.isString(this.originalError)) {
      return this.prefix+this.originalError;
    }
    else if (_.isObject(this.originalError)) {
      // Run toString() on Errors
      if ( this.originalError instanceof Error && _.isFunction(this.originalError.toString) ) {
        return this.prefix+this.originalError.toString();
      }
      // But for other objects, use util.inspect()
      return this.prefix + util.inspect(this.originalError);
    }
    else {
      var stringifiedErr;
      ...
      return this.prefix+stringifiedErr;
    }
};