...
module.exports = function helpReplaceCollection(query, orm, done) {

  // Validate arguments
  if (_.isUndefined(query) || !_.isObject(query)) {
    return done(new Error('Consistency violation: Invalid arguments - missing or invalid `query` argument (a stage 2 query).'));
  }

  if (_.isUndefined(orm) || !_.isObject(orm)) {
    return done(new Error('Consistency violation: Invalid arguments - missing or invalid `orm` argument.'));
  }
  ...
  if (manyToMany) {
    ...
    WLChild.destroy(criteriaOfDestruction, function $afterDestroyingChildRecords(err) {
      ...
      //  ╠╦╝║ ║║║║  │  ├┬┘├┤ ├─┤ │ ├┤   │─┼┐│ │├┤ ├┬┘└┬┘
      //  ╩╚═╚═╝╝╚╝  └─┘┴└─└─┘┴ ┴ ┴ └─┘  └─┘└└─┘└─┘┴└─ ┴
      WLChild.createEach(insertRecords, done, modifiedMeta);
