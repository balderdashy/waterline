...
module.exports = function helpReplaceCollection(query, orm, cb) {
  ...
  if (_.isUndefined(query) || !_.isObject(query)) {
    return cb(new Error('Consistency violation: Invalid arguments - missing or invalid `query` argument (a stage 2 query).'));
  }

  if (_.isUndefined(orm) || !_.isObject(orm)) {
    return cb(new Error('Consistency violation: Invalid arguments - missing or invalid `orm` argument.'));
  }
