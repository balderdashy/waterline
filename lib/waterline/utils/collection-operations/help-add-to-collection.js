...
module.exports = function helpAddToCollection(query, orm, done) {
  ...
  if (_.isUndefined(orm) || !_.isPlainObject(orm)) {
    return done(new Error('Consistency violation: Invalid arguments - missing `orm` argument.'));
  }
  ...
  if (manyToMany) {
    ...
    //  ╠╦╝║ ║║║║  │─┼┐│ │├┤ ├┬┘└┬┘
    //  ╩╚═╚═╝╝╚╝  └─┘└└─┘└─┘┴└─ ┴
    WLChild.createEach(joinRecords, done, modifiedMeta);
  ...
  WLChild.update(criteria, valuesToUpdate, done, modifiedMeta);
