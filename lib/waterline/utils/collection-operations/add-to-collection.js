...
module.exports = function addToCollection(query, orm, cb) {
  // Validate arguments
  ...
  var WLModel = orm.collections[query.using.toLowerCase()];
  ...
  if (schemaDef.referenceIdentity.toLowerCase() !== Object.getPrototypeOf(WLChild).identity.toLowerCase()) {
    manyToMany = true;
    WLChild = orm.collections[schemaDef.referenceIdentity.toLowerCase()];
  }
  ...
  if (manyToMany) {
    ...
    //  ╠╦╝║ ║║║║  │─┼┐│ │├┤ ├┬┘└┬┘
    //  ╩╚═╚═╝╝╚╝  └─┘└└─┘└─┘┴└─ ┴
    return WLChild.createEach(joinRecords, cb, query.meta);
  }
