...
Joins.prototype.modelOptions = function modelOptions() {
  ...
  this.options.joins = _.map(joins, function(join) {
    var child = [];
    ...
    _.each(self.joins, function(j) {
      if (j.child !== join.parent) {
        return;
      }
      child.push(j.alias);
    });
...
Joins.prototype.makeModels = function makeModels() {
  ...
  if (!this.values || !this.values.forEach) {
    return this.values;
  }
...
Joins.prototype.modelize = function modelize(value) {
  ...
  _.keys(value).forEach(function(key) {
    ...
    if (!self.options.showJoins || !self.options.joins) {
      return;
    }
    ...
    var transformer = self.collections[self.identity.toLowerCase()]._transformer._transformations;
    if (_.has(transformer, key)) {
    ...
    if (joinKey && _.indexOf(self.options.joins, joinKey) < 0) {
      return;
    }
    ...
    var collection = self.collections[joinKey];
