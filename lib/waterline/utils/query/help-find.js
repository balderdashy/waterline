  ...
  (function _getPopulatedRecords(proceed){
    ...
    var doJoinsInParentAdapter = (function () {
      ...
      _.all(parentQuery.joins, function(join) {
        ...
        return collections[join.childCollectionIdentity].adapterDictionary.find === WLModel.adapterDictionary.find;
      });
    })();
      ...
      parentAdapter.find(parentDatastoreName, parentQueryWithoutJoins, function(err, parentResults) {
        ...
        async.reduce(_.keys(joinsByAlias), parentResults, function(populatedParentRecords, alias, nextSetOfJoins) {
          ...
          if (aliasJoins.length === 2) {
            ...
            // Note that we're not using this to normalize the `where` clause (we'll set up `where` manually).
            normalizeCriteria(junctionTableQuery.criteria, firstJoin.childCollectionIdentity, WLModel.waterline);

            // Start our where clause with an empty `and` array.
            junctionTableQuery.criteria = { where: { and: [] }};
