...
module.exports = function helpFind(WLModel, s2q, done) {
  ...
  (function _getPopulatedRecords(proceed){
    ...
    // Otherwise we have some joining to do...
      ...
      console.log('joinsByAlias');
      console.dir(joinsByAlias, {depth: null});
      ...
      parentAdapter.find(parentDatastoreName, parentQuery, function(err, parentResults) {
        ...
        async.reduce(_.keys(joinsByAlias), parentResults, function(populatedParentRecords, alias, nextSetOfJoins) {
          ...
          if (aliasJoins.length === 2) {
            ...
            // The first query we want to do is from the parent table to the join table.
            ...
            // Start building the query to the join table.
            var joinTableQuery = {
            ...
            // Get a reference to the join table model.
            var joinTableModel = collections[firstJoin.childCollectionIdentity];

            // Grab all of the primary keys found in the parent query, and add them as an `in` clause
            // to the join table query criteria.
            var joinTableQueryInClause = {};
            joinTableQueryInClause[firstJoin.childKey] = {in: _.pluck(parentResults, firstJoin.parentKey)};
            joinTableQuery.criteria.where.and.push(joinTableQueryInClause);

            // Normalize the query criteria (sets skip, limit and sort to expected defaults).
            normalizeCriteria(joinTableQuery.criteria, firstJoin.childCollectionIdentity, WLModel.waterline);
            ...
            joinTableQuery.criteria.select = [ firstJoin.childKey, secondJoin.parentKey ];

            // We now have a valid "stage 3" query, so let's run that and get the join table results.
            // First, figure out what datastore the join table is on.
            var joinTableDatastoreName = joinTableModel.adapterDictionary.find;
            // Next, get the adapter for that datastore.
            var joinTableAdapter = joinTableModel.datastores[joinTableDatastoreName].adapter;
            // Finally, run the query on the adapter.
            joinTableAdapter.find(joinTableDatastoreName, joinTableQuery, function(err, joinTableResults) {

              if (err) { return nextSetOfJoins(err); }

              // Okay!  We have a set of records from the join table.
              // For example:
              ...
              // Get the unique parent primary keys from the join table result.
              var parentPks = _.uniq(_.pluck(joinTableResults, firstJoin.childKey));
              ...
              async.reduce(parentPks, {}, function(memo, parentPk, nextParentPk) {
                ...
                var joinTableRecordsForThisParent = _.filter(joinTableResults, function(row) {
                  return row[firstJoin.childKey] === parentPk;
                });

                // Create the "in" clause for the query.
                var childPks = _.pluck(joinTableRecordsForThisParent, secondJoin.parentKey);
                var inClause = {};
          ...
          // Otherwise there's one join in the set, so no junction table.
            ...
            return nextSetOfJoins(null, memo);
...
FQRunner.prototype.createParentOperation = function (datastores) {
  ...
  if (doesAdapterSupportNativeJoin) {
    ...
    console.log('datastore.collections', datastore.collections);
    ...
    var allJoinsAreSupported = _.all(datastore.joins, function(join) {
      return _.indexOf(datastore.collections, join.childCollectionIdentity) > -1;
    });

    if (allJoinsAreSupported) {
      console.log('allJoinsAreSupported', allJoinsAreSupported);
      ...
      console.log('originalS3Q', originalS3Q);
...
FQRunner.prototype.runOperation = function runOperation(operation, cb) {
  ...
  // Run the operation
  adapter[queryObj.method](datastoreName, queryObj, cb);
};
  ...
  _.each(this.operations, function(item) {
    ...
    if (item.queryObj.join.criteria) {
      ...
      if (_.has(userlandCriteria, 'where')) {
        if (!_.isUndefined(userlandCriteria.where) && _.keys(userlandCriteria.where).length) {
          criteria.where.and = criteria.where.and.concat(userlandCriteria.where.and || [userlandCriteria.where]);
          // delete userlandCriteria.where;
        }
      }


      if (_.isUndefined(userlandCriteria.sort)) { criteria.sort = userlandCriteria.sort; }
      if (_.isUndefined(userlandCriteria.skip)) { criteria.skip = userlandCriteria.skip; }
      if (_.isUndefined(userlandCriteria.limit)) { criteria.limit = userlandCriteria.limit; }

      // criteria = _.merge({}, userlandCriteria, criteria);

    }//>-  </if join instruction contains `criteria`>

    // Otherwise, set default skip and limit.
    else {
      criteria.skip = 0;
    ...
    if (criteria.skip > 0 || criteria.limit !== (Number.MAX_SAFE_INTEGER || 9007199254740991)) {


      _.each(parents, function(parent) {

        // Make a clone of the criteria so that our query will contain userland criteria.
        var tmpCriteria = _.merge({}, criteria);

        // Add the parent ID to the "and" clause of the where query
        var andObj = {};
        andObj[item.queryObj.join.childKey] = parent;
        tmpCriteria.where.and.push(andObj);

        // Mixin the user defined skip and limit
        if (_.has(criteria, 'skip')) {
          tmpCriteria.skip = criteria.skip;
        }

        if (_.has(criteria, 'limit')) {
          tmpCriteria.limit = criteria.limit;
        }
      ...
      var andObj = {};
      andObj[item.queryObj.join.childKey] = {'in': parents};
      criteria.where.and.push(andObj);
    ...
    localOpts.push({
      collectionName: item.child.collectionName,
      queryObj: {
        method: item.queryObj.method,
        using: self.collections[item.child.collectionName].tableName,
      },
      parent: idx,
      join: item.child.queryObj.join
    });
...
FQRunner.prototype.runChildOperations = function runChildOperations(intermediateResults, opt, cb) {
  ...
  var andObj = {};
  andObj[opt.join.childKey] = {'in': parents};
  criteria.where.and.push(andObj);

  // Check if the join contains any criteria
  if (opt.join.criteria) {
    var userlandCriteria = _.extend({}, opt.join.criteria);
    ...
    if (_.isUndefined(userlandCriteria.sort)) { delete userlandCriteria.sort; }
    if (_.isUndefined(userlandCriteria.skip)) { delete userlandCriteria.skip; }
    if (_.isUndefined(userlandCriteria.limit)) { delete userlandCriteria.limit; }
  ...
  // Add the criteria object we built up to the child stage 3 query.
  opt.queryObj.criteria = criteria;
