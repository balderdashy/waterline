...
module.exports = function helpFind(WLModel, s2q, done) {
  ...
  assert(WLModel, 'Live Waterline model should be provided as the 1st argument');
  assert(s2q, 'Stage two query (S2Q) should be provided as the 2nd argument');
  assert(_.isFunction(done), '`done` (3rd argument) should be a function');
  ...
  fqRunner.run(function _afterRunningFindOperations(err, values) {
    ...
    (function _roundUpResults(proceed){
      try {
        ...
        var joinedResults;
        try {
          joinedResults = InMemoryJoin(initialS3Q, values.cache, WLModel.primaryKey);
      ...
      try {
        ...
        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        // assert(_.isArray(results), '`results` from `find` method in adapter should always be an array!  If you are seeing this error, either there is a bug in this database adapter, or some heretofore unseen issue in Waterline.');
        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        ...
        assert(_.isArray(joins), '`joins` must be an array at this point.  But isntead, somehow it is this: '+util.inspect(joins, {depth:5})+'');
        var data;
        try {
...
FQRunner.prototype.createParentOperation = function (datastores) {
  ...
  if (doesAdapterSupportNativeJoin) {
    assert.equal(ParentWLModel.adapterDictionary.join, datastoreName, 'The `join` adapter method should not be pointed at a different datastore!  (Per-method datastores are longer supported.)');
      ...
      if (_.has(userCriteria, 'where')) {
        if (_.isUndefined(userCriteria.where)) {
          ...
          if (_.isArray(userCriteria.where)) {
            var pk = self.collections[item.queryObj.join.childCollectionIdentity].primaryKey;
            var obj = {};
...
FQRunner.prototype.runChildOperations = function runChildOperations(intermediateResults, opt, cb) {
  ...
  self.runOperation(opt, function(err, values) {
    ...
    self.cache[opt.join.parentCollectionIdentity] = _.uniq(self.cache[opt.join.parentCollectionIdentity], pk);
...
//  ├─┘├┬┘││││├─┤├┬┘└┬┘  ├┴┐├┤ └┬┘
//  ┴  ┴└─┴┴ ┴┴ ┴┴└─ ┴   ┴ ┴└─┘ ┴
FQRunner.prototype.findCollectionPK = function (identity) {
  // TODO: name this differently (it's simply a "model" now)
  // (also would be good to clarify that this is the column name of the pk -- not the attr name!)
  var WLModel = this.collections[identity];
  var pk = WLModel.primaryKey;
  return WLModel.schema[pk].columnName;
};
