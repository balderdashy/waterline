      ...
      parentAdapter.find(parentDatastoreName, parentQueryWithoutJoins, function(err, parentResults) {
        ...
        async.reduce(_.keys(joinsByAlias), parentResults, function(populatedParentRecords, alias, nextSetOfJoins) {
          ...
          if (aliasJoins.length === 2) {
            ...
            // Grab all of the primary keys found in the parent query, and add them as an `in` constraint
            // to the junction table query criteria.
            var junctionTableQueryInConstraint = {};
            junctionTableQueryInConstraint[firstJoin.childKey] = {in: _.pluck(parentResults, firstJoin.parentKey)};
            junctionTableQuery.criteria.where.and.push(junctionTableQueryInConstraint);
            ...
            junctionTableAdapter.find(junctionTableDatastoreName, junctionTableQuery, function(err, junctionTableResults) {
              ...
              async.reduce(parentPks, {}, function(memo, parentPk, nextParentPk) {
                ...
                // Create an `in` clause for our child table query that looks for just thosr primary keys.
                var inClause = {};
          ...
          // Otherwise there's one join in the set, so no junction table.
            ...
            // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
            // TODO: figure out why this && short-circuit is necessary -- should prbly be
            // replaced with an assertion.  There should always be a `where` clause.
            // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
            ...
            if (singleJoin.criteria.where && _.keys(singleJoin.criteria.where).length > 0) {
              ...
              if (singleJoin.criteria.where.and) {
                baseChildTableQuery.criteria.where.and.push(singleJoin.criteria.where.and);
              }
              // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
              // TODO: believe this is the same problem as the other place-- this needs an `else`, and `.push`
              // should be `.concat`
              // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
              // Otherwise push the whole "where" clause in to the "and" array.
            ...
            async.map(populatedParentRecords, function(parentRecord, nextParentRecord) {
              ...
              // Create the query clause that looks for child records that reference this parent record's PK value.
              var pkClause = {};

              // Look for child records whose join key value matches the parent record's join key value.
              pkClause[singleJoin.childKey] = parentRecord[singleJoin.parentKey];
              childTableQuery.criteria.where.and.push(pkClause);

              // We now have another valid "stage 3" query, so let's run that and get the child table results.
              childTableAdapter.find(childTableDatastoreName, childTableQuery, function(err, childTableResults) {

                if (err) {return nextParentRecord(err);}
                ...
                // If this is a to-one join, add the single result to the join key column
