...
module.exports = function normalizeNewRecord(newRecord, modelIdentity, orm, ensureTypeSafety) {
  ...
  _.each(_.keys(newRecord), function (supposedAttrName){
    ...
    try {
      normalizeValueToSet(newRecord[supposedAttrName], supposedAttrName, modelIdentity, orm, ensureTypeSafety);
    } catch (e) {
      switch (e.code) {
        ...
        case 'E_HIGHLY_IRREGULAR':
          ...
          // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
          //
          // TODO: refactor this stuff out and use `normalizeValueToSet()` utility
          //
          // on E_HIGHLY_IRREGULAR, use chained try/catch, w/ prefix like:
          // ```
          // 'New record contains an invalid key (`'+supposedAttrName+'`): '+e.message
          // ```
          //
          // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
          throw e;
