...
module.exports = function normalizeCriteria(criteria, modelIdentity, orm, ensureTypeSafety) {
  ...
  if (_.isArray(criteria.sort) && criteria.sort.length === 0) {
  ...
  if (_.isObject(criteria.sort) && !_.isArray(criteria.sort) && !_.isFunction(criteria.sort)) {
    ...
    // Otherwise, attempt to normalize this dictionary into array format under the
    // assumption that it was provided as a Mongo-style comparator dictionary.
    // (and freaking out if we see anything that makes us uncomfortable)
    // TODO
    throw new Error('Compatibility for mongo-style sort clause not implemented yet');
  ...
  criteria.sort = _.map(criteria.sort, function (comparatorDirective){
  ...
  // --• At this point, we should be dealing with a properly-formatted array of comparator directives.
