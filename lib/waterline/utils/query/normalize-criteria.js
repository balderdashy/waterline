...
module.exports = function normalizeCriteria(criteria, modelIdentity, orm, ensureTypeSafety) {
  ...
  if (_.isObject(criteria.sort) && !_.isArray(criteria.sort) && !_.isFunction(criteria.sort)) {
    ...
    criteria.sort = _.reduce(_.keys(criteria.sort), function (memo, sortByKey) {

      var sortDirection = criteria.sort[sortByKey];
      ...
      if (_.isString(sortDirection) && _.keys(criteria.sort).length === 1) {
        throw flaverr('E_HIGHLY_IRREGULAR', new Error(
