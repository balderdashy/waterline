...
module.exports = function normalizeCriteria(criteria, modelIdentity, orm) {
  ...
  _.each(criteria.omit, function (attrNameToOmit){
    ...
    if (WLModel.hasSchema === true) {
      ...
      // Make sure this matches a recognized attribute name.
      try {
        getAttribute(attrNameToOmit, WLModel.identity, orm);
      } catch (e){
        switch (e.code) {
          case 'E_ATTR_NOT_REGISTERED':
            throw flaverr('E_HIGHLY_IRREGULAR', new Error(
              'The `omit` clause in the provided criteria contains an item (`'+attrNameToOmit+'`) which is not a recognized attribute in this model (`'+WLModel.identity+'`).'
            ));
          default: throw e;
        }
      }//</catch>
    ...
    // Else if model is `schema: false`...
      ...
      throw flaverr('E_HIGHLY_IRREGULAR', new Error(
        'Cannot use `omit`, because the referenced model (`'+WLModel.identity+'`) does not declare itself `schema: true`.'
      ));
