 ...
 *         (If provided as `undefined`, it will be rejected!  Deal with that edge case before calling this utility!)
 *         > WARNING:
...
module.exports = function normalizeValueToSet(value, supposedAttrName, modelIdentity, orm, ensureTypeSafety) {
  ...
  // ================================================================================================
  assert(!_.isUndefined(value), new Error('Consistency violation: `value` must not be undefined. (This should be handled _before_ calling the normalizeValueToSet() utility.  Remember: in Sails/Waterline, we always treat keys with `undefined` values as if they were never there in the first place)'));
  ...
  if (!correspondingAttrDef) {
    ...
    try {
      ...
      switch (e.code) {
        case 'E_INVALID_PK_VALUE':
          throw flaverr('E_HIGHLY_IRREGULAR', new Error('If specifying the value for a plural (`collection`) association, you must do so by providing an array of associated ids.  But instead, for `'+supposedAttrName+'`, got: '+util.inspect(value, {depth:null})+''));
        default: throw e;
      }
    }
