...
module.exports = function transformPopulatedChildRecords(joins, records, WLModel) {
  ...
  // Sanity checks.
  if (!(_.isArray(joins))) { throw new Error('Failed check: `_.isArray(joins)`'); }
  if (!(_.isArray(records))) { throw new Error('Failed check: `_.isArray(records)`'); }
  if (!(_.isObject(WLModel))) { throw new Error('Failed check: `_.isObject(WLModel)`'); }
  if (!(_.isString(WLModel.identity))) { throw new Error('Failed check: `_.isString(WLModel.identity)`'); }
  if (!(_.isObject(WLModel.waterline))) { throw new Error('Failed check: `_.isObject(WLModel.waterline)`'); }
  if (!(_.isObject(WLModel.schema))) { throw new Error('Failed check: `_.isObject(WLModel.schema)`'); }
  ...
  _.each(records, function(record) {
    _.each(_.keys(record), function(key) {
      ...
      if (!_.isArray(record[key])) {
        if (!(joinKey)) { throw new Error('IWMIH, `joinKey` should always be truthy!  But instead, it is: '+joinKey); }
