    ...
    _.each(_.keys(record), function(key) {
      ...
      if (_.has(attr, 'referenceIdentity')) {
        joinKey = (function() {
          ...
          // Find the joins in this query that refer to the current key.
          var joinsUsingThisAlias = _.where(joins, { alias: key });
          // If there are no such joins, return `null`, signalling that we can continue to the next
          // key in the record (there's nothing to transform).
          if (joinsUsingThisAlias.length === 0) {
            return null;
          }
          // If the join is utilizing a junction table, return the identity of the child model
      ...
      if (!_.isArray(record[key])) {
        if (!(joinKey)) { throw new Error('Consistency violation: iWMIH, `joinKey` should always be truthy!  But instead, it is: '+joinKey); }
