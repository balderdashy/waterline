...
module.exports = function transformPopulatedChildRecords(joins, records, WLModel) {
  ...
  // Sanity checks.
  //¶_¶ assert(_.isArray(joins), 'Failed check: `_.isArray(joins)`');
  //¶_¶ assert(_.isArray(records), 'Failed check: `_.isArray(records)`');
  //¶_¶ assert(_.isObject(WLModel), 'Failed check: `_.isObject(WLModel)`');
  //¶_¶ assert(_.isString(WLModel.identity), 'Failed check: `_.isString(WLModel.identity)`');
  //¶_¶ assert(_.isObject(WLModel.waterline), 'Failed check: `_.isObject(WLModel.waterline)`');
  //¶_¶ assert(_.isObject(WLModel.schema), 'Failed check: `_.isObject(WLModel.schema)`');
  ...
  _.each(records, function(record) {
    _.each(_.keys(record), function(key) {
      ...
      if (!_.isArray(record[key])) {
        //¶_¶ assert(joinKey, 'IWMIH, `joinKey` should always be truthy!  But instead, it is: '+joinKey);
