...
module.exports = function transformPopulatedChildRecords(joins, records, WLModel) {
  ...
  // Sanity checks:
  if (!_.isArray(joins)){
    throw new Error('Consistency violation: Failed check: `_.isArray(joins)`');
  }
  if (!_.isArray(records)){
    throw new Error('Consistency violation: Failed check: `_.isArray(records)`');
  }
  if (!_.isObject(WLModel)){
    throw new Error('Consistency violation: Failed check: `_.isObject(WLModel)`');
  }
  if (!_.isString(WLModel.identity)){
    throw new Error('Consistency violation: Failed check: `_.isString(WLModel.identity)`');
  }
  if (!_.isObject(WLModel.waterline)){
    throw new Error('Consistency violation: Failed check: `_.isObject(WLModel.waterline)`');
  }
  if (!_.isObject(WLModel.schema)){
  ...
  // Set up a common local var for convenience / familiarity.
