...
module.exports = function transformPopulatedChildRecords(joins, records, WLModel) {
  ...
  assert(_.isArray(records), 'Failed check: `_.isArray(records)`');
  ...
  _.each(records, function(record) {
    _.each(_.keys(record), function(key) {
      ...
      // Look up the join instruction where this key is used.
      var usedInJoin = _.find(joins, { alias: key });

      // Then look up the alternative child model indicated by that.
      // (we may end up using this below.)
      var WLAlternativeChildModel = getModel(usedInJoin.childCollectionIdentity, orm);
      ...
      if (!_.isArray(record[key])) {
        assert(joinKey, 'IWMIH, `joinKey` should always be truthy!  But instead, it is: '+joinKey);
      ...
      _.each(record[key], function(originalChildRecord) {
        ...
        // at the join instruction instruction that was used.
          ...
          transformedChildRecord = collection._transformer.unserialize(originalChildRecord);
        }
