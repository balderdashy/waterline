...
module.exports = function forgeStageThreeQuery(options) {
  ...
  if (s3Q.method === 'find' || s3Q.method === 'findOne') {
    ...
    _.each(s3Q.populates, function(populateCriteria, populateAttribute) {
      ...
      try {
        ...
        var join = {
          ...
          parentKey: model.schema[modelPrimaryKey].columnName,
          ...
          childKey: originalModels[parentAttr.references].schema[parentAttr.on].columnName,
        ...
        if (reference && _.has(schemaAttribute, 'on')) {
          ...
          join = {
            ...
            childKey: referencedSchema.schema[reference.on].columnName,
            alias: populateAttribute,
            junctionTable: true,
            removeParentKey: !!parentAttr.foreignKey,
            model: false,
    ...
    // processing needs.
    if (_.indexOf(s3Q.criteria.select, '*') < 0) {
    ...
    if (_.indexOf(s3Q.criteria.select, '*') > -1) {
      var selectedKeys = [];
