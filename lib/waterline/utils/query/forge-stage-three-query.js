...
module.exports = function forgeStageThreeQuery(options) {
  ...
  var model;
  try {
    model = originalModels[identity.toLowerCase()];
  } catch (e) {
  ...
  if (stageTwoQuery.method === 'find' || stageTwoQuery.method === 'findOne') {
    ...
    _.each(stageTwoQuery.populates, function(populateCriteria, populateAttribute) {
      ...
      if (!populateCriteria || !_.keys(populateCriteria).length) {
        return;
      }
      ...
      try {
        ...
        var parentAttr = originalModels[identity.toLowerCase()].schema[attributeName];
        ...
        _.each(originalModels[parentAttr.referenceIdentity.toLowerCase()].schema, function(val, key) {
        ...
        var childPk = originalModels[parentAttr.referenceIdentity.toLowerCase()].primaryKey;
        select.push(childPk);
        ...
        var referencedSchema = originalModels[parentAttr.referenceIdentity.toLowerCase()];
        var reference = null;
        ...
        else if (referencedSchema.throughTable && referencedSchema.throughTable[identity.toLowerCase() + '.' + populateAttribute]) {
          join.select = false;
          reference = referencedSchema.schema[referencedSchema.throughTable[identity.toLowerCase() + '.' + populateAttribute]];
        }
        ...
        if (reference && _.has(schemaAttribute, 'on')) {
          var selects = [];
          _.each(originalModels[reference.referenceIdentity.toLowerCase()].schema, function(val, key) {
            // Ignore virtual attributes
            if(_.has(val, 'collection')) {
              return;
            }
          ...
          childPk = originalModels[reference.referenceIdentity.toLowerCase()].primaryKey;
    ...
    if (stageTwoQuery.joins.length) {
      ...
      var joinCollection = originalModels[lastJoin.childCollectionIdentity.toLowerCase()];

      // Ensure a join criteria exists
      lastJoin.criteria = lastJoin.criteria || {};

      // Move the select onto the criteria for normalization
      lastJoin.criteria.select = lastJoin.select;
