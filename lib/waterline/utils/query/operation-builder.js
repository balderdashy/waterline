...
var Operations = module.exports = function operationBuilder(context, queryObj) {
  ...
  this.currentIdentity = context.identity;
...
Operations.prototype.run = function run(cb) {
  ...
  this.runOperation(parentOp, function(err, results) {
    ...
    self.cache[parentOp.collectionName.toLowerCase()] = results;
...
Operations.prototype.seedCache = function seedCache() {
  ...
  _.each(this.collections, function(val, collectionName) {
    cache[collectionName.toLowerCase()] = [];
  });
...
Operations.prototype.buildOperations = function buildOperations() {
  ...
  if (!_.keys(this.queryObj.joins).length) {
    // Grab the collection
    var collection = this.collections[this.currentIdentity.toLowerCase()];
    if (!collection) {
...
Operations.prototype.stageOperations = function stageOperations(connections) {
  ...
  var parentCollection = this.collections[this.currentIdentity.toLowerCase()];
  ...
  _.each(connections, function(val, connectionName) {
    ...
    _.each(val.joins, function(join, idx) {
      ...
      // in the join method.
      var optCollection = self.collections[join.childCollectionIdentity.toLowerCase()];
...
Operations.prototype.createParentOperation = function createParentOperation(connections) {
  ...
  if (nativeJoin) {
    ...
    _.each(connection.joins, function(join) {
      if (_.indexOf(connection.collections, join.childCollectionIdentity) > -1) {
        return;
      }
...
Operations.prototype.getConnections = function getConnections() {
  ...
  _.each(this.queryObj.joins, function(join) {
    ...
    function getConnection(collName) {
      var collection = self.collections[collName.toLowerCase()];
...
Operations.prototype.runOperation = function runOperation(operation, cb) {
  ...
  if (!_.has(this.collections, collectionName.toLowerCase())) {
...
Operations.prototype.buildChildOpts = function buildChildOpts(parentResults) {
  ...
  _.each(this.operations, function(item) {
    ...
    var using = self.collections[item.collectionName.toLowerCase()];
    ...
    if (item.queryObj.join.criteria) {
      var userCriteria = _.merge({}, item.queryObj.join.criteria);
      ...
      if (_.has(userCriteria, 'where')) {
        if (_.isUndefined(userCriteria.where)) {
          ...
          if (_.isArray(userCriteria.where)) {
            var pk = self.collections[item.queryObj.join.childCollectionIdentity.toLowerCase()].primaryKey;
            var obj = {};
    ...
    localOpts.push({
      ...
      queryObj: {
        method: item.queryObj.method,
        using: self.collections[item.queryObj.child.collection.toLowerCase()].tableName
      },
      parent: idx,
...
Operations.prototype.collectChildResults = function collectChildResults(opts, cb) {
  ...
  async.eachSeries(opts, function(opt, next) {
    self.runChildOperations(intermediateResults, opt, function(err, values) {
      ...
      self.cache[opt.collectionName.toLowerCase()] = self.cache[opt.collectionName.toLowerCase()] || [];
      self.cache[opt.collectionName.toLowerCase()] = self.cache[opt.collectionName.toLowerCase()].concat(values);

      // Ensure the values are unique
      var pk = self.findCollectionPK(opt.collectionName);
      self.cache[opt.collectionName.toLowerCase()] = _.uniq(self.cache[opt.collectionName.toLowerCase()], pk);
...
Operations.prototype.runChildOperations = function runChildOperations(intermediateResults, opt, cb) {
  ...
  var cacheCopy = _.merge({}, self.cache[opt.join.parentCollectionIdentity.toLowerCase()]);
  self.cache[opt.join.parentCollectionIdentity.toLowerCase()] = [];
    ...
    _.each(values, function(val) {
      _.each(cacheCopy, function(copy) {
        if (copy[opt.join.parentKey] === val[opt.join.childKey]) {
          self.cache[opt.join.parentCollectionIdentity.toLowerCase()].push(copy);
        }
      });
    });

    // Ensure the values are unique
    var pk = self.findCollectionPK(opt.join.parentCollectionIdentity);
    self.cache[opt.join.parentCollectionIdentity.toLowerCase()] = _.uniq(self.cache[opt.join.parentCollectionIdentity.toLowerCase()], pk);
