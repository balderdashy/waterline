...
Operations.prototype.runOperation = function runOperation(operation, cb) {
  ...
  queryObj.criteria.joins = queryObj.joins || [];
...
Operations.prototype.buildChildOpts = function buildChildOpts(parentResults) {
  ...
  _.each(this.operations, function(item) {
    ...
    if (item.join.criteria) {
      ...
      // TODO: change this  (but be sure that it's supposed to be using attr names here!  normalizeCriteria doesn't know how to use columnNames atm.  I think it's probably attr names we want here, but just making sure.)
      console.warn('WARNING: this code is about to attempt to call normalizeCriteria, but it probably wont work, since now it expects more args to be provided.');
