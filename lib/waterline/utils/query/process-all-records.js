  ...
  _.each(records, function(record) {
    ...
    _.each(_.keys(record), function(key){
      ...
      if (WLModel.hasSchema) {
        if (!WLModel.attributes[key]) {
          throw new Error('Consistency violation: Property in record (`'+key+'`) does not correspond with a recognized attribute (but since this model was defined with `schema: true`, any unrecognized attributes should have already been dealt with and stripped before reaching this point in the code-- e.g. when transforming column names back to attribute names.)');
        }
      }//>-•
    ...
    _.each(WLModel.attributes, function(attrDef, attrName){
      ...
      // If this attribute is a singular association...
        ...
        // • a dictionary, or
        // • `null`
        if (!_.isObject(record[attrName]) && !_.isNull(record[attrName])){
          throw new Error('Consistency violation: An association in a result record has an unexpected data type.  Since `'+attrName+'` is a singular (association), it should either be `null` (if not populated or orphaned) or a dictionary (if successfully populated).  But in fact for this record, it was neither.  Instead, got: '+util.inspect(record[attrName], {depth:5})+'');
        }

        // Handle `null` / `''` when association is required.
        // TODO
        // ```~~~
