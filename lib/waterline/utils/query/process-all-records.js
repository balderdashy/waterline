    ...
    _.each(WLModel.attributes, function(attrDef, attrName){
      ...
      // Otherwise, this is a misc. attribute.
        ...
        switch (attrDef.type) {
          ...
          case 'string':
            try {
              rttc.validateStrict('string', record[attrName]);
            } catch (e) {
              switch (e.code) {
                case 'E_INVALID':
                  console.warn('\n'+
          ...
          case 'number':
            try {
              rttc.validateStrict('number', record[attrName]);
            } catch (e) {
              switch (e.code) {
                case 'E_INVALID':
                  console.warn('\n'+
          ...
          case 'boolean':
            try {
              rttc.validateStrict('boolean', record[attrName]);
            } catch (e) {
              switch (e.code) {
                case 'E_INVALID':
                  console.warn('\n'+
          ...
          case 'json':
            // Technically we could verify that the value is not circular,
            // but we explicitly skip that check here for performance reasons.
            break;
          ...
          default: throw new Error('Consistency violation: Attribute ('+attrName+') has invalid `type`.  (This should have been verified already in wl-schema.)');
        }//</switch>
