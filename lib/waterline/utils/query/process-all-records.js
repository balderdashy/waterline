 ...
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 */
module.exports = function processAllRecords(records, s2q, modelIdentity, orm) {
  ...
  if (!_.isObject(s2q)) {
    throw new Error('Consistency violation: Expected `s2q` to be a stage 2 query.  But instead, got: '+util.inspect(s2q,{depth:5})+'');
  }
  ...
  if (s2q.criteria) {
    if (!_.isObject(s2q.criteria) || !_.isArray(s2q.criteria.select)) {
      throw new Error('Consistency violation: processAllRecords() is supposed to be called using a stage 2 query.  But somehow, the provided query ended up with an invalid query key (`criteria`).  Here is the entire query for reference:\n```\n'+util.inspect(s2q, {depth:null})+'\n```\n');
    }
  }
  ...
  _.each(records, function(record) {
    ...
    _.each(WLModel.attributes, function(attrDef, attrName){
      ...
      // If this attribute is a plural association...
        ...
        // FUTURE: expand check using `s2q.populates[attrName]`
      ...
      // Otherwise, this is a misc. attribute.
        ...
        if(_.isUndefined(record[attrName])){
          ...
          var isValueExpectedForThisAttribute = (
            ...
            // If there is no `criteria` query key, then it means this is a `create` or
            // something like that.  In this case, we expect values for ALL known miscellaneous
            ...
            // attributes like this one.
            s2q.criteria.select[0] === '*' ||

            // Otherwise, if the `select` clause explicitly contains the name of the attribute
            // in question, then we know it's valid
            _.contains(s2q.criteria.select, attrName)
          ...
          if (isValueExpectedForThisAttribute) {
            ...
            console.warn('\n'+
              ...
              // ^^See next TODO -- this last line might need to be removed
            );

            // Then coerce it to the base value for the type.
            // (TODO: Revisit.  Maybe change this to do nothing?  Just for consistency.)
