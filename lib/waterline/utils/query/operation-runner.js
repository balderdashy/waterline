 ...
 * runOperations()
 *
 * (aka "operation runner")
 *
 * Run a set of generated operations and then perform an in-memory join if
 * needed. Afterwards, the normalized result set is turned into records.
 *
 * > Used in both `.find()` and `.findOne()` queries.
 * - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
 *
 * @param  {Ref}   operations
 *         A special "Operations" instance.
 *
...
module.exports = function runOperations(operations, stageThreeQuery, WLModel, cb) {

  assert(operations, 'An "Operations" instance should be provided as the first argument.');
  ...
  //  ╠╦╝║ ║║║║  │ │├─┘├┤ ├┬┘├─┤ │ ││ ││││└─┐
  //  ╩╚═╚═╝╝╚╝  └─┘┴  └─┘┴└─┴ ┴ ┴ ┴└─┘┘└┘└─┘
  operations.run(function _afterRunningOperations(err, values) {
    if (err) {
      return cb(err);
    }
