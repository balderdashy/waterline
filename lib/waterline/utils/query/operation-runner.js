...
module.exports = function operationRunner(operations, stageThreeQuery, collection, cb) {
  ...
  operations.run(function operarationRunCb(err, values) {
    ...
    if (!stageThreeQuery.joins || !stageThreeQuery.joins.length) {
      values = values.cache[collection.identity.toLowerCase()];
      return returnResults(values);
    }

    // If the values are already combined, return the results
    if (values.combined) {
      return returnResults(values.cache[collection.identity.toLowerCase()]);
    }
    ...
    function returnResults(results) {
      ...
      var data;
      try {
        data = Joins(joins, unserializedModels, collection.identity, collection.schema, collection.waterline.collections);
      } catch (e) {
        return cb(e);
      }
