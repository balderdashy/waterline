...
module.exports = function forgeStageTwoQuery(query, orm) {
  ...
  if (_.contains(queryKeys, 'numericAttrName')) {
    ...
    var numericAttrDef;
    try {
      numericAttrDef = getAttribute(query.numericAttrName, WLModel.identity, orm);
    } catch (e) {
      switch (e.code) {
        case 'E_ATTR_NOT_REGISTERED':
          throw buildUsageError('E_INVALID_NUMERIC_ATTR_NAME',
  ...
  if (_.contains(queryKeys, 'targetRecordIds')) {
    ...
    try {
      var pkAttrDef = getAttribute(WLModel.primaryKey, WLModel.identity, orm);
  ...
  if (_.contains(queryKeys, 'collectionAttrName')) {
    ...
    var associationDef;
    try {
      associationDef = getAttribute(query.collectionAttrName, WLModel.identity, orm);
    } catch (e) {
      switch (e.code) {
        case 'E_ATTR_NOT_REGISTERED':
          throw buildUsageError('E_INVALID_COLLECTION_ATTR_NAME',
  ...
  if (_.contains(queryKeys, 'associatedIds')) {
    ...
    var associatedPkType = (function(){
      var AssociatedModel = (function(){
        var _associationDef = getAttribute(query.collectionAttrName, WLModel.identity, orm);
        return getModel(_associationDef.collection, orm);
      })();
      var _associatedPkDef = getAttribute(AssociatedModel.primaryKey, AssociatedModel.identity, orm);
...
/*```
q = { using: 'user', method: 'find', criteria: {where: {id: '3d'}, limit: 3} };  require('./lib/waterline/utils/query/forge-stage-two-query')(q, { collections: { user: { attributes: { id: { type: 'string' } }, primaryKey: 'id', hasSchema: false } } }); console.log(util.inspect(q,{depth:null}));
```*/
