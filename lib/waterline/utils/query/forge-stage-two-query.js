...
module.exports = function forgeStageTwoQuery(query, orm) {
  ...
  if (query.method === 'destroy' && !_.isUndefined(WLModel.cascadeOnDestroy)) {
    if (!(_.isBoolean(WLModel.cascadeOnDestroy))) { throw new Error('Consistency violation: if specified, expecting `cascadeOnDestroy` model setting to be `true` or `false`.  But instead, got: '+util.inspect(WLModel.cascadeOnDestroy, {depth:5})+''); }
  ...
  if (query.method === 'update' && !_.isUndefined(WLModel.fetchRecordsOnUpdate)) {
    if (!(_.isBoolean(WLModel.fetchRecordsOnUpdate))) { throw new Error('Consistency violation: if specified, expecting `fetchRecordsOnUpdate` model setting to be `true` or `false`.  But instead, got: '+util.inspect(WLModel.fetchRecordsOnUpdate, {depth:5})+''); }
  ...
  if (query.method === 'destroy' && !_.isUndefined(WLModel.fetchRecordsOnDestroy)) {
    if (!(_.isBoolean(WLModel.fetchRecordsOnDestroy))) { throw new Error('Consistency violation: if specified, expecting `fetchRecordsOnDestroy` model setting to be `true` or `false`.  But instead, got: '+util.inspect(WLModel.fetchRecordsOnDestroy, {depth:5})+''); }
  ...
  if (query.method === 'create' && !_.isUndefined(WLModel.fetchRecordsOnCreate)) {
    if (!(_.isBoolean(WLModel.fetchRecordsOnCreate))) { throw new Error('Consistency violation: if specified, expecting `fetchRecordsOnCreate` model setting to be `true` or `false`.  But instead, got: '+util.inspect(WLModel.fetchRecordsOnCreate, {depth:5})+''); }
  ...
  if (query.method === 'createEach' && !_.isUndefined(WLModel.fetchRecordsOnCreateEach)) {
    if (!(_.isBoolean(WLModel.fetchRecordsOnCreateEach))) { throw new Error('Consistency violation: if specified, expecting `fetchRecordsOnCreateEach` model setting to be `true` or `false`.  But instead, got: '+util.inspect(WLModel.fetchRecordsOnCreateEach, {depth:5})+''); }
    ...
    _.each(_.keys(query.valuesToSet), function (attrNameToSet){
      ...
      try {
        ...
        switch (e.code) {
          ...
          // ```
          // if (!(_.isArray(e.ruleViolations) && e.ruleViolations.length > 0)) { throw new Error('Consistency violation: this error should ALWAYS have a non-empty array as its `ruleViolations` property.  But instead, its `ruleViolations` property is: '+util.inspect(e.ruleViolations, {depth: 5})+'\nAlso, for completeness/context, here is the error\'s complete stack: '+e.stack); }
          // throw flaverr({
