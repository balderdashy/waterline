...
module.exports = function forgeStageTwoQuery(query, orm) {
  ...
  if (!_.isUndefined(WLModel.dontUseObjectIds)) {
    if (!_.isBoolean(WLModel.dontUseObjectIds)) {
      throw new Error('Consistency violation: If specified, expecting `dontUseObjectIds` model setting to be `true` or `false`.  But instead, got: '+util.inspect(WLModel.dontUseObjectIds, {depth:5})+'');
    }
    // Only bother setting the `dontUseObjectIds` meta key if the model setting is `true`.
    // (because otherwise it's `false`, which is the default anyway)
    if (WLModel.dontUseObjectIds) {
      query.meta = query.meta || {};
      query.meta.dontUseObjectIds = WLModel.dontUseObjectIds;
    }
