  ...
  if (query.meta !== undefined) {
    ...
    if (query.meta.fetch !== undefined) {
      ...
      if (_.contains(['findOrCreate', 'updateOne', 'destroyOne', 'archiveOne'], query.method)) {
        console.warn(
          'warn: `fetch` should not be provided when calling .'+query.method+'().  '+
          'This method *always* behaves as if `fetch` was set to `true`.  '+
          'If successful, it always returns the affected record.'
        );
      }//ﬁ
