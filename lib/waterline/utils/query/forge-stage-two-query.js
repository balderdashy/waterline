 ...
 *                   - E_INVALID_NEW_RECORD              (for misc issues--note that most errors use E_VALIDATION though)
 *                   - E_INVALID_NEW_RECORDS             (for misc issues--note that most errors use E_VALIDATION though)
 *                   - E_INVALID_VALUES_TO_SET           (for misc issues--note that most errors use E_VALIDATION though)
...
module.exports = function forgeStageTwoQuery(query, orm) {
  ...
  if (query.method === 'create' && !_.isUndefined(WLModel.fetchRecordsOnCreate)) {
    assert(_.isBoolean(WLModel.fetchRecordsOnCreate), 'If specified, expecting `fetchRecordsOnCreate` model setting to be `true` or `false`.  But instead, got: '+util.inspect(WLModel.fetchRecordsOnCreate, {depth:5})+'');
  ...
  if (query.method === 'createEach' && !_.isUndefined(WLModel.fetchRecordsOnCreateEach)) {
    assert(_.isBoolean(WLModel.fetchRecordsOnCreateEach), 'If specified, expecting `fetchRecordsOnCreateEach` model setting to be `true` or `false`.  But instead, got: '+util.inspect(WLModel.fetchRecordsOnCreateEach, {depth:5})+'');
  ...
  if (_.contains(queryKeys, 'newRecord')) {
    ...
    try {
      ...
      switch (e.code){
        ...
        case 'E_REQUIRED'://<<TODO: replace w/ E_VALIDATION
    ...
    _.each(_.keys(query.valuesToSet), function (attrNameToSet){
      ...
      try {
        ...
        switch (e.code) {
          ...
          case 'E_INVALID':
            // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
            // TODO: bring this back instead of the `throw`:
            ...
            // vdnErrorsByAttribute[attrNameToSet] = e;
            // return;
            // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
            // (^^just commented it out temporarily)

            // temporary:
            throw new Error('The wrong type of data was specified for `'+attrNameToSet+'`.  '+e.message);
