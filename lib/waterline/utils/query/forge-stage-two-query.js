...
module.exports = function forgeStageTwoQuery(query, orm) {
  ...
  if (_.contains(queryKeys, 'valuesToSet')) {
    ...
    // Along the way, keep track of failed type safety checks as well as
    // high-level validation rule violations, so we can aggregate them
    // into a single E_VALIDATION error below.
    var hasAnyVdnErrors;
    ...
    _.each(_.keys(query.valuesToSet), function (attrNameToSet){
      ...
      try {
        ...
        switch (e.code) {
          ...
          case 'E_INVALID':
            // TODO: aggregate
            // vdnErrorsByAttrName[supposedAttrName] = ...TODO...
    ...
    if (hasAnyVdnErrors) {
      var numInvalidAttrs = _.keys(vdnErrorsByAttrName).length;
