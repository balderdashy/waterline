...
module.exports = function forgeStageTwoQuery(query, orm) {
  ...
  // Set the `mongoCollectionsNotUsingObjectIds` meta key of the query to an array of identities of
  // collections that have declared that their primary keys are NOT ObjectIDs.
  //
  // Note that if no collections have this flag set, the meta key won't be set at all.  This avoids
  // the weirdness of seeing this key pop up in a query for a non-mongo adapter.
  query.meta = query.meta || {};
  (function() {
    var mongoCollectionsNotUsingObjectIds = _.reduce(orm.collections, function(memo, collection) {
      if (collection.dontUseObjectIds === true) { memo.push(collection.identity); }
      return memo;
    }, []);
    if (mongoCollectionsNotUsingObjectIds.length > 0) {
      query.meta.mongoCollectionsNotUsingObjectIds = mongoCollectionsNotUsingObjectIds;
    }
  })();
  ...
  if (_.contains(queryKeys, 'criteria')) {
    ...
    if (_.isObject(query.criteria) && (!_.isUndefined(query.criteria.limit) || !_.isUndefined(query.criteria.skip) || !_.isUndefined(query.criteria.sort))) {
