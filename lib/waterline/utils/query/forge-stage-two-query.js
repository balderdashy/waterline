...
module.exports = function forgeStageTwoQuery(query, orm) {
  ...
  if (query.method === 'destroy' && !_.isUndefined(WLModel.cascadeOnDestroy)) {
    //¶_¶ assert(_.isBoolean(WLModel.cascadeOnDestroy), 'If specified, expecting `cascadeOnDestroy` model setting to be `true` or `false`.  But instead, got: '+util.inspect(WLModel.cascadeOnDestroy, {depth:5})+'');
  ...
  if (query.method === 'update' && !_.isUndefined(WLModel.fetchRecordsOnUpdate)) {
    //¶_¶ assert(_.isBoolean(WLModel.fetchRecordsOnUpdate), 'If specified, expecting `fetchRecordsOnUpdate` model setting to be `true` or `false`.  But instead, got: '+util.inspect(WLModel.fetchRecordsOnUpdate, {depth:5})+'');
  ...
  if (query.method === 'destroy' && !_.isUndefined(WLModel.fetchRecordsOnDestroy)) {
    //¶_¶ assert(_.isBoolean(WLModel.fetchRecordsOnDestroy), 'If specified, expecting `fetchRecordsOnDestroy` model setting to be `true` or `false`.  But instead, got: '+util.inspect(WLModel.fetchRecordsOnDestroy, {depth:5})+'');
  ...
  if (query.method === 'create' && !_.isUndefined(WLModel.fetchRecordsOnCreate)) {
    //¶_¶ assert(_.isBoolean(WLModel.fetchRecordsOnCreate), 'If specified, expecting `fetchRecordsOnCreate` model setting to be `true` or `false`.  But instead, got: '+util.inspect(WLModel.fetchRecordsOnCreate, {depth:5})+'');
  ...
  if (query.method === 'createEach' && !_.isUndefined(WLModel.fetchRecordsOnCreateEach)) {
    //¶_¶ assert(_.isBoolean(WLModel.fetchRecordsOnCreateEach), 'If specified, expecting `fetchRecordsOnCreateEach` model setting to be `true` or `false`.  But instead, got: '+util.inspect(WLModel.fetchRecordsOnCreateEach, {depth:5})+'');
    ...
    _.each(_.keys(query.valuesToSet), function (attrNameToSet){
      ...
      try {
        ...
        switch (e.code) {
          ...
          // ```
          // //¶_¶ assert(_.isArray(e.ruleViolations) && e.ruleViolations.length > 0, 'This error should ALWAYS have a non-empty array as its `ruleViolations` property.  But instead, its `ruleViolations` property is: '+util.inspect(e.ruleViolations, {depth: 5})+'\nAlso, for completeness/context, here is the error\'s complete stack: '+e.stack);
          // throw flaverr({
