...
var getModel = require('./private/get-model');
var getAttribute = require('./private/get-attribute');
    ...
    _.each(_.keys(query.populates), function (populateAttrName) {
      ...
      // validate and fully-normalize the provided subcriteria.
        ...
        if (process.env.NODE_ENV === 'production') {
          ...
          // Determine if we are populating an xD/A association.
          var isPopulatingXDA;// TODO
          ...
          if (isPopulatingXDA) {
            ...
            var isDangerous = (
              ...
              _.isEqual(subcriteria.sort, [])
            );
            if (isDangerous) {
              throw buildUsageError('E_INVALID_POPULATES',
                'Could not use the specified subcriteria for populating `'+populateAttrName+'`: '+
                'Since this is an xD/A association (i.e. it spans multiple datastores, or uses an '+
                'adapter that does not support native joins), it is not a good idea to populate it '+
                'along with a subcriteria that uses `limit`, `skip`, and/or `sort`-- at least not in '+
                'a production environment.  To overcome this, either (A) remove or change this subcriteria, '+
                'or (B) configure all involved models to use the same datastore, and/or '+
