...
module.exports = function forgeStageTwoQuery(query, orm) {
    ...
    try {
      ...
      switch (e.code) {
        ...
        case 'E_WOULD_RESULT_IN_NOTHING':
          throw flaverr('E_NOOP', 'The provided criteria would not match any records.  '+e.message);
  ...
  if (_.contains(queryKeys, 'targetRecordIds')) {
    ...
    if (query.targetRecordIds.length === 0) {
      throw flaverr('E_NOOP', 'No target record ids were provided.');
    }//-•
  ...
  if (_.contains(queryKeys, 'associatedIds')) {
    ...
    var isRelevantMethod = (query.method === 'removeFromCollection' || query.method === 'addToCollection');
    if (query.associatedIds.length === 0 && isRelevantMethod) {
      throw flaverr('E_NOOP', 'No associated ids were provided.');
    }//-•
