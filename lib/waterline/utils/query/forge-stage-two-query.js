...
module.exports = function forgeStageTwoQuery(query, orm) {
  ...
  //  ││││ │ │ ├─┤ │ ├┤   ├─┤├┬┘│ ┬└─┐ ┌┘
  //  ┴ ┴└─┘ ┴ ┴ ┴ ┴ └─┘  ┴ ┴┴└─└─┘└─┘ o
  if (!_.isUndefined(WLModel.mutateArgs)) {
    if (!_.isBoolean(WLModel.mutateArgs)) {
      throw new Error('Consistency violation: If specified, expecting `mutateArgs` model setting to be `true` or `false`.  But instead, got: '+util.inspect(WLModel.mutateArgs, {depth:5})+'');
    }

    // Only bother setting the `mutateArgs` meta key if the corresponding
    // model setting is `true`  (because otherwise it's `false`, which is the default anyway)
    if (WLModel.mutateArgs) {
      query.meta = query.meta || {};
      query.meta.mutateArgs = WLModel.mutateArgs;
    }
