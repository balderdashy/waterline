...
module.exports = function forgeStageTwoQuery(query, orm) {
  ...
  if (!_.isUndefined(query.meta)) {
    ...
    if (query.method === 'findOrCreate' && !_.isUndefined(query.meta.fetch)) {
      throw buildUsageError(
        'E_INVALID_META',
  ...
  // //   ││├┤ ├┤ ├─┘  │  │  │ ││││├┤    │├┤   ├─┤├─┘├─┘├┬┘│ │├─┘├┬┘│├─┤ │ ├┤
  // //  ─┴┘└─┘└─┘┴    └─┘┴─┘└─┘┘└┘└─┘┘  ┴└    ┴ ┴┴  ┴  ┴└─└─┘┴  ┴└─┴┴ ┴ ┴ └─┘
  // // Last but not least, we'll deep-clone the entire query as-is (unless instructed not to do so).
  // // This is to avoid munging data that could mess with any userland code that reuses criteria--
  // // both here in FS2Q **AND** later on in FS3Q etc.
  // //
  ...
  // if (!query.meta || query.meta.dontCloneArguments !== true) {
  //   _.each(_.keys(query), function(key){
  //     // Don't deep-clone functions.
  ...
  //     if (!_.isFunction(query[key])) {
  //       query[key] = _.cloneDeep(query[key]);
  //     }
  //     // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  //     // TODO: Make this smarter (b/c of `type:'ref'` attr vals in valuesToSet,
  //     // newRecord, newRecords, criteria, etc.)
  //     // This will probably mean distributing this deep clone behavior out to the
  //     // various places it's liable to come up.  In reality, this will be more
  //     // performant anyway, since we won't be unnecessarily cloning things like
  //     // big JSON values, etc.
  //     //
  ...
  if (_.contains(queryKeys, 'valuesToSet')) {
    ...
    _.each(_.keys(query.valuesToSet), function (attrNameToSet){
      ...
      try {
        query.valuesToSet[attrNameToSet] = normalizeValueToSet(query.valuesToSet[attrNameToSet], attrNameToSet, query.using, orm, true);
      } catch (e) {
        switch (e.code) {
    ...
    if (query.targetRecordIds.length > 1 && isRelevantMethod && isTryingToSetOneOrMoreAssociatedIds) {
      ...
      if (isAssociationExclusive) {
        throw buildUsageError(
          ...
          'make this association shared (use `collection` + `via` instead of `model` on the other side).  In other words, imagine '+
          'trying to run a query like `Car.replaceCollection([1,2], \'wheels\', [99, 98])`.  If a wheel always belongs to one '+
          'particular car via `wheels`, then this query would be impossible.  To make it possible, you\'d have to make each wheel '+
          'capable of being associated with more than one car.',
          query.using
        );
      }//-•
