...
var normalizeNewRecord = require('./normalize-new-record');
...
module.exports = function forgeStageTwoQuery(query, orm) {
  ...
  if (_.contains(queryKeys, 'valuesToSet')) {
    ...
    _.each(_.keys(query.valuesToSet), function (attrNameToSet){
      ...
      try {
        query.valuesToSet[attrNameToSet] = normalizeValueToSet(query.valuesToSet[attrNameToSet], attrNameToSet, modelIdentity, orm, ensureTypeSafety);
      } catch (e) {
        switch (e.code) {
