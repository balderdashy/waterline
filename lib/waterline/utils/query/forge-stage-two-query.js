 ...
 *                   - E_NOOP                    (relevant for various diferent methods, like find/count/addToCollection/etc. -- UNLIKE THE OTHERS, THIS IS NOT A USAGE ERROR!)
 *
 *
...
module.exports = function forgeStageTwoQuery(query, orm) {
    ...
    try {
      ...
      switch (e.code) {
        ...
        case 'E_WOULD_RESULT_IN_NOTHING':
          throw flaverr('E_NOOP', new Error('The provided criteria would not match any records.  '+e.message));
  ...
  if (_.contains(queryKeys, 'newRecord')) {
    ...
    try {
      query.newRecord = normalizeNewRecord(query.newRecord, query.using, orm, theMomentBeforeFS2Q, ensureTypeSafety);
    } catch (e) {
      switch (e.code){
  ...
  if (_.contains(queryKeys, 'newRecords')) {
    ...
    query.newRecords = _.map(query.newRecords, function (newRecord){
      ...
      try {
        return normalizeNewRecord(newRecord, query.using, orm, theMomentBeforeFS2Q, ensureTypeSafety);
      } catch (e) {
        switch (e.code){
          ...
          case 'E_HIGHLY_IRREGULAR':
            throw buildUsageError('E_INVALID_NEW_RECORDS',
              'Could not parse one of the provided new records: '+e.message
            );
  ...
  if (_.contains(queryKeys, 'valuesToSet')) {
    ...
    _.each(_.keys(query.valuesToSet), function (attrNameToSet){
      ...
      // > Note that we explicitly DO NOT allow values to be provided for collection attributes (plural associations).
      try {
    ...
    // If not, then set the value as the current timestamp.
    // TODO
  ...
  if (_.contains(queryKeys, 'targetRecordIds')) {
    ...
    if (query.targetRecordIds.length === 0) {
      throw flaverr('E_NOOP', new Error('No target record ids were provided.'));
    }//-•
  ...
  if (_.contains(queryKeys, 'associatedIds')) {
    ...
    if (query.associatedIds.length === 0 && isQueryMeaningfulWithNoAssociatedIds) {
      throw flaverr('E_NOOP', new Error('No associated ids were provided.'));
    }//-•
...
/*```
q = { using: 'user', method: 'find', criteria: {where: {id: '3d', foo: { startsWith: 'b', contains: 'bar'} }, limit: 3} };  require('./lib/waterline/utils/query/forge-stage-two-query')(q, { collections: { user: { attributes: { id: { type: 'string', required: true, unique: true } }, primaryKey: 'id', hasSchema: false } } }); console.log(util.inspect(q,{depth:null}));
```*/
...
/**
 * to demonstrate filter normalization, and that it checks pk values...
 */

/*```
q = { using: 'user', method: 'find', criteria: {where: {id: '3.5'}, limit: 3} };  require('./lib/waterline/utils/query/forge-stage-two-query')(q, { collections: { user: { attributes: { id: { type: 'number', required: true, unique: true } }, primaryKey: 'id', hasSchema: false } } }); console.log(util.inspect(q,{depth:null}));
```*/
...
/*```
q = { using: 'user', method: 'find', criteria: {where: {id: { '>': '5' } }, limit: 3} };  require('./lib/waterline/utils/query/forge-stage-two-query')(q, { collections: { user: { attributes: { id: { type: 'number', required: true, unique: true } }, primaryKey: 'id', hasSchema: false } } }); console.log(util.inspect(q,{depth:null}));
```*/
