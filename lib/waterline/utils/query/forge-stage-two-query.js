...
module.exports = function forgeStageTwoQuery(query, orm) {
  ...
  if (_.contains(queryKeys, 'valuesToSet')) {
    ...
    _.each(WLModel.attributes, function (attrDef, attrName) {
      ...
      // If the value is explicitly set, we can skip it.
      if (!_.isUndefined(query.valuesToSet[attrName])) {

        // If this attribute is defined as a timestamp, log a warning about setting it manually.
        if (attrDef.autoUpdatedAt || attrDef.autoCreatedAt) {
          console.warn('Manually updating a value for `' + attrName + '` in `' + WLModel.globalId + '` record, even though it has `' + (attrDef.autoCreatedAt ? 'autoCreatedAt' : 'autoUpdatedAt') + ': true`...');
        }

        return;
      }

      // If the value isn't an autoUpdatedAt timestamp, we can skip it.
