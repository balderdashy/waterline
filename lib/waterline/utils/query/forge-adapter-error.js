...
module.exports = function forgeAdapterError(err, omen, modelIdentity, orm){
  ...
  if (!_.isError(err)) {
    return flaverr({
      name: 'AdapterError',
      message:
      'Malformed error from adapter: Should always be an Error instance, but instead, got:\n'+
      '```\n'+
  ...
  if (!_.isObject(err.footprint) || !_.isString(err.footprint.identity) || err.footprint.identity === '') {
    return flaverr({
      name: 'AdapterError',
      message:
      'Malformed error from adapter: If Error has a `footprint`, it should be a dictionary with a valid `identity`.  '+
      'But instead, the error\'s `footprint` is:\n'+
      '```\n'+
    ...
    case 'notUnique': return (function(){
      ...
      if (!_.isArray(err.footprint.keys)) {
        return flaverr({
          name: 'AdapterError',
          message:
          'Malformed error from adapter: Since `footprint.identity` is "notUnique", this error\'s '+
          'footprint should have an array of `keys`!  But instead, the error\'s `footprint.keys` is:\n'+
          '```\n'+
      ...
      //  ├┤ │ ││ │ │ ├─┘├┬┘││││ │   │└─┐  ├─┘├┬┘│ │├─┘├┤ ├┬┘│ └┬┘  ├┤ │ │├┬┘│││├─┤ │  │ ├┤  ││
      //  └  └─┘└─┘ ┴ ┴  ┴└─┴┘└┘ ┴   ┴└─┘  ┴  ┴└─└─┘┴  └─┘┴└─┴─┘┴   └  └─┘┴└─┴ ┴┴ ┴ ┴  ┴ └─┘─┴┘
      // Standardize the error, mapping the `footprint.keys` (~=column names) back to
      // attribute names, and attaching a `toJSON()` function.
      ...
      // Build the new uniqueness error.
      return flaverr({
