...
module.exports = function forgeAdapterError(err, omen, adapterMethodName, modelIdentity, orm){
  ...
  try {
    ...
    // console.log('Trying to handle an error from the adapter, and so about to look up model with identity `'+modelIdentity+'`....');
    // console.log('Current list of all registered models\' identities: ',_.keys(orm.collections));
    // console.log('And here\'s each of their self-declared `identity` props: ',_.pluck(_.values(orm.collections), 'identity'));
    ...
    return flaverr({
      message: 'Consistency violation: Waterline encountered an unexpected internal error.\n'+
      'Details:\n'+
      '```\n'+
      e.stack+'\n'+
      '```'
