...
module.exports = function normalizeConstraint (constraint, attrName, modelIdentity, orm){
  assert(!_.isUndefined(constraint), 'The internal normalizeConstraint() utility must always be called with a first argument (the constraint to normalize).  But instead, got: '+util.inspect(constraint, {depth:5})+'');
  assert(_.isString(attrName), 'The internal normalizeConstraint() utility must always be called with a valid second argument (a string).  But instead, got: '+util.inspect(attrName, {depth:5})+'');
  assert(_.isString(modelIdentity), 'The internal normalizeConstraint() utility must always be called with a valid third argument (a string).  But instead, got: '+util.inspect(modelIdentity, {depth:5})+'');
  ...
  if (_.isObject(constraint) && !_.isFunction(constraint) && !_.isArray(constraint)) {
    ...
    assert(numKeys === 1, 'If provided as a dictionary, the constraint passed in to the internal normalizeConstraint() utility must always have exactly one key.  (Should have been normalized already.)  But instead, got: '+util.inspect(constraint, {depth:5})+'');
