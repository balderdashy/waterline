...
module.exports = function normalizeFilter (filter, attrName, modelIdentity, orm){
  ...
  // (In the current version of Waterline, filtering by plural associations is not supported,
  ...
  if (_.isObject(filter) && !_.isFunction(filter)) {
    ...
    // ------------------------------------------------------------------------------------------
    // TODO: Fracture multi-key complex filters back over in normalizeWhereClause()
    // (this is critical for backwards-compatible support for "range" filters)
    //
    // > NOTE: we might need to pull this up to normalizeWhereClause() to make it actually possible
    //
    // This is instead of doing stuff like this:
    // if (!_.isUndefined(filter.nin)) {
    //   throw flaverr('E_FILTER_NOT_USABLE', new Error(
    //     'Cannot filter by `'+attrName+'` using `!` and `nin` modifiers at the same time-- '+
    //     'at least not in the same filter.  Instead, please use `and` to break this up into '+
    //     'two separate filters.'
    //   ));
    // }//-•
    // ------------------------------------------------------------------------------------------
    // ^^ that fracturing needs to go in a separate file -- it'll completely go away from here.
    ...
    //  ├─┤├─┤│││ │││  ├┤   ├┤ ┌┴┬┘ │ ├┬┘├─┤│││├┤ │ ││ │└─┐  ││││ │ │││├┤ │├┤ ├┬┘└─┐
    //  ┴ ┴┴ ┴┘└┘─┴┘┴─┘└─┘  └─┘┴ └─ ┴ ┴└─┴ ┴┘└┘└─┘└─┘└─┘└─┘  ┴ ┴└─┘─┴┘┴└  ┴└─┘┴└─└─┘
    // Now, check that there's no extra, unrecognized properties in here.
    // (if there are, fail with an error)
    //
    // Ensure that there is only one key.
    // TODO
    ...
    if (!_.isUndefined(filter['!'])) {
      ...
      // Validate/normalize this modifier.
      // TODO
    ...
    //  ║║║║
    //  ╩╝╚╝
    else if (!_.isUndefined(filter.in)) {
    ...
    //  ║║║║║║║
    //  ╝╚╝╩╝╚╝
    else if (!_.isUndefined(filter.nin)) {
    ...
    //  ║  ║ ║║║║ ║ ╠═╣║║║║╚═╗
    //  ╚═╝╚═╝╝╚╝ ╩ ╩ ╩╩╝╚╝╚═╝
    else if (!_.isUndefined(filter.contains)) {
    ...
    //  ╚═╗ ║ ╠═╣╠╦╝ ║ ╚═╗  ║║║║ ║ ╠═╣
    //  ╚═╝ ╩ ╩ ╩╩╚═ ╩ ╚═╝  ╚╩╝╩ ╩ ╩ ╩
    else if (!_.isUndefined(filter.startsWith)) {
    ...
    //  ║╣ ║║║ ║║╚═╗  ║║║║ ║ ╠═╣
    //  ╚═╝╝╚╝═╩╝╚═╝  ╚╩╝╩ ╩ ╩ ╩
    else if (!_.isUndefined(filter.endsWith)) {
    ...
    //  ║  ║╠╩╗║╣
    //  ╩═╝╩╩ ╩╚═╝
    else if (!_.isUndefined(filter.like)) {
    ...
    //  └┘   └─┘┘└┘┴└─└─┘└─┘└─┘└─┘┘└┘┴└─┘└─┘─┴┘  ┴ ┴└─┘─┴┘┴└  ┴└─┘┴└─
      ...
      // A complex filter must always contain at least one recognized modifier.
      //
  ...
  //
    ...
    // Ensure that this filter is a valid eq filter, including schema-aware
    // normalization vs. the attribute def.
    //
    // > If there is no attr def, treat it like `type: 'json'`.
    // TODO
