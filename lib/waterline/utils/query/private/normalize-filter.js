...
module.exports = function normalizeFilter (filter, attrName, modelIdentity, orm){
  ...
  if (_.isObject(filter) && !_.isFunction(filter)) {
    ...
    if (modifierKind === '!=') {
      ...
      // (And ALWAYS allow `null`.)
      if (!_.isString(modifier) && !_.isNumber(modifier) && !_.isBoolean(modifier) && !_.isNull(modifier)){
        throw flaverr('E_FILTER_NOT_USABLE', new Error(
          'A `!=` ("not equal") modifier should always be a string, number, boolean, or `null`.  But instead, got: '+util.inspect(modifier, {depth:null})+''
        ));
      }//-•
  ...
  // > If there is no attr def, then check that it's a string, number, or boolean.
    ...
    // First, ensure this is a primitive.
    // (And ALWAYS allow `null`.)
    if (!_.isString(filter) && !_.isNumber(filter) && !_.isBoolean(filter) && !_.isNull(filter)){
      throw flaverr('E_FILTER_NOT_USABLE', new Error(
        'An equivalency filter should always be a string, number, boolean, or `null`.  But instead, got: '+util.inspect(filter, {depth:null})+''
      ));
    }//-•
    ...
    if (attrDef){
      ...
      // -- - - - - - - -- - - - - - - -- - - - - - - -- - - - - - - -- - - - - - - -- - - - - - - -- - - - - - -
      // TODO: maybe extrapolate this code into a utility since it's used in a few places in this file
      // -- - - - - - - -- - - - - - - -- - - - - - - -- - - - - - - -- - - - - - - -- - - - - - - -- - - - - - -

      // Ensure this modifier is valid vs. the attribute's declared data type.
      // • TODO: support singular (model) associations
      var expectedType = attrDef.type;
      try {
        filter = rttc.validate(expectedType, filter);
      } catch (e) {
        switch (e.code) {

          case 'E_INVALID':
            throw flaverr('E_FILTER_NOT_USABLE', new Error(
              'An equivalency filter should match the declared data type of the corresponding attribute: '+e.message
            ));
