 ...
 * normalizeFilter()
 *
 * Validate and normalize the provided constraint (aka filter).
 *
 * ------------------------------------------------------------------------------------------
...
module.exports = function normalizeFilter (filter, attrName, modelIdentity, orm){
  assert(!_.isUndefined(filter), 'The internal normalizeFilter() utility must always be called with a first argument (the constraint to normalize).  But instead, got: '+util.inspect(filter, {depth:5})+'');
  assert(_.isString(attrName), 'The internal normalizeFilter() utility must always be called with a valid second argument (a string).  But instead, got: '+util.inspect(attrName, {depth:5})+'');
  assert(_.isString(modelIdentity), 'The internal normalizeFilter() utility must always be called with a valid third argument (a string).  But instead, got: '+util.inspect(modelIdentity, {depth:5})+'');
  ...
  if (_.isArray(filter)) {
    ...
    var inConstraintShorthandArray = filter;
    filter = { in: inConstraintShorthandArray };
  ...
  if (_.isObject(filter) && !_.isFunction(filter) && !_.isArray(filter)) {
    ...
    assert(numKeys === 1, 'If provided as a dictionary, the constraint passed in to the internal normalizeFilter() utility must always have exactly one key.  (Should have been normalized already.)  But instead, got: '+util.inspect(filter, {depth:5})+'');
  ...
  // > If there is no attr def, then check that it's a string, number, boolean, or `null`.
    ...
    try {
      filter = normalizeComparisonValue(filter, attrName, modelIdentity, orm);
    } catch (e) {
      switch (e.code) {
