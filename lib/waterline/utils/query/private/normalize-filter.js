...
module.exports = function normalizeFilter (filter, attrName, modelIdentity, orm){
  ...
  if (_.isObject(filter) && !_.isFunction(filter)) {
    ...
    //  ╚═╝╩╚═╚═╝╩ ╩ ╩ ╚═╝╩╚═   ╩ ╩ ╩╩ ╩╝╚╝
      ...
      // Then, if it matches a known attribute, verify that the attribute does not declare
      // itself `type: 'boolean'` (it wouldn't make any sense to try that.)
      ...
      try {
        modifier = normalizeValueVsAttribute(modifier, attrName, modelIdentity, orm);
        console.log('NOMRAIZED IT', modifier, typeof modifier);
      } catch (e) {
        switch (e.code) {
    ...
    //  ╚═╝╚═╝╝╚╝ ╩ ╩ ╩╩╝╚╝╚═╝
      ...
      if (attrDef && (attrDef.type === 'number' || attrDef.type === 'boolean')){
        // TODO
      }//>-•
    ...
    //  ╩═╝╩╩ ╩╚═╝
      ...
      // > You should really NEVER use anything other than an ACTUAL string for `like`,
      // > because of the special % syntax.  So we won't try to normalize for you.
      // TODO
