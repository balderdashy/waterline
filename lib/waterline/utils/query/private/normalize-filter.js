...
var normalizeVsAttribute = require('./normalize-vs-attribute');
...
module.exports = function normalizeFilter (filter, attrName, modelIdentity, orm){
  assert(!_.isUndefined(filter), 'The internal normalizeFilter() utility must always be called with a first argument (the filter to normalize).  But instead, got: '+util.inspect(filter, {depth:5})+'');
  ...
  if (_.isObject(filter) && !_.isFunction(filter) && !_.isArray(filter)) {
    ...
    assert(numKeys === 1, 'If provided as a dictionary, the filter passed in to the internal normalizeFilter() utility must always have exactly one key.  (Should have been normalized already.)  But instead, got: '+util.inspect(filter, {depth:5})+'');
    ...
    //
    // --• At this point, we're doing doing uninformed transformations of the filter.
    ...
    if (modifierKind === '!=') {
      ...
      try {
        modifier = normalizeVsAttribute(modifier, attrName, modelIdentity, orm);
      } catch (e) {
        switch (e.code) {
    ...
    //  ╩╝╚╝
      ...
      modifier = _.map(modifier, function (item){
        ...
        try {
          item = normalizeVsAttribute(item, attrName, modelIdentity, orm);
        } catch (e) {
          switch (e.code) {
  ...
  // > If there is no attr def, then check that it's a string, number, boolean, or `null`.
    ...
    try {
      filter = normalizeVsAttribute(filter, attrName, modelIdentity, orm);
    } catch (e) {
      switch (e.code) {
