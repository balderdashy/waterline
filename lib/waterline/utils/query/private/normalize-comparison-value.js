...
module.exports = function normalizeComparisonValue (value, attrName, modelIdentity, orm){
  //¶_¶ assert(!_.isUndefined(value), 'This internal utility must always be called with a first argument (the value to normalize).  But instead, got: '+util.inspect(value, {depth:5})+'');
  //¶_¶ assert(_.isString(attrName), 'This internal utility must always be called with a valid second argument (the attribute name).  But instead, got: '+util.inspect(attrName, {depth:5})+'');
  //¶_¶ assert(_.isString(modelIdentity), 'This internal utility must always be called with a valid third argument (the model identity).  But instead, got: '+util.inspect(modelIdentity, {depth:5})+'');
  //¶_¶ assert(_.isObject(orm), 'This internal utility must always be called with a valid fourth argument (the orm instance).  But instead, got: '+util.inspect(orm, {depth:5})+'');
  ...
  if (attrDef) {
    //¶_¶ assert(!attrDef.collection, 'Should not call this internal utility on a plural association (i.e. `collection` attribute).');
  }
  ...
  // > that you might not necessarily be possible to create/update in Waterline.)
    ...
    //¶_¶ assert(_.isString(attrDef.type) && attrDef.type !== '', 'There is no way this attribute (`'+attrName+'`) should have been allowed to be registered with neither a `type`, `model`, nor `collection`!  Here is the attr def: '+util.inspect(attrDef, {depth:5})+'');
