...
module.exports = function normalizePkValueOrValues (pkValueOrPkValues, expectedPkType){
  if (!(expectedPkType === 'string' || expectedPkType === 'number')) { throw new Error('The internal normalizePkValueOrValues() utility must always be called with a valid second argument ("string" or "number").  But instead, got: '+util.inspect(expectedPkType, {depth:5})+''); }
