...
module.exports = function buildUsageError(code, details, modelIdentity) {
  ...
  if (!(_.isString(code))) { throw new Error('`code` must be provided as a string, but instead, got: '+util.inspect(code, {depth:5})+''); }
  if (!(_.isString(details))) { throw new Error('`details` must be provided as a string, but instead got: '+util.inspect(details, {depth:5})+''); }
  if (!(_.isString(modelIdentity))) { throw new Error('`modelIdentity` must be provided as a string, but instead, got: '+util.inspect(code, {depth:5})+''); }
