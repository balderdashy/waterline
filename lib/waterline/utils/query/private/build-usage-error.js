...
module.exports = function buildUsageError(code, details, modelIdentity) {
  ...
  //¶_¶ assert(_.isString(code), '`code` must be provided as a string, but instead, got: '+util.inspect(code, {depth:5})+'');
  //¶_¶ assert(_.isString(details), '`details` must be provided as a string, but instead got: '+util.inspect(details, {depth:5})+'');
  //¶_¶ assert(_.isString(modelIdentity), '`modelIdentity` must be provided as a string, but instead, got: '+util.inspect(code, {depth:5})+'');
