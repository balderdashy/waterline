...
module.exports = function buildUsageError(code, details, modelIdentity) {
  ...
  assert(_.isString(code), '`code` must be provided as a string, but instead, got: '+util.inspect(code, {depth:5})+'');
  assert(_.isString(details), '`details` must be provided as a string, but instead got: '+util.inspect(details, {depth:5})+'');
  assert(_.isString(modelIdentity), '`modelIdentity` must be provided as a string, but instead, got: '+util.inspect(code, {depth:5})+'');
