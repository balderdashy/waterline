...
module.exports = function getAttribute(attrName, modelIdentity, orm) {
  ...
  assert(_.isString(attrName) && attrName !== '', new Error('Consistency violation: `attrName` must be a non-empty string.'));
  // ================================================================================================

  // Build a disambiguating prefix + paranthetical phrase for use in the "Consistency violation"-style error messages in this file.
  ...
  assert(_.isObject(attrDef) && !_.isArray(attrDef) && !_.isFunction(attrDef), new Error('Consistency violation: The referenced attribute (`'+attrName+'`, from model `'+modelIdentity+'`) corresponds with a CORRUPTED attribute definition: '+util.inspect(attrDef, {depth:null})+''));
  ...
  if (!_.isUndefined(attrDef.model)) {
    assert(_.isString(attrDef.model) && attrDef.model !== '', new Error('Consistency violation: The referenced attribute (`'+attrName+'`, from model `'+modelIdentity+'`) has an invalid `model` property.  If specified, `model` should be a non-empty string.  But instead, got: '+util.inspect(attrDef.model, {depth:null})+''));
  ...
  // (note that we don't get too deep here-- though we could)
    ...
    assert(_.isString(attrDef.collection) && attrDef.collection !== '', new Error('Consistency violation: The referenced attribute (`'+attrName+'`, from model `'+modelIdentity+'`) has an invalid `collection` property.  If specified, `collection` should be a non-empty string.  But instead, got: '+util.inspect(attrDef.collection, {depth:null})+''));
    var otherWLModel;
    try {
    ...
    if (!_.isUndefined(attrDef.via)) {
      assert(_.isString(attrDef.via) && attrDef.via !== '', new Error('Consistency violation: The referenced attribute (`'+attrName+'`, from model `'+modelIdentity+'`) has an invalid `via` property.  If specified, `via` should be a non-empty string.  But instead, got: '+util.inspect(attrDef.via, {depth:null})+''));
      ...
      assert(otherWLModel.attributes[attrDef.via], new Error('Consistency violation: The referenced attribute (`'+attrName+'`, from model `'+modelIdentity+'`) is an association, because it declares a `collection`.  But that association also specifies a `via` ('+attrDef.via+'`) which does not correspond with a recognized attribute on the other model (`'+attrDef.collection+'`)'));
    }
  }
  // Check that this is a valid, miscellaneous attribute.
  else {
    assert(_.isString(attrDef.type) && attrDef.type !== '', new Error('Consistency violation: The referenced attribute (`'+attrName+'`, from model `'+modelIdentity+'`) has an invalid `type` property.  If specified, `type` should be a non-empty string.  But instead, got: '+util.inspect(attrDef.type, {depth:null})+''));
    assert(_.contains(KNOWN_ATTR_TYPES, attrDef.type), new Error('Consistency violation: The referenced attribute (`'+attrName+'`, from model `'+modelIdentity+'`) has an unrecognized `type`: `'+attrDef.type+'`.'));
    assert(attrDef.required === true || attrDef.required === false, new Error('Consistency violation: The referenced attribute (`'+attrName+'`, from model `'+modelIdentity+'`) has an unrecognized `required`.  By this time, it should always be true or false.  But instead, got: '+util.inspect(attrDef.required, {depth:null})+''));
    if (attrDef.required) {
      assert(_.isUndefined(attrDef.defaultsTo), new Error('Consistency violation: The referenced attribute (`'+attrName+'`, from model `'+modelIdentity+'`) has `required: true`, but it also specifies a `defaultsTo`.  This should never have been allowed-- defaultsTo should be undefined!  But instead, got: '+util.inspect(attrDef.defaultsTo, {depth:null})+''));
    }
  }
  // ================================================================================================
