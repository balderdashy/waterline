...
module.exports = function normalizeWhereClause(whereClause, modelIdentity, orm, ensureTypeSafety) {
  ...
  try {
    ...
    (function _recursiveStep(branch, recursionDepth, parent, indexInParent, isDisjunct){
      ...
      if (origBranchKeys.length === 0) {
        ...
        // Otherwise, an empty dictionary means that this branch is universal (Ξ)
        // That is, that it would match _everything_.
        //
        // > We'll treat this in the same way as we would a `E_FILTER_WOULD_MATCH_EVERYTHING` exception
        // > coming from our call to `normalizeFilter()` later in this recursive function.
        throw flaverr('E_UNIVERSAL', new Error('Would match everything'));


        // // here is the old way:   (TODO: remove this once sure this will actually work)
        // // ------------------------------------------------------------------------------------
        // // ```
        ...
        // AND HERE ARE THE INTERNAL CONCEPTS FOR THE NEW WAY which is implemented above:
        // // ------------------------------------------------------------------------------------
        // // If this is a disjunct, then...
        // if (isDisjunct) {

        //   // We know that we can consider the entire parent `or` as always matching.
        //   //
        //   // So we'll throw a special signal indicating to the previous recursive step that it should stop
        //   // what it's doing and transform the parent `or` into an `and` instead, with an empty array on its RHS.
        //   // > This will cause it to be interpreted as a no-op predicate (the exact behavior of which will depend
        //   // > on ITS parent, and so on, recursively up).
        //   // TODO
        //   return;

        // }//‡
        // // Otherwise, this is a conjunct (part of an `and`).
        // else {

        //   // We know that this conjunct doesn't actually affect the parent `and`.
        //   // So we'll throw a special signal indicating to the previous recursive step that it should
        //   // rip this conjunct out of the parent `and` after it's finished iterating.
        //   // > This MAY cause the `and` array to become empty-- and if it does, that's ok.  We check for
        //   // > that later, and interpret it as a no-op predicate (the exact behavior of which will depend on
        //   // > ITS parent, and so on, recursively up).
        //   // TODO
        //   return;
      ...
      //
      // So in order to provide the simplest possible interface for adapter implementors,
      // we need to handle this in a special way.
      //
      // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      // At first you might think it should work something like this:
      // ```
      ...
      // ```
      //
      // But an empty array of conjuncts/disjuncts is not EXACTLY invalid, per se.
      ...
      // | ("matches everything")  | conjunct.         | parent will match | `where` clause    |
      // |                         |                   | EVERYTHING.       | with `{}`.        |
      // |                         |                   |                   |                   |
      // | Ξ                       | x ∩ Ξ = x         | x ∪ Ξ = Ξ         | Ξ                 |
      // | (universal)             | <<identity>>      | <<annihilator>>   | (universal)       |
      // |-------------------------|-------------------|-------------------|-------------------|
      // |                         |                   |                   |                   |
      // | `{ or:  [] }`           | Throw to indicate | Rip out this      | Throw E_WOULD_... |
      // | ("matches nothing")     | parent will NEVER | disjunct.         | RESULT_IN_NOTHING |
      // |                         | match anything.   |                   | error to indicate |
      // |                         |                   |                   | that this query   |
      // |                         |                   |                   | is a no-op.       |
      // |                         |                   |                   |                   |
      // | Ø                       | x ∩ Ø = Ξ         | x ∪ Ø = x         | Ø                 |
      // | (void)                  | <<annihilator>>   | <<identity>>      | (void)            |
      // |-------------------------|-------------------|-------------------|-------------------|
      //
      ...
      //
      // > **There are some edge cases to watch out for:**
      // > • If removing this conjunct/disjunct would cause the parent predicate operator to have
      // >   NO items, then recursively apply the normalization all the way back up the tree,
      // >   unless we hit the root (in which case, follow the same strategy discussed above).
      // > • If there is no containing conjunct/disjunct (i.e. because we're at the top-level),
      // >   then either throw a E_WOULD_RESULT_IN_NOTHING error (if this is an `or`), or
      // >   revert the criteria to `{}` so it matches everything (if this is an `and`)
      // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      ...
      //   │ │ │├─┘───│  ├┤ └┐┌┘├┤ │
      //   ┴ └─┘┴     ┴─┘└─┘ └┘ └─┘┴─┘
      // First, if there is no parent (this is the top level)...
      if (recursionDepth === 0) {
        assert(soleBranchKey === 'and' || soleBranchKey === 'or');
      ...
      // Otherwise, IWMIH, we know this branch has a parent.

      //  ┌─┐┌─┐┬─┐┌─┐┌┐┌┌┬┐  ┬┌─┐  ╔═╗╦═╗
      //  ├─┘├─┤├┬┘├┤ │││ │   │└─┐  ║ ║╠╦╝
      //  ┴  ┴ ┴┴└─└─┘┘└┘ ┴   ┴└─┘  ╚═╝╩╚═
      // If the parent is an `or` (this branch is a disjunct)...
      if (isDisjunct) {
      ...
      //  ├─┘├─┤├┬┘├┤ │││ │   │└─┐  ╠═╣║║║ ║║
      //  ┴  ┴ ┴┴└─└─┘┘└┘ ┴   ┴└─┘  ╩ ╩╝╚╝═╩╝
      // Otherwise, the parent is an `and` (this branch is a conjunct)...
      else {
      ...
      }//</else  (parent is `and` -- i.e. this branch is a conjunct)>


    })//</self-invoking recursive function (def)>
    //
    // Kick off our recursion with the `where` clause:
    (whereClause, 0, undefined, undefined, undefined);
