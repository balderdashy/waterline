...
module.exports = function normalizeWhereClause(whereClause, modelIdentity, orm, ensureTypeSafety) {
  ...
  (function _recursiveStep(someDictionary){

    // IWMIH, we know that `someDictionary` is a dictionary.
    // But that's about all we can trust.
    //
    // > In an already-fully-normalized `where` clause, we'd know that this dictionary
    // > would ALWAYS be a conjunct/disjunct.  But since we're doing the normalizing here,
    // > we have to be more forgiving-- both for usability and backwards-compatibility.


    // Loop over each key in this dictionary.
    _.each(_.keys(someDictionary), function (key){
      ...
      var rhs = someDictionary[key];


      //  ┌─┐┬ ┬┌─┐┬─┐┌┬┐┬ ┬┌─┐┌┐┌┌┬┐  ┌─┐┌─┐┬─┐  ╦╔╗╔  ╔═╗╦╦ ╔╦╗╔═╗╦═╗
      //  └─┐├─┤│ │├┬┘ │ ├─┤├─┤│││ ││  ├┤ │ │├┬┘  ║║║║  ╠╣ ║║  ║ ║╣ ╠╦╝
      //  └─┘┴ ┴└─┘┴└─ ┴ ┴ ┴┴ ┴┘└┘─┴┘  └  └─┘┴└─  ╩╝╚╝  ╚  ╩╩═╝╩ ╚═╝╩╚═
      // Else if this is "IN" shorthand...
      if (_.isArray(rhs)) {

        // TODO: move these checks down to where they also apply to explicit use of the `in` modifier w/i a complex filter
        // ================================================================================================================================================================
        ...
        rhs = {
          in: someDictionary[key]
        };
        someDictionary[key] = rhs;
      ...
      if (_.contains(PREDICATE_OPERATORS, key)) {
        ...
        _.each(rhs, function (conjunctOrDisjunct){
          ...
          // Recursive call
          _recursiveStep(conjunctOrDisjunct);
            ...
            // be validated according to which modifer it is
              ...
              if (_.contains(STRING_SEARCH_MODIFIERS, subAttrModifierKey)) {
                if (!_.isString(modifier) && !_.isNumber(modifier) && !_.isBoolean(modifier)){
                  throw flaverr('E_WHERE_CLAUSE_UNUSABLE', new Error('Unexpected value at sub-attribute modifier (`'+subAttrModifierKey+'`) for `'+key+'`:'+util.inspect(modifier,{depth: null})+'\n(The right-hand side of a string search modifier like `'+subAttrModifierKey+'` must always be a string, number, or boolean.)'));
                }
              }
