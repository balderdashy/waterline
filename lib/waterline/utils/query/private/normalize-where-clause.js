  ...
  try {
    ...
    (function _recursiveStep(branch, recursionDepth, parent, indexInParent){
      ...
      if (indexesToRemove.length > 0) {
        var newArr = [];
        for (var i = 0; i < conjunctsOrDisjuncts.length; i++) {
          if(indexesToRemove.indexOf(i) === -1) {
            newArr.push(conjunctsOrDisjuncts[i]);
          }
        }//</for>
        Array.prototype.splice.apply(conjunctsOrDisjuncts, [0, conjunctsOrDisjuncts.length].concat(newArr));
      }//>-
