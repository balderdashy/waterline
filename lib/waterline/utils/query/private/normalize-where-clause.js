  ...
  (function _recursiveStep(branch, recursionDepth, parent, keyOrIndexFromParent){
    ...
    if (origBranchKeys.length > 1) {
      ...
      _.each(origBranchKeys, function (origKey){
        ...
        // ```
        //
        // If so, still automatically map it.
        // But log a deprecation warning here as well, since it's more explicit to
        // avoid using predicates within multi-facet shorthand (i.e. could have used
        // an additional `and` predicate instead.)
        //
        // > NOTE: This could change- there are two sides to it, for sure.
    ...
    if (!_.contains(PREDICATE_OPERATOR_KINDS, soleBranchKey)) {
      ...
      if (isComplexFilter && _.keys(branch[soleBranchKey]).length > 1){
        ...
        // If this complex filter has multiple keys, fracture it before proceeding.
        // TODO
