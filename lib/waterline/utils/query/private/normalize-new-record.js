...
module.exports = function normalizeNewRecord(newRecord, modelIdentity, orm, ensureTypeSafety) {
  ...
  _.each(WLModel.attributes, function (attrDef, attrName) {
    ...
    if (!attrDef.required) {
      ...
      // Or generate the autoCreatedAt timestamp.
      else if (attrDef.autoCreatedAt) {

        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        // TODO: support autoCreatedAt  (with special support for createdAt)
        // (note that this should probably just be pulled up the the top level -- because think of the `newRecords`
        // QK from .createEach(), and how they all ought to share the exact same timestamp)
        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      }
    ...
    throw flaverr('E_MISSING_REQUIRED', new Error(
      ...
      'but instead, got: '+util.inspect(newRecord[attrName], {depth: null})+''
    ));
