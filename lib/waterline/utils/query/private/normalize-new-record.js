...
module.exports = function normalizeNewRecord(newRecord, modelIdentity, orm, currentTimestamp, ensureTypeSafety) {
  ...
  _.each(_.keys(newRecord), function (supposedAttrName){
    ...
    try {
      newRecord[supposedAttrName] = normalizeValueToSet(newRecord[supposedAttrName], supposedAttrName, modelIdentity, orm, currentTimestamp, ensureTypeSafety, true);
    } catch (e) {
      switch (e.code) {
  ...
  _.each(WLModel.attributes, function (attrDef, attrName) {
    ...
    if (!attrDef.required) {
      ...
      // when initially creating a record)
        ...
        // > Note that this is passed in so that all new records share the exact same timestamp
        // > (in a `.createEach()` scenario, for example)
