 ...
 *
 * @throws {Error} If it encounters any invalid values within the provided `newRecord`
 *   |     @property {String} code
 *   |      - E_VALIDATION
 *   |     @property {Array} all
 *   |         [
 ...
 *   |           }
 *   |         ]
 *   |     @property {Dictionary} byAttribute
 *   |         {
 *   |           foo: [
 *   |             {
 *   |               message: '...',
...
module.exports = function normalizeNewRecord(newRecord, modelIdentity, orm, currentTimestamp) {
  ...
  // Along the way, keep track of high-level validation rule violations.
  var hasAnyVdnErrors;
  var vdnErrorsByAttrName = {};
    ...
    try {
      ...
      switch (e.code) {
        ...
        case 'E_VIOLATES_RULES':
          ...
          vdnErrorsByAttrName[supposedAttrName] = e.ruleViolations;
          hasAnyVdnErrors = true;
          return;
  ...
  if (hasAnyVdnErrors) {
    throw flaverr('E_VALIDATION', new Error(
  ...
  if (_.isNull(newRecord[WLModel.primaryKey])) {
    throw flaverr('E_HIGHLY_IRREGULAR', new Error(
      'Cannot specify `null` as the primary key value (`'+WLModel.primaryKey+'`) for a new record.  '+
