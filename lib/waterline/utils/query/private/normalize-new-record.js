...
module.exports = function normalizeNewRecord(newRecord, modelIdentity, orm, currentTimestamp) {
  ...
  // Along the way, keep track of any rule violations.
  var allRuleViolationsMaybe;
  var allRuleViolationsByAttrNameMaybe;
  _.each(_.keys(newRecord), function (supposedAttrName){
    ...
    try {
      newRecord[supposedAttrName] = normalizeValueToSet(newRecord[supposedAttrName], supposedAttrName, modelIdentity, orm, true);
    } catch (e) {
      switch (e.code) {
        ...
        case 'E_TYPE':
          throw flaverr('E_TYPE', new Error(
        ...
        case 'E_VIOLATES_RULES':
          ...
          // Ensure we've started our list+set of violations.
          allRuleViolationsMaybe = allRuleViolationsMaybe || [];
          allRuleViolationsByAttrNameMaybe = allRuleViolationsByAttrNameMaybe || {};

          // Add these violations to our flat list, attaching the attribute name as `attrName`.
          // (Note that we also attach `problem: 'rule'`.  This is just to allow for future diversification.)
          e.ruleViolations.forEach(function (violation){
            violation.attrName = supposedAttrName;
            violation.problem = 'rule';

            allRuleViolationsMaybe.push(violation);
          });

          // And also add them to our "by attr name" dictionary.
          allRuleViolationsByAttrNameMaybe[supposedAttrName] = e.ruleViolations;

          // Then continue on to the next property.
          return;
  ...
  // If any value violated high-level rules, then throw.
  if (allRuleViolationsMaybe) {
    ...
    //    - Value was not an email address.
    // ```
    var summary = _.reduce(allRuleViolationsByAttrNameMaybe, function (memo, ruleViolations, attrName){
      memo += '  • '+attrName+'\n';
      _.each(ruleViolations, function(violation){
        memo += '    - '+violation.message+'\n';
      });
      return memo;
    }, '');

    // Build & throw actual error.
    throw flaverr({
      code: 'E_VIOLATES_RULES',
      modelIdentity: modelIdentity,
      ruleViolations: allRuleViolationsMaybe,
      toJSON: function(){
        return {
      ...
      'Invalid value(s) provided for new `'+modelIdentity+'`:\n'+
      summary
    ));
  ...
  _.each(WLModel.attributes, function (attrDef, attrName) {
    ...
    // Default singular associations to `null`.
      ...
      assert(_.isUndefined(attrDef.defaultsTo), '`defaultsTo` should never be defined for an association.  But `'+attrName+'` declares just such an inappropriate `defaultsTo`: '+util.inspect(attrDef.defaultsTo, {depth:5})+'');
      newRecord[attrName] = null;
    }
