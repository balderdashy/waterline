...
module.exports = function normalizeNewRecord(newRecord, modelIdentity, orm, currentTimestamp) {
  ...
  _.each(WLModel.attributes, function (attrDef, attrName) {
    ...
    // Default singular associations to `null`.
      ...
      if (!(_.isUndefined(attrDef.defaultsTo))) { throw new Error('`defaultsTo` should never be defined for an association.  But `'+attrName+'` declares just such an inappropriate `defaultsTo`: '+util.inspect(attrDef.defaultsTo, {depth:5})+''); }
      newRecord[attrName] = null;
    }
    ...
    // > the exact same timestamp (in a `.createEach()` scenario, for example)
      ...
      if (!(attrDef.type === 'number' || attrDef.type === 'string')) { throw new Error('If an attribute has `autoCreatedAt: true` or `autoUpdatedAt: true`, then it should always have either `type: \'string\'` or `type: \'number\'`.  But the definition for attribute (`'+attrName+'`) has somehow gotten into an impossible state: It has `autoCreatedAt: '+attrDef.autoCreatedAt+'`, `autoUpdatedAt: '+attrDef.autoUpdatedAt+'`, and `type: \''+attrDef.type+'\'`'); }
