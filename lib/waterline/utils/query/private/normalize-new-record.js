  ...
  _.each(WLModel.attributes, function (attrDef, attrName) {
    ...
    if (!_.isUndefined(newRecord[attrName])) {
      ...
      if (attrDef.autoCreatedAt || attrDef.autoUpdatedAt) {
        console.warn('Manually setting a value for `' + attrName + '` in new `' + WLModel.globalId + '` record, even though it has `' + (attrDef.autoCreatedAt ? 'autoCreatedAt' : 'autoUpdatedAt') + ': true`...');
      }
