...
module.exports = function normalizePkValue (pkValue, expectedPkType){
  assert(expectedPkType === 'string' || expectedPkType === 'number', new Error('Consistency violation: The internal normalizePkValue() utility must always be called with a valid second argument ("string" or "number").  But instead, got: '+util.inspect(expectedPkType, {depth:null})+''));
  ...
  if (expectedPkType === 'string') {
    if (!_.isString(pkValue)) {
      ...
      throw flaverr('E_INVALID_PK_VALUE', new Error('The provided value is not a valid string: '+util.inspect(pkValue,{depth:null})+''));
    }//-•

    // Empty string ("") is never a valid primary key value.
    if (pkValue === '') {
      throw flaverr('E_INVALID_PK_VALUE', new Error('Cannot use empty string ('+util.inspect(pkValue,{depth:null})+') as a primary key value.'));
    }//-•
  ...
  // Else if explicitly expecting numbers...
    ...
    if (!_.isNumber(pkValue)) {
      ...
      if (!_.isString(pkValue)) {
        throw flaverr('E_INVALID_PK_VALUE', new Error(
          'Instead of a number, got: '+util.inspect(pkValue,{depth:null})+''
        ));
      }//-•
      ...
      if (!canPrblyCoerceIntoValidNumber) {
        throw flaverr('E_INVALID_PK_VALUE', new Error(
          'Instead of a number, the provided value (`'+util.inspect(pkValue,{depth:null})+'`) is a string, and it cannot be coerced automatically (contains characters other than numerals 0-9).'
        ));
      }//-•
      ...
      if (coercedNumber > (Number.MAX_SAFE_INTEGER||9007199254740991)) {
        throw flaverr('E_INVALID_PK_VALUE', new Error(
          'Instead of a number, the provided value (`'+util.inspect(pkValue,{depth:null})+'`) is a string, and it cannot be coerced automatically (despite its numbery appearance, it\'s just too big!)'
        ));
      }//-•
    ...
    if (pkValue === 0) {
      throw flaverr('E_INVALID_PK_VALUE', new Error('Cannot use zero ('+util.inspect(pkValue,{depth:null})+') as a primary key value.'));
    }//-•

    // A negative number is never a valid primary key value.
    if (pkValue < 0) {
      throw flaverr('E_INVALID_PK_VALUE', new Error('Cannot use a negative number ('+util.inspect(pkValue,{depth:null})+') as a primary key value.'));
    }//-•

    // A floating point number is never a valid primary key value.
    if (Math.floor(pkValue) !== pkValue) {
      throw flaverr('E_INVALID_PK_VALUE', new Error('Cannot use a floating point number ('+util.inspect(pkValue,{depth:null})+') as a primary key value.'));
    }//-•

    // Neither Infinity nor -Infinity are ever valid as primary key values.
    if (Infinity === pkValue || -Infinity === pkValue) {
      throw flaverr('E_INVALID_PK_VALUE', new Error('Cannot use `Infinity` or `-Infinity` (`'+util.inspect(pkValue,{depth:null})+'`) as a primary key value.'));
    }//-•
    ...
    if (pkValue > (Number.MAX_SAFE_INTEGER||9007199254740991)) {
      throw flaverr('E_INVALID_PK_VALUE', new Error('Cannot use the provided value (`'+util.inspect(pkValue,{depth:null})+'`), because it is too large to safely fit into a JavaScript integer (i.e. `> Number.MAX_SAFE_INTEGER`)'));
    }//-•

    // Now do one last check as a catch-all, w/ a generic error msg.
    if (!isSafeNaturalNumber(pkValue)) {
      throw flaverr('E_INVALID_PK_VALUE', new Error('Cannot use the provided value (`'+util.inspect(pkValue,{depth:null})+'`) as a primary key value -- it is not a "safe", natural number (see https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Number/isSafeInteger).'));
    }
