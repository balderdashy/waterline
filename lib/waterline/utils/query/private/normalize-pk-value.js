...
module.exports = function normalizePkValue (pkValue, expectedPkType){
  if (!(expectedPkType === 'string' || expectedPkType === 'number')) { throw new Error('The internal normalizePkValue() utility must always be called with a valid second argument ("string" or "number").  But instead, got: '+util.inspect(expectedPkType, {depth:5})+''); }
