...
module.exports = function normalizePkValue (pkValue, expectedPkType){
  if (process.env.NODE_ENV !== 'production') { assert(expectedPkType === 'string' || expectedPkType === 'number', 'The internal normalizePkValue() utility must always be called with a valid second argument ("string" or "number").  But instead, got: '+util.inspect(expectedPkType, {depth:5})+''); }
  ...
  // Else if explicitly expecting numbers...
    ...
    if (!_.isNumber(pkValue)) {
      ...
      if (!canPrblyCoerceIntoValidNumber) {
        throw flaverr('E_INVALID_PK_VALUE', new Error(
          'Instead of a number, the provided value (`'+util.inspect(pkValue,{depth:5})+'`) is a string, and it cannot be coerced automatically (contains characters other than numerals 0-9).'
        ));
      }//-•
      ...
      if (coercedNumber > (Number.MAX_SAFE_INTEGER||9007199254740991)) {
        throw flaverr('E_INVALID_PK_VALUE', new Error(
          'Instead of a number, the provided value (`'+util.inspect(pkValue,{depth:5})+'`) is a string, and it cannot be coerced automatically (despite its numbery appearance, it\'s just too big!)'
        ));
      }//-•
