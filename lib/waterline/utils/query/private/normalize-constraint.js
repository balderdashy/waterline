...
module.exports = function normalizeConstraint (constraint, attrName, modelIdentity, orm){
  ...
  if (!_.isString(attrName)) {
    throw new Error('Consistency violation: The internal normalizeConstraint() utility must always be called with a valid second argument (a string).  But instead, got: '+util.inspect(attrName, {depth:5})+'');
  }
  ...
  if (_.isObject(constraint) && !_.isFunction(constraint) && !_.isArray(constraint)) {
    ...
    if (numKeys !== 1) {
      throw new Error('Consistency violation: If provided as a dictionary, the constraint passed in to the internal normalizeConstraint() utility must always have exactly one key.  (Should have been normalized already.)  But instead, got: '+util.inspect(constraint, {depth:5})+'');
    }
