...
module.exports = function normalizeConstraint (constraintRhs, attrName, modelIdentity, orm, meta){
  ...
  if (_.isObject(constraintRhs) && !_.isFunction(constraintRhs) && !_.isArray(constraintRhs)) {
    ...
    if (numKeys === 0) {
      throw flaverr('E_CONSTRAINT_NOT_USABLE', new Error(
        'If specifying a complex constraint, there should always be at least one modifier.  But the constraint RHS provided for `'+attrName+'` has no keys-- it is just `{}`, an empty dictionary (aka plain JavaScript object).'
      ));
    }//-•
  ...
  // > If there is no attr def, then check that it's a string, number, boolean, or `null`.
    ...
    try {
      constraintRhs = normalizeComparisonValue(constraintRhs, attrName, modelIdentity, orm);
    } catch (e) {
      switch (e.code) {
