 ...
 * normalizeConstraint()
 *
 * Validate and normalize the provided constraint.
 *
 * ------------------------------------------------------------------------------------------
 * @param  {Ref} constraint              [may be MUTATED IN PLACE!]
 *
...
module.exports = function normalizeConstraint (constraint, attrName, modelIdentity, orm){
  if (process.env.NODE_ENV !== 'production') { assert(!_.isUndefined(constraint), 'The internal normalizeConstraint() utility must always be called with a first argument (the constraint to normalize).  But instead, got: '+util.inspect(constraint, {depth:5})+''); }
  if (process.env.NODE_ENV !== 'production') { assert(_.isString(attrName), 'The internal normalizeConstraint() utility must always be called with a valid second argument (a string).  But instead, got: '+util.inspect(attrName, {depth:5})+''); }
  if (process.env.NODE_ENV !== 'production') { assert(_.isString(modelIdentity), 'The internal normalizeConstraint() utility must always be called with a valid third argument (a string).  But instead, got: '+util.inspect(modelIdentity, {depth:5})+''); }
  ...
  if (_.isArray(constraint)) {
    ...
    var inConstraintShorthandArray = constraint;
    constraint = { in: inConstraintShorthandArray };
  ...
  if (_.isObject(constraint) && !_.isFunction(constraint) && !_.isArray(constraint)) {
    ...
    var numKeys = _.keys(constraint).length;
    if (numKeys === 0) {
      throw flaverr('E_CONSTRAINT_NOT_USABLE', new Error(
        'If specifying a complex constraint, there should always be at least one modifier.  But the constraint provided for `'+attrName+'` has no keys-- it is just `{}`, an empty dictionary (aka plain JavaScript object).'
      ));
    }//-•

    if (process.env.NODE_ENV !== 'production') { assert(numKeys === 1, 'If provided as a dictionary, the constraint passed in to the internal normalizeConstraint() utility must always have exactly one key.  (Should have been normalized already.)  But instead, got: '+util.inspect(constraint, {depth:5})+''); }
    ...
    var modifierKind = _.keys(constraint)[0];
    ...
    if (!MODIFIER_KINDS[modifierKind] && MODIFIER_ALIASES[modifierKind]) {
      ...
      delete constraint[originalModifierKind];
      modifierKind = MODIFIER_ALIASES[originalModifierKind];
      constraint[modifierKind] = modifier;
    ...
    if (modifierKind === '!=') {
      ...
      try {
        modifier = normalizeComparisonValue(modifier, attrName, modelIdentity, orm);
      } catch (e) {
        switch (e.code) {
    ...
    //  ╩╝╚╝
      ...
      modifier = _.map(modifier, function (item){
        ...
        try {
          item = normalizeComparisonValue(item, attrName, modelIdentity, orm);
        } catch (e) {
          switch (e.code) {
    ...
    // A complex constraint must always contain a recognized modifier.
      ...
      throw flaverr('E_CONSTRAINT_NOT_USABLE', new Error(
        'Unrecognized modifier (`'+modifierKind+'`) within provided constraint for `'+attrName+'`.'
      ));
  ...
  // > If there is no attr def, then check that it's a string, number, boolean, or `null`.
    ...
    try {
      constraint = normalizeComparisonValue(constraint, attrName, modelIdentity, orm);
    } catch (e) {
      switch (e.code) {
