...
module.exports = function normalizeConstraint (constraintRhs, constraintTarget, modelIdentity, orm, meta){
  ...
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  // FUTURE: Support dot notation in constraint targets for lookups within embeds
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -

  // Try to look up the definition of the attribute that this constraint is referring to.
  var attrDef;
  try {
    attrDef = getAttribute(constraintTarget, modelIdentity, orm);
  } catch (e){
    switch (e.code) {
  ...
  if (WLModel.hasSchema === true) {
    ...
    if (!attrDef) {
      throw flaverr('E_CONSTRAINT_NOT_USABLE', new Error(
        '`'+constraintTarget+'` is not a recognized attribute for this '+
  ...
  // Else if model is `schema: false`...
    ...
    if (!isValidAttributeName(constraintTarget)) {
      throw flaverr('E_CONSTRAINT_NOT_USABLE', new Error(
        '`'+constraintTarget+'` is not a valid name for an attribute in Waterline.  '+
  ...
  if (attrDef && attrDef.collection) {
    throw flaverr('E_CONSTRAINT_NOT_USABLE', new Error(
      'Cannot filter by `'+constraintTarget+'` because it is a plural association (which wouldn\'t make sense).'
    ));
  }//-•
  ...
  if (_.isObject(constraintRhs) && !_.isFunction(constraintRhs) && !_.isArray(constraintRhs)) {
    ...
    if (numKeys === 0) {
      throw flaverr('E_CONSTRAINT_NOT_USABLE', new Error(
        'If specifying a complex constraint, there should always be at least one modifier.  But the constraint RHS provided for `'+constraintTarget+'` has no keys-- it is just `{}`, an empty dictionary (aka plain JavaScript object).'
      ));
    }//-•
