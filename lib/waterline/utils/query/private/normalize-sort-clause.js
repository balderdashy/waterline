...
module.exports = function normalizeSortClause(sortClause, modelIdentity, orm) {
  ...
  sortClause = _.map(sortClause, function (comparatorDirective){
    ...
    if (WLModel.hasSchema === true) {
      ...
      // (Also look up the attribute definition, because we're going to do a bit more.)
      var attrDef;
      try {
        attrDef = getAttribute(sortByKey, modelIdentity, orm);
      } catch (e){
        switch (e.code) {
      ...
      // That is, it must be either a singular (`model`) association, or explicitly
      // declare a JSON-compatible type ("string", "number", "boolean", or "json").
      //
      // In other words: it must NOT be a plural (`collection`) association, and it
      // must NOT declare `type: 'ref'`.
      if (attrDef.collection) {
        throw flaverr('E_SORT_CLAUSE_UNUSABLE', new Error(
          'Cannot sort by `'+sortByKey+'` because it corresponds with an "unsortable" attribute '+
      ...
      if (attrDef.type === 'ref') {
        throw flaverr('E_SORT_CLAUSE_UNUSABLE', new Error(
          'Cannot sort by `'+sortByKey+'` because it corresponds with an "unsortable" attribute '+
          'definition for this model (`'+modelIdentity+'`).  This attribute is `type: \'ref\', so '+
          'sorting by it is not supported.'
        ));
      }//-•
