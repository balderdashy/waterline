...
module.exports = function normalizeCriteria(criteria, modelIdentity, orm, ensureTypeSafety) {
  ...
  if (!_.isArray(criteria.select)) {
    throw flaverr('E_HIGHLY_IRREGULAR', new Error(
      ...
      util.inspect(criteria.select, {depth:null})+''
    ));
  }//-•
  ...
  assert(_.isObject(criteria) && !_.isArray(criteria) && !_.isFunction(criteria), 'At this point, the criteria should have already been normalized into a dictionary!  But instead somehow it looks like this: '+util.inspect(criteria, {depth:null})+'');
