 ...
 *                 the provided criteria, including e.g. an invalid filter is specified
 *                 for an association.
...
module.exports = function normalizeCriteria(criteria, modelIdentity, orm, ensureTypeSafety) {
  ...
  try {
    criteria.where = normalizeWhereClause(criteria.where, modelIdentity, orm, ensureTypeSafety);
  } catch (e) {
    switch (e.code) {

      case 'E_WHERE_CLAUSE_UNUSABLE':
        throw flaverr('E_HIGHLY_IRREGULAR', new Error(
          'Could not use the provided `where` clause.  Refer to the documentation '+
      ...
      default:
        throw new Error('Consistency violation: Encountered unexpected internal error when attempting to normalize/validate the `where` clause in the provided criteria:\n'+util.inspect(criteria, {depth:null})+'\n\nError details:\n'+e.stack);
    }
  }//>-•
  ...
  if (_.isUndefined(criteria.limit)) {
    console.log('oh its undefined.  ok');
    console.log('hey Number.MAX_SAFE_INTEGER is ',Number.MAX_SAFE_INTEGER);
    criteria.limit = Number.MAX_SAFE_INTEGER;
    console.log('changed it to:',criteria.limit);
  }//>-
  ...
  try {
    ...
    switch (e.code) {
      ...
      case 'E_SORT_CLAUSE_UNUSABLE':
        throw flaverr('E_HIGHLY_IRREGULAR', new Error(
          'Could not use the provided `sort` clause.  Refer to the documentation '+
          'for up-to-date info on supported query language syntax:\n'+
          '(http://sailsjs.com/docs/concepts/models-and-orm/query-language)\n'+
          'Details: '+ e.message
        ));
      ...
      // kind of unexpected, internal error on our part.
      default:
        throw new Error('Consistency violation: Encountered unexpected internal error when attempting to normalize/validate the `sort` clause in the provided criteria:\n'+util.inspect(criteria, {depth:null})+'\n\nError details:\n'+e.stack);
    }
  }//>-•
