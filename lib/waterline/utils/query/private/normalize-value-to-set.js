...
module.exports = function normalizeValueToSet(value, supposedAttrName, modelIdentity, orm, allowCollectionAttrs) {
  ...
  // ================================================================================================
  //¶_¶ assert(_.isString(supposedAttrName) && supposedAttrName !== '', '`supposedAttrName` must be a non-empty string.');
  ...
  // > We'll use loose validation (& thus also light coercion) on the value and see what happens.
    ...
    //¶_¶ assert(_.isString(correspondingAttrDef.type) && correspondingAttrDef.type !== '', 'There is no way this attribute (`'+supposedAttrName+'`) should have been allowed to be registered with neither a `type`, `model`, nor `collection`!  Here is the attr def: '+util.inspect(correspondingAttrDef, {depth:5})+'');
