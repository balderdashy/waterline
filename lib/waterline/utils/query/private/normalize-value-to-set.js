...
module.exports = function normalizeValueToSet(value, supposedAttrName, modelIdentity, orm, ensureTypeSafety, allowCollectionAttrs) {
  ...
  // > We'll use loose validation (& thus also light coercion) on the value and see what happens.
    ...
    if (isProvidingNullForIncompatibleOptionalAttr) {
      // ----------------------------------------------------------------------------
      // Note that, when creating new records, since this attribute does not define
      // a `defaultsTo`, you can simply omit this key to achieve the same effect.
      // TODO: pull that (^^^) up to normalize-new-record, since it's kind of important
      // ----------------------------------------------------------------------------
      throw flaverr('E_INVALID', new Error(
        ...
        'please set the value for this attribute to the base value for its type, '+(function (){
