...
module.exports = function normalizeValueToSet(value, supposedAttrName, modelIdentity, orm, allowCollectionAttrs) {
  ...
  // > We'll use loose validation (& thus also light coercion) on the value and see what happens.
    ...
    //  │  ├─┤├┤ │  ├┴┐  ├┤ │ │├┬┘  ╠╦╝║ ║║  ║╣   ╚╗╔╝║║ ║║  ╠═╣ ║ ║║ ║║║║╚═╗
    //  └─┘┴ ┴└─┘└─┘┴ ┴  └  └─┘┴└─  ╩╚═╚═╝╩═╝╚═╝   ╚╝ ╩╚═╝╩═╝╩ ╩ ╩ ╩╚═╝╝╚╝╚═╝
    // If our (potentially-mildly-coerced) value is anything other than `null`, then
    // strictly enforce the validation ruleset defined on the corresponding attribute.
    // If there are any rule violations, stick them in an Error and throw it.
    // (High-level validation rules are ALWAYS skipped for `null`.)
    var doCheckForRuleViolations = !_.isNull(value);
    if (doCheckForRuleViolations) {

      var ruleset = correspondingAttrDef.validations;
      assert(_.isObject(ruleset) && !_.isArray(ruleset) && !_.isFunction(ruleset), 'The `validations` attribute key should always be a dictionary.  But for the `'+modelIdentity+'` model\'s `'+supposedAttrName+'` attribute, it somehow ended up as this instead: '+util.inspect(correspondingAttrDef.validations,{depth:5})+'');
