...
var normalizePkValueOrValues = require('./normalize-pk-values');
...
module.exports = function normalizeValueToSet(value, supposedAttrName, modelIdentity, orm, allowCollectionAttrs) {
  ...
  //  └  └─┘┴└─  ╩  ╩═╝╚═╝╩╚═╩ ╩╩═╝  ╩ ╩╚═╝╚═╝╚═╝╚═╝╩╩ ╩ ╩ ╩╚═╝╝╚╝
    ...
    if (!allowCollectionAttrs) {
      throw flaverr('E_HIGHLY_IRREGULAR', new Error(
        'This method does not allow values to be set for plural (`collection`) associations '+
  ...
  //  └  └─┘┴└─  ╚═╝╩╝╚╝╚═╝╚═╝╩═╝╩ ╩╩╚═  ╩ ╩╚═╝╚═╝╚═╝╚═╝╩╩ ╩ ╩ ╩╚═╝╝╚╝
    ...
    if (_.isNull(value)) {
      ...
      if (correspondingAttrDef.required) {
        throw flaverr('E_HIGHLY_IRREGULAR', new Error(
          'Cannot set `null` as the value for `'+supposedAttrName+'`.  `null` CAN be '+
  ...
  // > We'll use loose validation (& thus also light coercion) on the value and see what happens.
    ...
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    // TODO: expand meaning of `required` to be backwards-compatible and to take
    // into account the final spec for Waterline v0.13 (see aforementioned spreadsheet)
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    if (doCheckForRuleViolations) {
      ...
      if (ruleViolations.length > 0) {
        throw flaverr({
          ...
          'The value specified for `'+supposedAttrName+'` violates one or more validation rules.'
        ));
      }//-•
