    ...
    if (correspondingAttrDef.required) {
      ...
      if (value === '') {
        throw flaverr('E_REQUIRED', new Error(
          'Cannot set "" (empty string) for `'+supposedAttrName+'` because it is a required attribute.'
        ));
      }//>-•
      ...
      if (_.isNull(value)) {
        throw flaverr('E_REQUIRED', new Error(
          'Cannot set `null` for `'+supposedAttrName+'` because it is a required attribute.'
        ));
      }//-•
