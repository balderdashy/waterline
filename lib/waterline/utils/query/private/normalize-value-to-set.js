...
module.exports = function normalizeValueToSet(value, supposedAttrName, modelIdentity, orm, allowCollectionAttrs) {
  ...
  // ================================================================================================
  if (!(_.isString(supposedAttrName) && supposedAttrName !== '')) { throw new Error('`supposedAttrName` must be a non-empty string.'); }
  ...
  //  └  └─┘┴└─  ╩  ╩═╝╚═╝╩╚═╩ ╩╩═╝  ╩ ╩╚═╝╚═╝╚═╝╚═╝╩╩ ╩ ╩ ╩╚═╝╝╚╝
    ...
    if (!allowCollectionAttrs) {
      throw flaverr('E_HIGHLY_IRREGULAR', new Error(
        'As a precaution, prevented replacing entire collection association (`'+supposedAttrName+'`).  '+
  ...
  // > We'll use loose validation (& thus also light coercion) on the value and see what happens.
    ...
    if (!(_.isString(correspondingAttrDef.type) && correspondingAttrDef.type !== '')) { throw new Error('There is no way this attribute (`'+supposedAttrName+'`) should have been allowed to be registered with neither a `type`, `model`, nor `collection`!  Here is the attr def: '+util.inspect(correspondingAttrDef, {depth:5})+''); }
    ...
    if (doCheckForRuleViolations) {
      if (!(_.isObject(ruleset) && !_.isArray(ruleset) && !_.isFunction(ruleset))) { throw new Error('If set, an attribute\'s validations ruleset (`validations`) should always be a dictionary (plain JavaScript object).  But for the `'+modelIdentity+'` model\'s `'+supposedAttrName+'` attribute, it somehow ended up as this instead: '+util.inspect(correspondingAttrDef.validations,{depth:5})+''); }
