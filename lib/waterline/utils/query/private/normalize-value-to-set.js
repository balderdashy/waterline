...
module.exports = function normalizeValueToSet(value, supposedAttrName, modelIdentity, orm, allowCollectionAttrs) {
  ...
  // > We'll use loose validation (& thus also light coercion) on the value and see what happens.
    ...
    if (!(_.isString(correspondingAttrDef.type) && correspondingAttrDef.type !== '')) { throw new Error('Consistency violation: there is no way this attribute (`'+supposedAttrName+'`) should have been allowed to be registered with neither a `type`, `model`, nor `collection`!  Here is the attr def: '+util.inspect(correspondingAttrDef, {depth:5})+''); }
