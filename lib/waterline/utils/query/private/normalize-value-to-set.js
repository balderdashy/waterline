...
module.exports = function normalizeValueToSet(value, supposedAttrName, modelIdentity, orm, allowCollectionAttrs) {
  ...
  // > We'll use loose validation (& thus also light coercion) on the value and see what happens.
    ...
    assert(_.isObject(correspondingAttrDef.validations) && !_.isArray(correspondingAttrDef.validations) && !_.isFunction(correspondingAttrDef.validations), 'The `validations` attribute key should always be a dictionary.  But for the `'+modelIdentity+'` model\'s `'+supposedAttrName+'` attribute, it somehow ended up as this instead: '+util.inspect(correspondingAttrDef.validations,{depth:5})+'');

    //  ┌─┐┬ ┬┌─┐┌─┐┬┌─  ┌─┐┌─┐┬─┐  ╦═╗╦ ╦╦  ╔═╗  ╦  ╦╦╔═╗╦  ╔═╗╔╦╗╦╔═╗╔╗╔╔═╗
    //  │  ├─┤├┤ │  ├┴┐  ├┤ │ │├┬┘  ╠╦╝║ ║║  ║╣   ╚╗╔╝║║ ║║  ╠═╣ ║ ║║ ║║║║╚═╗
    //  └─┘┴ ┴└─┘└─┘┴ ┴  └  └─┘┴└─  ╩╚═╚═╝╩═╝╚═╝   ╚╝ ╩╚═╝╩═╝╩ ╩ ╩ ╩╚═╝╝╚╝╚═╝
    // Strictly enforce the validation ruleset defined on the corresponding attribute
    // vs. our (potentially-mildly-coerced) value.  If there are any violations, stick
    // them in an Error and throw it.
    var ruleViolations;
    try {
      ruleViolations = anchor(value, correspondingAttrDef.validations);
    } catch (e) {
      throw new Error(
        'Could not run validation rules for `'+supposedAttrName+'`.  '+
        '(Usually this is related to a misconfigured validation rule in '+
        'the model definition.)  Error details: '+e.stack
      );
      // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      // FUTURE: in wl-schema, check validation rule config on lift.
      // Then this `throw` can just be changed to:
      // ```
      // throw new Error('Consistency violation: Unexpected error occurred when attempting to validate the provided `'+supposedAttrName+'`.  '+e.stack);
      // ```
      // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    }//</ catch >
