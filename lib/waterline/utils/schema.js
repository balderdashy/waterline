...
schema.normalizeAttributes = function(attrs) {
  return _.chain(attrs)
    ...
    .omit(_.isFunction)
    .transform(function(attributes, val, key) {
      ...
      if(_.has(val, 'collection')) {
        attrs[key].collection = val.collection.toLowerCase();
      }

      // Ensure Model property is lowercased
      if(_.has(val, 'model')) {
        attrs[key].model = val.model.toLowerCase();
      }
    }, {})
    .value();
};
...
schema.instanceMethods = _.partial(_.pick, _, _.isFunction);
...
schema.serializeJoins = function(criteria, collections) {
  ...
  joins.forEach(function(join) {
    ...
    if(!_.has(collections[join.parent], 'tableName')) return;
    if(!_.has(collections[join.child], 'tableName')) return;
