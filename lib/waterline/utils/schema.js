...
/**
 * Find all Association Keys in a model attributes object.
 *
 * Looks through all the attributes in a model and pulls out keys that have association
 * bindings in them. These should match [model, collection].
 *
 * Example:
 * {
 *   attributes: {
...
schema.associations = function(attrs) {
  var models = {},
      collections = {};
  ...
  Object.keys(attrs).forEach(function(key) {
    ...
    if(attrs[key].model) {
      models[key] = attrs[key].model;
      delete attrs[key];
      return;
    }

    if(attrs[key].collection) {
      collections[key] = attrs[key].collection;
      delete attrs[key];
      return;
    }
  });

  return { models: models, collections: collections };
};
