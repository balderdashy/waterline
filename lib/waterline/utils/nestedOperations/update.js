...
module.exports = function(parents, values, associations, cb) {
  ...
  var operations = buildOperations.call(this, associations, values);
...
function buildOperations(associations, values) {
  ...
  return _.transform(associations, function(operations, association) {
    ...
    if(!_.isArray(optValues)) optValues = [optValues];
    queueOperations.call(this, association, operations[association], optValues);
  }, {}, this);
...
function queueOperations(association, operation, values) {
  ...
  if(_.has(attribute, 'foreignKey')) modelName = attribute.references;
  if(!modelName) return;
  ...
  if(collection.junctionTable) {
    ...
    _.each(self.parents, function(parent) {
      _.each(values, function(val) {
        if(_.has(parent, association) && _.isFunction(parent[association].add)) {
...
function updateRunner(operations, cb) {
  ...
  async.each(_.keys(operations), associationLoop, cb);
...
function buildParentRemoveOperations(parent, operations) {
  ...
  return _.transform(operations, function(opts, operation, association) {
    ...
    if(_.has(childAttribute, 'foreignKey')) {
      ...
      criteria = {
        ...
        nullify: _.has(child, 'junctionTable') ? false : true
      };
...
function buildParentLinkOperations(parent, operations) {
  ...
  return _.transform(operations, function(recordsToCreate, operation, association) {
    _.each(operation.add, function(opt) {
      ...
      if(_.has(parent[association], 'add')) {
...
function createNewRecords(parent, recordsToCreate, cb) {
  ...
  async.each(_.keys(recordsToCreate), mapAssociations, cb);
}
