...
module.exports = function getModel(modelIdentity, orm) {
  ...
  if (process.env.NODE_ENV !== 'production') { assert(_.isString(modelIdentity), '`modelIdentity` must be a non-empty string.  Instead got: '+modelIdentity); }
  if (process.env.NODE_ENV !== 'production') { assert(modelIdentity !== '', '`modelIdentity` must be a non-empty string.  Instead got :'+modelIdentity); }
  if (process.env.NODE_ENV !== 'production') { assert(_.isObject(orm) && !_.isArray(orm) && !_.isFunction(orm), '`orm` must be a valid Waterline ORM instance (must be a dictionary)'); }
  if (process.env.NODE_ENV !== 'production') { assert(_.isObject(orm.collections) && !_.isArray(orm.collections) && !_.isFunction(orm.collections), '`orm` must be a valid Waterline ORM instance (must have a dictionary of "collections")'); }
  // ================================================================================================
  ...
  if (process.env.NODE_ENV !== 'production') { assert(_.isObject(WLModel) && !_.isArray(WLModel) && !_.isFunction(WLModel), 'All model definitions must be dictionaries, but somehow, the referenced Waterline model (`'+modelIdentity+'`) seems to have become corrupted.  Here it is: '+util.inspect(WLModel, {depth: 1})); }
  ...
  if (process.env.NODE_ENV !== 'production') { assert(_.isString(WLModel.primaryKey), 'The referenced Waterline model (`'+modelIdentity+'`) defines an invalid `primaryKey` setting.  Should be a string (the name of the primary key attribute), but instead, it is: '+util.inspect(WLModel.primaryKey, {depth:5})); }
  var pkAttrDef = WLModel.attributes[WLModel.primaryKey];
  if (process.env.NODE_ENV !== 'production') { assert(!_.isUndefined(pkAttrDef), 'The referenced Waterline model (`'+modelIdentity+'`) declares `primaryKey: \''+WLModel.primaryKey+'\'`, yet there is no `'+WLModel.primaryKey+'` attribute defined in the model!'); }
  if (process.env.NODE_ENV !== 'production') { assert(_.isObject(pkAttrDef) && !_.isArray(pkAttrDef) && !_.isFunction(pkAttrDef), 'The `primaryKey` (`'+WLModel.primaryKey+'`) in the referenced Waterline model (`'+modelIdentity+'`) corresponds with a CORRUPTED attribute definition: '+util.inspect(pkAttrDef, {depth:5})+'\n(^^this should have been caught already!)'); }
  if (process.env.NODE_ENV !== 'production') { assert(pkAttrDef.required === true || pkAttrDef.required === false, 'The `primaryKey` (`'+WLModel.primaryKey+'`) in the referenced Waterline model (`'+modelIdentity+'`) corresponds with a CORRUPTED attribute definition '+util.inspect(pkAttrDef, {depth:5})+'\n(^^this should have been caught already!  `required` must be either true or false!)'); }
