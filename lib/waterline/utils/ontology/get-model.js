...
module.exports = function getModel(modelIdentity, orm) {
  ...
  if (!(_.isString(modelIdentity))) { throw new Error('`modelIdentity` must be a non-empty string.  Instead got: '+modelIdentity); }
  if (modelIdentity === '') { throw new Error('`modelIdentity` must be a non-empty string.  Instead got :'+modelIdentity); }
  if (!(_.isObject(orm) && !_.isArray(orm) && !_.isFunction(orm))) { throw new Error('`orm` must be a valid Waterline ORM instance (must be a dictionary)'); }
  if (!(_.isObject(orm.collections) && !_.isArray(orm.collections) && !_.isFunction(orm.collections))) { throw new Error('`orm` must be a valid Waterline ORM instance (must have a dictionary of "collections")'); }
  // ================================================================================================
  ...
  if (!(_.isObject(WLModel) && !_.isArray(WLModel) && !_.isFunction(WLModel))) { throw new Error('Consistency violation: all model definitions must be dictionaries, but somehow, the referenced Waterline model (`'+modelIdentity+'`) seems to have become corrupted.  Here it is: '+util.inspect(WLModel, {depth: 1})); }
  if (!(_.isObject(WLModel.attributes) && !_.isArray(WLModel.attributes) && !_.isFunction(WLModel.attributes))) { throw new Error('Consistency violation: all model definitions must have a dictionary of `attributes`.  But somehow, the referenced Waterline model (`'+modelIdentity+'`) seems to have become corrupted and has a missing or invalid `attributes` property.  Here is the Waterline model: '+util.inspect(WLModel, {depth: 1})); }
  if (!(_.isString(WLModel.primaryKey))) { throw new Error('Consistency violation: the referenced Waterline model (`'+modelIdentity+'`) defines an invalid `primaryKey` setting.  Should be a string (the name of the primary key attribute), but instead, it is: '+util.inspect(WLModel.primaryKey, {depth:5})); }
  var pkAttrDef = WLModel.attributes[WLModel.primaryKey];
  if (_.isUndefined(pkAttrDef)) { throw new Error('Consistency violation: the referenced Waterline model (`'+modelIdentity+'`) declares `primaryKey: \''+WLModel.primaryKey+'\'`, yet there is no `'+WLModel.primaryKey+'` attribute defined in the model!'); }
  if (!(_.isObject(pkAttrDef) && !_.isArray(pkAttrDef) && !_.isFunction(pkAttrDef))) { throw new Error('Consistency violation: the `primaryKey` (`'+WLModel.primaryKey+'`) in the referenced Waterline model (`'+modelIdentity+'`) corresponds with a CORRUPTED attribute definition: '+util.inspect(pkAttrDef, {depth:5})+'\n(^^this should have been caught already!)'); }
  if (!(pkAttrDef.required === true || pkAttrDef.required === false)) { throw new Error('Consistency violation: the `primaryKey` (`'+WLModel.primaryKey+'`) in the referenced Waterline model (`'+modelIdentity+'`) corresponds with a CORRUPTED attribute definition '+util.inspect(pkAttrDef, {depth:5})+'\n(^^this should have been caught already!  `required` must be either true or false!)'); }
