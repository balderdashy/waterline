...
module.exports = function getModel(modelIdentity, orm) {
  ...
  assert(_.isUndefined(pkAttrDef.required) || pkAttrDef.required === true || pkAttrDef.required === false, new Error('Consistency violation: The `primaryKey` (`'+WLModel.primaryKey+'`) in the referenced Waterline model (`'+modelIdentity+'`) corresponds with a CORRUPTED attribute definition '+util.inspect(pkAttrDef, {depth:null})+'\n(^^this should have been caught already!  `required` must be either undefined, true, or false!)'));
