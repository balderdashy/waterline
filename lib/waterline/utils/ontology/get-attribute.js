...
module.exports = function getAttribute(attrName, modelIdentity, orm) {
  ...
  if (process.env.NODE_ENV !== 'production') { assert(_.isString(attrName) && attrName !== '', '`attrName` must be a non-empty string.'); }
  // ================================================================================================
  ...
  if (process.env.NODE_ENV !== 'production') { assert(_.isObject(attrDef) && !_.isArray(attrDef) && !_.isFunction(attrDef), 'The referenced attribute (`'+attrName+'`, from model `'+modelIdentity+'`) corresponds with a CORRUPTED attribute definition: '+util.inspect(attrDef, {depth:5})+''); }
  ...
  if (!_.isUndefined(attrDef.model)) {
    if (process.env.NODE_ENV !== 'production') { assert(_.isString(attrDef.model) && attrDef.model !== '', 'The referenced attribute (`'+attrName+'`, from model `'+modelIdentity+'`) has an invalid `model` property.  If specified, `model` should be a non-empty string.  But instead, got: '+util.inspect(attrDef.model, {depth:5})+''); }
    if (process.env.NODE_ENV !== 'production') { assert(_.isUndefined(attrDef.via), 'The referenced attribute (`'+attrName+'`, from model `'+modelIdentity+'`) is an association, because it declares a `model`.  But with a "model" association, the `via` property should always be undefined.  But instead, it is: '+util.inspect(attrDef.via, {depth:5})+''); }
  ...
  // (note that we don't get too deep here-- though we could)
    ...
    if (process.env.NODE_ENV !== 'production') { assert(_.isString(attrDef.collection) && attrDef.collection !== '', 'The referenced attribute (`'+attrName+'`, from model `'+modelIdentity+'`) has an invalid `collection` property.  If specified, `collection` should be a non-empty string.  But instead, got: '+util.inspect(attrDef.collection, {depth:5})+''); }
    var OtherWLModel;
    try {
    ...
    if (!_.isUndefined(attrDef.via)) {
      if (process.env.NODE_ENV !== 'production') { assert(_.isString(attrDef.via) && attrDef.via !== '', 'The referenced attribute (`'+attrName+'`, from model `'+modelIdentity+'`) has an invalid `via` property.  If specified, `via` should be a non-empty string.  But instead, got: '+util.inspect(attrDef.via, {depth:5})+''); }
      ...
      if (!_.isUndefined(attrDef.through)) {
        ...
        if (process.env.NODE_ENV !== 'production') { assert(OtherWLModel.attributes[attrDef.via], 'The referenced attribute (`'+attrName+'`, from model `'+modelIdentity+'`) is an association, because it declares a `collection`.  But that association also specifies a `via` ('+attrDef.via+'`) which does not correspond with a recognized attribute on the other model (`'+attrDef.collection+'`)'); }
      }
    }
  }
  // Check that this is a valid, miscellaneous attribute.
  else {
    if (process.env.NODE_ENV !== 'production') { assert(_.isString(attrDef.type) && attrDef.type !== '', 'The referenced attribute (`'+attrName+'`, from model `'+modelIdentity+'`) has an invalid `type` property.  If specified, `type` should be a non-empty string.  But instead, got: '+util.inspect(attrDef.type, {depth:5})+''); }
    if (process.env.NODE_ENV !== 'production') { assert(_.contains(KNOWN_ATTR_TYPES, attrDef.type), 'The referenced attribute (`'+attrName+'`, from model `'+modelIdentity+'`) has an unrecognized `type`: `'+attrDef.type+'`.'); }
    ...
    if (attrDef.required) {
      if (process.env.NODE_ENV !== 'production') { assert(_.isUndefined(attrDef.defaultsTo), 'The referenced attribute (`'+attrName+'`, from model `'+modelIdentity+'`) has `required: true`, but it also specifies a `defaultsTo`.  This should never have been allowed-- defaultsTo should be undefined!  But instead, got: '+util.inspect(attrDef.defaultsTo, {depth:5})+''); }
    }
  }
  // ================================================================================================
