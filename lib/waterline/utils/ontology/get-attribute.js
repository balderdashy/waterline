...
module.exports = function getAttribute(attrName, modelIdentity, orm) {
  ...
  //¶_¶ assert(_.isString(attrName) && attrName !== '', '`attrName` must be a non-empty string.');
  // ================================================================================================
  ...
  //¶_¶ assert(_.isObject(attrDef) && !_.isArray(attrDef) && !_.isFunction(attrDef), 'The referenced attribute (`'+attrName+'`, from model `'+modelIdentity+'`) corresponds with a CORRUPTED attribute definition: '+util.inspect(attrDef, {depth:5})+'');
  ...
  if (!_.isUndefined(attrDef.model)) {
    //¶_¶ assert(_.isString(attrDef.model) && attrDef.model !== '', 'The referenced attribute (`'+attrName+'`, from model `'+modelIdentity+'`) has an invalid `model` property.  If specified, `model` should be a non-empty string.  But instead, got: '+util.inspect(attrDef.model, {depth:5})+'');
    //¶_¶ assert(_.isUndefined(attrDef.via), 'The referenced attribute (`'+attrName+'`, from model `'+modelIdentity+'`) is an association, because it declares a `model`.  But with a "model" association, the `via` property should always be undefined.  But instead, it is: '+util.inspect(attrDef.via, {depth:5})+'');
    //¶_¶ assert(_.isUndefined(attrDef.dominant), 'The referenced attribute (`'+attrName+'`, from model `'+modelIdentity+'`) is an association, because it declares a `model`.  But with a "model" association, the `dominant` property should always be undefined.  But instead, it is: '+util.inspect(attrDef.dominant, {depth:5})+'');
    try {
  ...
  // (note that we don't get too deep here-- though we could)
    ...
    //¶_¶ assert(_.isString(attrDef.collection) && attrDef.collection !== '', 'The referenced attribute (`'+attrName+'`, from model `'+modelIdentity+'`) has an invalid `collection` property.  If specified, `collection` should be a non-empty string.  But instead, got: '+util.inspect(attrDef.collection, {depth:5})+'');
    var OtherWLModel;
    try {
    ...
    if (!_.isUndefined(attrDef.via)) {
      //¶_¶ assert(_.isString(attrDef.via) && attrDef.via !== '', 'The referenced attribute (`'+attrName+'`, from model `'+modelIdentity+'`) has an invalid `via` property.  If specified, `via` should be a non-empty string.  But instead, got: '+util.inspect(attrDef.via, {depth:5})+'');
      ...
      if (!_.isUndefined(attrDef.through)) {
        ...
        //¶_¶ assert(ThroughWLModel.attributes[attrDef.via], 'The referenced attribute (`'+attrName+'`, from model `'+modelIdentity+'`) is a "through" association, because it declares a `through`.  But the association\'s specified `via` ('+attrDef.via+'`) does not correspond with a recognized attribute on the junction model (`'+attrDef.through+'`)');
        //¶_¶ assert(ThroughWLModel.attributes[attrDef.via].model, 'The referenced attribute (`'+attrName+'`, from model `'+modelIdentity+'`) is a "through" association, but its specified `via` ('+attrDef.via+'`) corresponds with an unexpected attribute on the junction model (`'+attrDef.through+'`).  The attribute referenced by `via` should be a singular ("model") association, but instead, got: '+util.inspect(ThroughWLModel.attributes[attrDef.via],{depth: 5})+'');

      }
      else {
        //¶_¶ assert(OtherWLModel.attributes[attrDef.via], 'The referenced attribute (`'+attrName+'`, from model `'+modelIdentity+'`) is an association, because it declares a `collection`.  But that association also specifies a `via` ('+attrDef.via+'`) which does not correspond with a recognized attribute on the other model (`'+attrDef.collection+'`)');
      }
    }
  }
  // Check that this is a valid, miscellaneous attribute.
  else {
    //¶_¶ assert(_.isString(attrDef.type) && attrDef.type !== '', 'The referenced attribute (`'+attrName+'`, from model `'+modelIdentity+'`) has an invalid `type` property.  If specified, `type` should be a non-empty string.  But instead, got: '+util.inspect(attrDef.type, {depth:5})+'');
    //¶_¶ assert(_.contains(KNOWN_ATTR_TYPES, attrDef.type), 'The referenced attribute (`'+attrName+'`, from model `'+modelIdentity+'`) has an unrecognized `type`: `'+attrDef.type+'`.');
    //¶_¶ assert(attrDef.required === true || attrDef.required === false, 'The referenced attribute (`'+attrName+'`, from model `'+modelIdentity+'`) has an unrecognized `required` property in its definition.  By this time, it should always be true or false.  But instead, got: '+util.inspect(attrDef.required, {depth:5})+'');
    if (attrDef.required) {
      //¶_¶ assert(_.isUndefined(attrDef.defaultsTo), 'The referenced attribute (`'+attrName+'`, from model `'+modelIdentity+'`) has `required: true`, but it also specifies a `defaultsTo`.  This should never have been allowed-- defaultsTo should be undefined!  But instead, got: '+util.inspect(attrDef.defaultsTo, {depth:5})+'');
    }
  }
  // ================================================================================================
