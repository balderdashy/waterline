...
module.exports = function isCapableOfOptimizedPopulate(attrName, modelIdentity, orm) {
  ...
  if (!_.isString(attrName)) { throw new Error('Must specify `attrName` as a string.  But instead, got: '+util.inspect(attrName, {depth:5})+''); }
  if (!_.isString(modelIdentity)) { throw new Error('Must specify `modelIdentity` as a string.  But instead, got: '+util.inspect(modelIdentity, {depth:5})+''); }
  if (_.isUndefined(orm)) { throw new Error('Must pass in `orm` (a reference to the Waterline ORM instance).  But instead, got: '+util.inspect(orm, {depth:5})+''); }
  ...
  if (!(attrDef.model || attrDef.collection)) { throw new Error('Attempting to check whether attribute `'+attrName+'` of model `'+modelIdentity+'` is capable of optimized populate, but it\'s not even an association!'); }
