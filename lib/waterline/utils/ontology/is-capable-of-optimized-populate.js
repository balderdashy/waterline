...
module.exports = function isCapableOfOptimizedPopulate(attrName, modelIdentity, orm) {
  ...
  if (!_.isString(attrName)) { throw new Error('Consistency violation: must specify `attrName` as a string.  But instead, got: '+util.inspect(attrName, {depth:5})+''); }
  if (!_.isString(modelIdentity)) { throw new Error('Consistency violation: must specify `modelIdentity` as a string.  But instead, got: '+util.inspect(modelIdentity, {depth:5})+''); }
  if (_.isUndefined(orm)) { throw new Error('Consistency violation: must pass in `orm` (a reference to the Waterline ORM instance).  But instead, got: '+util.inspect(orm, {depth:5})+''); }
  ...
  if (!(attrDef.model || attrDef.collection)) { throw new Error('Consistency violation: attempting to check whether attribute `'+attrName+'` of model `'+modelIdentity+'` is capable of optimized populate, but it\'s not even an association!'); }
  ...
  if (!_.isString(PrimaryWLModel.datastore) || !_.isString(OtherWLModel.datastore)) {
    ...
    // ```
    // //¶_¶ assert(_.isString(PrimaryWLModel.datastore));
    // //¶_¶ assert(_.isString(OtherWLModel.datastore));
    // ```
  }
  ...
  if (JunctionWLModel) {
    ...
    if (!_.isString(JunctionWLModel.datastore)) {
      ...
      // ^^^TODO: instead of the above lines (^^^) replace it with the following lines:
      // ```
      // //¶_¶ assert(_.isString(JunctionWLModel.datastore));
      // ```
    }
  ...
  //¶_¶ assert(_.isString(relevantDatastoreName));
