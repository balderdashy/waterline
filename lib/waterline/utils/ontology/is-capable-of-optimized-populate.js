...
module.exports = function isCapableOfOptimizedPopulate(attrName, modelIdentity, orm) {
  ...
  // Grab the mapped schema for the relationship
  var primarySchemaDef = PrimaryWLModel.schema[attrName];

  // Check if the primary schema def is pointing to a junction table
  var childWLModel = getModel(primarySchemaDef.referenceIdentity, orm);
  ...
  var adapterMethodDictionary = PrimaryWLModel.adapterDictionary;
  if (_.has(adapterMethodDictionary, 'join') && adapterMethodDictionary.join === _.first(JunctionWLModel.datastore)) {
    doesAdapterSupportOptimizedPopulates = true;
  }
