...
module.exports = function isCapableOfOptimizedPopulate(attrName, modelIdentity, orm) {
  ...
  if (process.env.NODE_ENV !== 'production') { assert(_.isString(attrName), 'Must specify `attrName` as a string.  But instead, got: '+util.inspect(attrName, {depth:5})+''); }
  if (process.env.NODE_ENV !== 'production') { assert(_.isString(modelIdentity), 'Must specify `modelIdentity` as a string.  But instead, got: '+util.inspect(modelIdentity, {depth:5})+''); }
  if (process.env.NODE_ENV !== 'production') { assert(!_.isUndefined(orm), 'Must pass in `orm` (a reference to the Waterline ORM instance).  But instead, got: '+util.inspect(orm, {depth:5})+''); }
  ...
  if (process.env.NODE_ENV !== 'production') { assert(attrDef.model || attrDef.collection, 'Attempting to check whether attribute `'+attrName+'` of model `'+modelIdentity+'` is capable of optimized populate, but it\'s not even an association!'); }
  ...
  if (!_.isString(PrimaryWLModel.datastore) || !_.isString(OtherWLModel.datastore)) {
    ...
    // ```
    // if (process.env.NODE_ENV !== 'production') { assert(_.isString(PrimaryWLModel.datastore)); }
    // if (process.env.NODE_ENV !== 'production') { assert(_.isString(OtherWLModel.datastore)); }
    // ```
  }
  ...
  if (JunctionWLModel) {
    ...
    if (!_.isString(JunctionWLModel.datastore)) {
      ...
      // ^^^TODO: instead of the above lines (^^^) replace it with the following lines:
      // ```
      // if (process.env.NODE_ENV !== 'production') { assert(_.isString(JunctionWLModel.datastore)); }
      // ```
    }
  ...
  if (process.env.NODE_ENV !== 'production') { assert(_.isString(relevantDatastoreName)); }
  ...
  if (!PrimaryWLModel._adapter.join) {
    return false;
  }//-•
