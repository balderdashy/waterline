...
module.exports = function isCapableOfOptimizedPopulate(attrName, modelIdentity, orm) {
  ...
  // ```
  var isUsingSameDatastore = (_.first(PrimaryWLModel.datastore) === _.first(OtherWLModel.datastore));
  ...
  var JunctionWLModel;
  // To accomplish this, we'll grab the already-mapped relationship info (attached to
  // the model as the `schema` property, back in wl-schema).  
  var childWLModel = getModel(PrimaryWLModel.schema[attrName].referenceIdentity, orm);
  // (^^TODO: figure this out.  It looks like "childWLModel" can just go away?  What is it being used for?  Isn't this just the JunctionWLModel declared above?  So can't we just do JunctionWLModel = (...)?)

  // If the child model has a junction or through table flag then the JunctionWLModel
  // gets set to childWLModel.
  if (_.has(Object.getPrototypeOf(childWLModel), 'junctionTable') && _.isBoolean(Object.getPrototypeOf(childWLModel).junctionTable)) {
    JunctionWLModel = childWLModel;
  }
  // TODO: figure out if this is actually supposed to be exactly the same in both cases, or if this is a typo.
  if (_.has(Object.getPrototypeOf(childWLModel), 'throughTable') && _.isPlainObject(Object.getPrototypeOf(childWLModel).throughTable)) {
    JunctionWLModel = childWLModel;
  }
  // -----
  ...
  if (JunctionWLModel) {
    // ```
    isUsingSameDatastore = isUsingSameDatastore && (_.first(JunctionWLModel.datastore) === _.first(PrimaryWLModel.datastore));
    console.warn('TODO: outdated semantics (see https://github.com/balderdashy/waterline/commit/ecd3e1c8f05e27a3b0c1ea4f08a73a0b4ad83c07#commitcomment-20271012)  The `datastore` property should be a string, not an array.');
    // ```
    // ^^^TODO: instead of the above lines (^^^) replace it with the following lines:
    // ```
  ...
  var relevantDatastoreName = _.first(PrimaryWLModel.datastore);
  console.warn('TODO: outdated semantics (see https://github.com/balderdashy/waterline/commit/ecd3e1c8f05e27a3b0c1ea4f08a73a0b4ad83c07#commitcomment-20271012)  The `datastore` property should be a string, not an array.');
  ...
  if (PrimaryWLModel.adapterDictionary.join !== relevantDatastoreName) {
    return false;
  }//-•
