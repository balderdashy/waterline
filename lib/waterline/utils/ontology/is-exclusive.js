...
module.exports = function isExclusive(attrName, modelIdentity, orm) {
  ...
  if (!(attrDef.model || attrDef.collection)) { throw new Error('Attempting to check whether attribute `'+attrName+'` of model `'+modelIdentity+'` is an "exclusive" association, but it\'s not even an association in the first place!'); }
