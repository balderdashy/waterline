 ...
 * @returns {Array}  [results, complete w/ populations]
 *
 * @throws {Error} on invalid input
 */
module.exports = function integrate(cache, joinInstructions, primaryKey) {

  // Ensure valid usage
  var invalid = false;
  invalid = invalid || !_.isPlainObject(cache);
  invalid = invalid || !_.isArray(joinInstructions);
  invalid = invalid || !_.isPlainObject(joinInstructions[0]);
  invalid = invalid || !_.isString(joinInstructions[0].parentCollectionIdentity);
  invalid = invalid || !_.isString(primaryKey);
  if (invalid) {
    throw new Error('Invalid Integrator arguments.');
  }
  ...
  var results = cache[ joinInstructions[0].parentCollectionIdentity ];
    ...
    function eachAssociation(instructions, alias) {
      ...
      if (instructions.length === 2) {
        ...
        populate({
          ...
          childRows: innerJoin({
            left: leftOuterJoin({
              left: cache[instructions[0].parentCollectionIdentity],
              right: cache[instructions[0].childCollectionIdentity],
              leftKey: parentPK,
              rightKey: fkToParent
            }),
            right: cache[instructions[1].childCollectionIdentity],
            leftKey: CHILD_ATTR_PREFIX + fkToChild,
            rightKey: childPK,
            childNamespace: GRANDCHILD_ATTR_PREFIX
          }),
