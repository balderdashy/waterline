...
var normalize = module.exports = {
  ...
  expandPK: function(context, options) {
    ...
    if (!context.autoPK) {
      ...
      for(var key in context.attributes) {
        if(!_.has(context.attributes[key], 'primaryKey')) continue;
    ...
    if(_.isNumber(options) || _.isString(options) || _.isArray(options)) {
  ...
  criteria: function(origCriteria) {
    ...
    if(_.isArray(criteria)) return criteria;
    ...
    if(criteria.where) {
      var falsy = _.any(criteria.where, function(prop) {
        return _.isArray(prop) && prop.length === 0;
      });
    ...
    if(criteria.where && hop(criteria.where, 'or')) {
      ...
      _.each(criteria.where.or, function(clause, i) {
        _.each(clause, function(value) {
          if(_.isArray(clause) && clause.length === 0) {
            _clone.splice(i, 1);
          }
        });
      });
    ...
    if(hop(criteria, 'sort') && criteria.sort !== null) {
      ...
      // and normalize binary sorting criteria
      _.each(criteria.sort, function(sortCriteria, attr, sort) {
        if(sortCriteria === 'asc') sort[attr] = 1;
        else if(sortCriteria === 'desc' || sortCriteria === 0) sort[attr] = -1;
      });
  ...
  callback: function (cb) {
    ...
    if (_.isFunction(cb)) {
      wrappedCallback = function (err) {
        ...
        var modifiedArgs = _.rest(arguments);
