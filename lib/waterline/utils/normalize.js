...
var localUtil = require('./helpers');
var hop = localUtil.object.hasOwnProperty;
...
module.exports = {
  ...
  expandPK: function(context, options) {
    ...
    if (!context.autoPK) {
      ...
      for (var key in context.attributes) {
        if (!localUtil.object.hasOwnProperty(context.attributes[key], 'primaryKey')) continue;
  ...
  criteria: function(origCriteria) {
    ...
    // where we need the PK of the collection or a .findOrCreateEach (***except not anymore -- this is superceded by normalizeCriteria() anyway though***)
    ...
    if (hop(criteria, 'sort') && criteria.sort !== null) {
      ...
      if (!_.isObject(criteria.sort) && !_.isFunction(criteria.sort)) {
        throw new Error('Usage: Invalid sort criteria.  Got: '+util.inspect(criteria.sort, {depth: null}));
      }
    }
  ...
  likeCriteria: function(criteria, attributes, enhancer) {
    ...
    if (enhancer) criteria.where = localUtil.objMap(criteria.where, enhancer);
  ...
  resultSet: function(resultSet) {
    ...
    return localUtil.pluralize(resultSet, numberizeModel);
  },
...
function numberizeModel(model) {
  return localUtil.objMap(model, numberize);
}
