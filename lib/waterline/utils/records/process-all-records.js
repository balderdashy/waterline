  ...
  _.each(records, function(record) {
    ...
    _.each(WLModel.attributes, function(attrDef, attrName){
      ...
      if (attrName === WLModel.primaryKey) {
        ...
        // Verify that the record has some kind of reasonably-accurate-looking primary key.
        // (this is not a 100% thorough check, it's just here to be helpful.  Specifically,
        // it checks truthiness and primitive-ness)
        if (!record[attrName] || _.isObject(record[attrName])) {
          console.warn('\n'+
      ...
      // If this attribute is a plural association...
        ...
        // If the value is completely absent (`undefined`), then do nothing.
        if (_.isUndefined(record[attrName])) {
          // > This might just mean this collection wasn't populated.
          // > It could also mean there is crazy data in there, but we don't try
          // > to figure that out in any automatic way at the moment.
          // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        ...
        // Otherwise, if the value is anything besides an array...
      ...
      // If this attribute is a singular association...
        ...
        // If the value is completely absent, log a warning.
        if (_.isUndefined(record[attrName])) {
          // TODO
        }
        // Otherwise, verify that the corresponding value in the record is either:
        // • a dictionary
        // • `null`, or
        // • pk value (but note that we're not being 100% thorough here, for performance)
        else if (!_.isObject(record[attrName]) && !_.isNull(record[attrName]) && (record[attrName] === '' || record[attrName] === 0 || (!_.isNumber(record[attrName]) && !_.isString(record[attrName]))) ){
      ...
      // Otherwise, this is a misc. attribute.
        ...
        // Otherwise, a corresponding value of somet kind was specified in the record.
        else {
