...
var mapRecordsDeep = require('./private/map-records-deep');
...
module.exports = function processAllRecords(records, meta, modelIdentity, orm) {
  ...
  records = mapRecordsDeep(records, function _eachParentOrChildRecord(record, WLModel){
    ...
    if (WLModel.customToJSON) {
      console.log('ATTACHING TOJSON!!');
      // TODO: actually do this properly
      var Record = function Record(){};
      Record.prototype.toJSON = WLModel.customToJSON;

      var recordInstance = new Record();
      _.each(_.keys(record), function(key) {
        recordInstance[key] = record[key];
      });
  ...
  }, false, modelIdentity, orm);//</mapRecordsDeep>


  // Return potentially-modified records.
  return records;
