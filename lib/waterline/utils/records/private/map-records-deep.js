 ...
 *
 * The return value from the iteratee will be used as the new value for the record,
 * whether it's a parent record or a child record.  (This can be used to transform
 * records from plain dictionaries into special instances, for example.)
 *
...
module.exports = function mapRecordsDeep(records, iteratee, arePhysical, modelIdentity, orm) {
  ...
  records = _.map(records, function iterateRecords(record) {
    ...
    record = iteratee(record, ParentWLModel, 1);
    ...
    _.each(ParentWLModel.attributes, function iterateAttributes(attrDef, attrName) {
      ...
      if (attrDef.model) {
        ...
        childRecord = iteratee(childRecord, ChildWLModel, 2);

        // Set the child record under the appropriate property of the parent record
        // in case it was just changed.
        if (arePhysical) {
          record[attrDef.columnName] = childRecord;
        }
        else {
          record[attrName] = childRecord;
        }
        ...
        for (var i=0; i<childRecords.length; i++) {
          // Note that every child record is not guaranteed to be a dictionary!
          // (if you need this guarantee, use the `iteratee` to verify this when
          // it runs for parent records)
          childRecords[i] = iteratee(childRecords[i], ChildWLModel, 2);
        }
  ...
  }); // </ _.map() over each parent record >


  // Return the potentially-modified array of parent records.
  return records;
