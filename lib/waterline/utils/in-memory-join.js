...
//   ╚════╝  ╚═════╝ ╚═╝╚═╝  ╚═══╝╚══════╝
//
// Uses the Integrator to perform in-memory joins for a query. Used in both
...
module.exports = function inMemoryJoins(query, cache, primaryKey, cb) {
  Integrator(cache, query.joins, primaryKey, function integratorCb(err, results) {
    if (err) {
      return cb(err);
    }

    // If there were no results from the integrator, there is nothing else to do.
    if (!results) {
      return cb();
    }
    ...
    _.each(processedResults, function(result) {
      ...
      _.each(query.joins, function(join) {
        ...
        if (_.has(join.criteria, 'sort')) {
          result[join.alias] = Sorter(result[join.alias], join.criteria.sort);
        }
    ...
    return cb(undefined, processedResults);
  });
};
