    ...
    // Else if explicitly expecting numbers...
      ...
      if (!_.isNumber(thisPkValue)) {
        ...
        if (!canPrblyCoerceIntoValidNumber) {
          throw flaverr('E_INVALID_PK_VALUES', new Error('Expecting a number (or potentially a homogeneous array of numbers) for these primary key value(s).  But provided primary key value (`'+util.inspect(thisPkValue,{depth:null})+'`) is not a valid number, and cannot be coerced into one.'));
        }//-•
