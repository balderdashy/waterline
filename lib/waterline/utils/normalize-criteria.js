...
module.exports = function normalizeCriteria(criteria, modelIdentity, orm) {
  ...
  // If `orm` was provided, look up the model definition (`modelDef`) as well
  // as its primary attribute's name (`pkAttrName`) and definition (`pkAttrDef`).
  //
  // > Otherwise, if no `orm` was provided, then leave all three variables as `undefined`.
  // > In that case, we skip schema-aware validations in the relevant code below.
  var modelDef;
  var pkAttrName;
  var pkAttrDef;
  ...
  if (orm) {
    ...
    // > Check that the model definition exists, and do a couple of
    // > quick sanity checks on it.
    modelDef = orm.collections[modelIdentity];
    assert(!_.isUndefined(modelDef), new Error('Consistency violation: Provided `modelIdentity` references a model (`'+modelIdentity+'`) which does not exist in the provided `orm`.'));
    ...
    // > Check that the name of a primary key attribute is defined,
    // > and that it corresponds with a valid attribute definition.)
    pkAttrName = modelDef.primaryKey;
    ...
    pkAttrDef = modelDef.attributes[pkAttrName];
  ...
  if (_.isArray(criteria) || _.isNumber(criteria) || _.isString(criteria)) {
    try {
      ...
      var pkValues = normalizePkValues(criteria, pkAttrDef.type);
      ...
      criteria.where[pkAttrName] = pkValues;
