...
module.exports = function normalizeCriteria(criteria, modelIdentity, orm) {
  ...
  // If `limit` is still not a number at this point, it means it was
  // highly irregular in the first place (e.g. it might have been
  // provided as a dictionary, regexp, etc.)
  if (!_.isNumber(criteria.limit) || _.isNaN(criteria.limit)) {
    throw flaverr('E_HIGHLY_IRREGULAR', new Error(
      'The `limit` clause in the provided criteria is invalid.  It should be provided as a positive integer, but instead got: '+
      util.inspect(criteria.limit, {depth:null})+''
    ));
  }//-•


  // IWMIH, then we know limit must be a number.

  // A floating point number is never a valid limit.
  if (Math.floor(criteria.limit) !== criteria.limit) {
    throw flaverr('E_HIGHLY_IRREGULAR', new Error(
      'Cannot use a floating point number ('+util.inspect(criteria.limit,{depth:null})+') for `limit`.'
    ));
  }//-•
  ...
  // If specified limit is too large to be safely represented as a
  // JavaScript integer, then we'll call it highly irregular.
  //
  // > Remember, if it happens to have been provided as `Infinity`, we
  // > already handled that above by changing it to Number.MAX_SAFE_INTEGER.
