...
module.exports = function normalizeCriteria(criteria, modelIdentity, orm) {
  ...
  // until this a stage 3 query.  Technically, I _think_ this utility
  // can be used for normalizing criteria in stage 3 queries, I'm pretty
  ...
  _.each(criteria.select, function (attrNameToKeep){
    ...
    } else { throw new Error('Consistency violation: Every Waterline model should always have the `schema` model setting as either `true` or `false` (should have been normalized by waterline-schema).  But somehow, this model (`'+WLModel.identity+'`) has `schema: '+util.inspect(WLModel.schema, {depth:null})+'`'); }
  ...
  _.each(criteria.omit, function (attrNameToOmit){
    ...
    if (attrNameToOmit === WLModel.primaryKey) {
      throw flaverr('E_HIGHLY_IRREGULAR', new Error(
        'The `omit` clause in the provided criteria explicitly attempts to omit the primary key (`'+WLModel.primaryKey+'`).  But in the current version of Waterline, this is not possible. .  It explicitly attempts to omit the primary key it should be a safe, natural number.  But instead, got: '+
        util.inspect(criteria.limit, {depth:null})+''
      ));
    }//-•
    ...
    if (WLModel.schema === true) {
      ...
      // Make sure each item matches a recognized attribute name.
      // TODO
    ...
    // Else if model is `schema: false`...
      ...
      // In this, we probably just give up.
      // TODO: double-check that there's not a clean way to do this cleanly in a way that
      // supports both SQL and noSQL adapters.  Worst case, we throw a nice E_HIGHLY_IRREGULAR
      // error here explaining what's up.  Best case, we get it to work for Mongo et al somehow,
      // in which case we'd then also want to verify that each item is at least a valid Waterline
      // attribute name. (TODO)
