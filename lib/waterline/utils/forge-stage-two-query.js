...
module.exports = function forgeStageTwoQuery(query, orm) {
  ...
  if (_.contains(queryKeys, 'valuesToSet')) {
    ...
    // Strip keys w/ `undefined` on the RHS.
    // TODO


    // If this model declares `schema: true`, then check to be sure there are no values
    // specified for unrecognized attributes.
    // TODO


    // Check to be sure that there are no values specified that would attempt to set
    // a `required` attribute to `null`.
    //
    // > This is a bit different than `required` elsewhere in the world of machines/RTTC,
    // > because the world of data (i.e. JSON, databases, APIs, etc.) equate `undefined`
    // > and `null`.  To us in the JS/Node/Waterline world, if the RHS of a key is `undefined`,
    // > that means the same thing as if the key wasn't provided at all.  So because of that,
    // > we must use `null` to indicate both that we want to "clear something out" or that
    // > something "has no value".
    // TODO


    // Run RTTC loose validation vs. attributes' declared types.
    // (only if `meta.typeSafety` is set to `'strict'`)
    if (queryKeys.meta.typeSafety === 'strict') {

      // Check each provided value vs. the corresponding attribute definition's declared `type`.
      // (if it doesn't match any known attribute, then treat it as `type: 'json'`.)
      //
      // > Note: This is just like normal RTTC validation, with one major exception:
      // > • `null` is valid against any type (instead of only vs. `json` and `ref`.)
      // >
      // > This is because databases don't understand `undefined`, and so `null`
    ...
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    // > Note that high-level (anchor) validations are completely separate
    // > these type safety checks.  High-level validations are implemented
