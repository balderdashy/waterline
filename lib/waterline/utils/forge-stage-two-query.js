...
module.exports = function forgeStageTwoQuery(query, orm) {
  ...
  if (!_.isUndefined(query.meta)) {
    ...
    if (!_.isObject(query.meta) || _.isArray(query.meta) || _.isFunction(query.meta)) {
      throw flaverr('E_INVALID_META', new Error(
    ...
    try {
      ...
      switch (e.code) {
        ...
        case 'E_HIGHLY_IRREGULAR':
          throw flaverr('E_INVALID_CRITERIA', e);
          // If it turns out it's better for this error to be more
          // verbose, change over to using this instead:
          // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
