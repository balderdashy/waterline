...
var getModelInfo = require('./get-model-info');
...
module.exports = function forgeStageTwoQuery(query, orm) {
  ...
  // Look up the model definition (`modelDef`), and also, purely for convenience, get
  // references to its primary key attribute's name (`pkAttrName`) and definition (`pkAttrDef`).
  var modelInfo;
  try {
    modelInfo = getModelInfo(query.using);
  } catch (e) {
    switch (e.code) {
  ...
  var modelDef = modelInfo.modelDef;
  var pkAttrName = modelInfo.pkAttrName;
  var pkAttrDef = modelInfo.pkAttrDef;
