...
module.exports = function forgeStageTwoQuery(query, orm) {
    ...
    try {
      ...
      switch (e.code) {
        case 'E_INVALID_PK_VALUES':
          throw new Error('Usage error: The first argument passed to `.addToCollection()` should be the ID (or IDs) of target records whose collection will be modified.\nDetails: '+e.message);
        default: throw e;
      }
    }
  ...
  if (!_.isUndefined(query.collectionAttrName)) {
    ...
    if (!_.isString(query.collectionAttrName)) {
      throw new Error('Usage error: The second argument to `addToCollection()` should be the name of a collection association from this model, but instead got: '+util.inspect(query.collectionAttrName,{depth:null}));
    }
    ...
    if (!associationDef) {
      throw new Error('Usage error: The second argument to `addToCollection()` should be the name of a collection association, but there is no association named `'+query.collectionAttrName+'` defined in this model (`'+modelDef.identity+'`).');
    }

    // Validate that the association with this name is a collection association.
    if (!associationDef.collection) {
      throw new Error('Usage error: The second argument to `addToCollection()` should be the name of a collection association, but the association or attribute named `'+query.collectionAttrName+'` defined in this model (`'+modelDef.identity+'`) is NOT a collection association.');
    }
