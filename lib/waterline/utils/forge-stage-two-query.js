...
module.exports = function forgeStageTwoQuery(query, orm) {
      ...
      try {
        ...
        switch (e.code) {
          case 'E_INVALID_PK_VALUES':
            throw new Error('Invalid primary key value(s): '+e.message);
          default:
            throw e;
        }
      }//< / try : normalizePkValues >

    } catch (e) { throw flaverr('E_INVALID_TARGET_RECORD_IDS', e); }
  ...
  if (!_.isUndefined(query.collectionAttrName)) {
    ...
    try {
      ...
      if (!_.isString(query.collectionAttrName)) {
        throw new Error('Instead of a string, got: '+util.inspect(query.collectionAttrName,{depth:null}));
      }
      ...
      if (!associationDef) {
        throw new Error('There is no attribute named `'+query.collectionAttrName+'` defined in this model.');
      }

      // Validate that the association with this name is a collection association.
      if (!associationDef.collection) {
        throw new Error('The attribute named `'+query.collectionAttrName+'` defined in this model is NOT a collection association.');
      }

    } catch (e) { throw flaverr('E_INVALID_COLLECTION_ATTR_NAME', e); }
  ...
  if (!_.isUndefined(query.associatedIds)) {
    ...
    } catch (e) { throw flaverr('E_INVALID_ASSOCIATED_IDS', e); }
