...
module.exports = function CollectionBuilder(collection, datastores, context) {
  ...
  if (!_.has(collection.prototype, 'datastore')) {
    ...
    if (!_.has(datastores, 'default')) {
      throw new Error('No `datastore` was specified in the definition for model `' + collection.prototype.identity+'`, and there is no datastore named "default" to fall back to.');
    }
  ...
  if (!_.has(datastores, datastoreName)) {
    throw new Error('Unrecognized datastore (`' + datastoreName + '`) specified in the definition for model `' + collection.prototype.identity + '`.  Please make sure it exists (and if you\'re unsure, use "default".)');
  }
