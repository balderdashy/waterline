...
module.exports = function find( /* criteria?, populates?, done?, meta? */ ) {
  ...
  (function(proceed) {
    ...
    //  ╠╦╝║ ║║║║  │ │├─┘├┤ ├┬┘├─┤ │ ││ ││││└─┐
    //  ╩╚═╚═╝╝╚╝  └─┘┴  └─┘┴└─┴ ┴ ┴ ┴└─┘┘└┘└─┘
    var fqRunner = buildFQRunner(WLModel, query);
    var stageThreeQuery = fqRunner.queryObj;
    runFindOperations(fqRunner, stageThreeQuery, WLModel, function opRunnerCb(err, records) {
      if (err) {
        return done(err);
      }
      ...
      // Process the records
      processAllRecords(records, modelIdentity, orm);
