...
module.exports = function update(criteria, valuesToSet, done, metaContainer) {
  ...
  (function(proceed) {
    ...
    adapter.update(datastoreName, query, function _afterTalkingToAdapter(err, rawAdapterResult) {
      if (err) {
        if (!_.isError(err)) {
        ...
        if (err.code === 'E_UNIQUE') {
          // TODO: come up with some cleanish way to extrapolate this code in createEach()/update()/create()
          // and share it in all three places.  (See createEach() for what's there so far.)
        }//>-
