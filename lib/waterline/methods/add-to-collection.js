...
var addToCollectionHelper = require('../utils/collection-operations/add-to-collection');
...
module.exports = function addToCollection(/* targetRecordIds?, collectionAttrName?, associatedIds?, done?, meta? */) {
  ...
  if (!done) {
    return new Deferred(this, addToCollection, query);
  } // --•
      ...
      case 'E_INVALID_ASSOCIATED_IDS':
        return done(
          flaverr(
            { name: 'Usage error' },
            new Error(
              'The associated ids (i.e. third argument) passed to `.addToCollection()` should be '+
  ...
  addToCollectionHelper(query, this.waterline, done);
};
