...
module.exports = function findOne( /* criteria?, populates?, done?, meta? */ ) {
  ...
  (function(proceed) {
    ...
    //  ╠╦╝║ ║║║║  │ │├─┘├┤ ├┬┘├─┤ │ ││ ││││└─┐
    //  ╩╚═╚═╝╝╚╝  └─┘┴  └─┘┴└─┴ ┴ ┴ ┴└─┘┘└┘└─┘
    // Note: `OperationRunner` is responsible for running the `transformer`.
    // (i.e. so that column names are transformed back into attribute names)
    OperationRunner(operations, stageThreeQuery, self, function opRunnerCb(err, record) {
      if (err) {
        return done(err);
      }
      ...
      // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      // TODO: move this into the operation runner (or better yet, have the operation runner
      // stop dealing w/ findOne and handle it here inline)
      // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
