...
var buildFQRunner = require('../utils/query/operation-builder');
var runFindOperations = require('../utils/query/run-find-operations');
...
module.exports = function findOne( /* criteria?, populates?, done?, meta? */ ) {
  ...
  (function(proceed) {
    ...
    //  ╠╦╝║ ║║║║  │ │├─┘├┤ ├┬┘├─┤ │ ││ ││││└─┐
    //  ╩╚═╚═╝╝╚╝  └─┘┴  └─┘┴└─┴ ┴ ┴ ┴└─┘┘└┘└─┘
    // Note: `runFindOperations` is responsible for running the `transformer`.
    // (i.e. so that column names are transformed back into attribute names)
    var fqRunner = buildFQRunner(self, query);
    var stageThreeQuery = fqRunner.queryObj;
    runFindOperations(fqRunner, stageThreeQuery, self, function opRunnerCb(err, records) {
      if (err) {
        return done(err);
      }
      ...
      var foundRecord = _.first(records);

      // Check for adapter/data issues in the record.
      if (foundRecord) {
        processAllRecords([foundRecord], self.identity, self.waterline);
      }
