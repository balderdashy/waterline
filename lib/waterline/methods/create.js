...
module.exports = function create(values, done, metaContainer) {
  ...
  (function(proceed) {
    ...
    _.each(WLModel.attributes, function checkForCollection(attributeVal, attributeName) {
    ...
    adapter.create(datastoreName, query, function _afterTalkingToAdapter(err, rawAdapterResult) {
      ...
      if (!_.isObject(rawAdapterResult) || _.isArray(rawAdapterResult) || _.isFunction(rawAdapterResult)) {
        return done(new Error('Consistency violation: expected `create` adapter method to send back the created record.  But instead, got: ' + util.inspect(rawAdapterResult, {depth:5})+''));
      }
      ...
      async.each(_.keys(collectionResets), function resetCollection(collectionAttribute, next) {
        WLModel.replaceCollection(targetId, collectionAttribute, collectionResets[collectionAttribute], next, query.meta);
      }, function(err) {
        if (err) {
          return done(err);
        }
        ...
        (function(proceed) {
          ...
          if (_.has(WLModel._callbacks, 'afterCreate')) {
            return WLModel._callbacks.afterCreate(transformedRecord, proceed);
          }
