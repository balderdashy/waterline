...
module.exports = function create(values, done, metaContainer) {
  ...
  Error.captureStackTrace(omen, create);
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  // FUTURE: do something fancier here to keep track of this so that it supports both sorts of usages
  ...
  (function _maybeRunBeforeLC(proceed){
    ...
    adapter.create(WLModel.datastore, query, function _afterTalkingToAdapter(err, rawAdapterResult) {
      if (err) {
        ...
        // console.log('Error from adapter!  Here is whats in its footprint',err.footprint); TODO remove
        if (!_.isError(err)) {
        ...
        if (err.footprint && err.footprint.identity === 'notUnique') {
          // console.log('transforming UNIQUNEESS error:',err); TODO remove
