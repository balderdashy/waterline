...
module.exports = function createEach( /* newRecords?, done?, meta? */ ) {
  ...
  _.each(query.newRecords, function(record) {
    ...
    _.each(WLModel.attributes, function _checkForCollectionResets(attributeVal, attributeName) {
  ...
  if (!datastoreName) {
    return done(new Error('The adapter used in the ' + modelIdentity + ' model doesn\'t support the `createEach` method.)'));
  }
