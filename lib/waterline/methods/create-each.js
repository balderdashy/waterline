...
module.exports = function createEach( /* newRecords?, done?, meta? */ ) {
  ...
  // Build an Error instance up here in order to grab a stack trace.
  ...
  var pretendError = new Error('Pretend');
  ...
  adapter.createEach(WLModel.datastore, query, function(err, rawAdapterResult) {
    if (err) {
      ...
      if (err.footprint && err.footprint.identity === 'notUnique') {
        err = transformUniquenessError(err, pretendError, modelIdentity, orm);
      }//>-
