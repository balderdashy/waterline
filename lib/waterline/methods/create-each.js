...
module.exports = function createEach( /* newRecords?, done?, meta? */ ) {
  ...
  adapter.createEach(datastoreName, stageThreeQuery, function(err, rawAdapterResult) {
    ...
    _.each(rawAdapterResult, function(record) {
      ...
      try {
        transformedRecord = self._transformer.unserialize(record);
      } catch (e) {
    ...
    try {
      processAllRecords(transformedRecords, query.meta, self.identity, self.waterline);
    ...
    _.each(transformedRecords, function(record, i) {

      // Grab the dictionary of collection resets corresponding to this record.
      var reset = allCollectionResets[i];
