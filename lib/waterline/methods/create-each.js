...
module.exports = function createEach( /* newRecords?, done?, meta? */ ) {
  ...
  // Run the operation
  adapter.createEach(datastoreName, stageThreeQuery, function(err, rawAdapterResult) {
    if (err) {
      return done(err);
    }
    ...
    if (!_.has(stageThreeQuery.meta, 'fetch') || stageThreeQuery.meta.fetch === false) {
      ...
      if (!_.isUndefined(rawAdapterResult)) {
        console.warn(
          ...
          'should NOT have sent back anything as the 2nd argument when triggering the callback from its `createEach` method.  '+
    ...
    try {
      processAllRecords(rawAdapterResult, undefined, self.identity, self.waterline);
    ...
    _.each(rawAdapterResult, function(record, idx) {
