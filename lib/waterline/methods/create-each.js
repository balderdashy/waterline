...
module.exports = function createEach( /* newRecords?, done?, meta? */ ) {
  ...
  _.each(query.newRecords, function(record) {
    try {
      record = processValues(record, self);
    } catch (e) {
      // Don't return out the callback because the loop, like time, will keep on
      // ticking after we move on.
      processErrors.push(e);
    }
  });

  if (processErrors.length) {
    return done(processErrors);
  }
