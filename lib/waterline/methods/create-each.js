...
module.exports = function createEach( /* newRecords?, done?, meta? */ ) {
  ...
  adapter.createEach(datastoreName, query, function(err, rawAdapterResult) {
    if (err) {
      ...
      // the keys (mostly column names) back to attribute names.  (If an unrecognized
      // key is seen, then )
      //
      ...
      if (err.code === 'E_UNIQUE') {
        if (!_.isObject(err.footprint) || !_.isArray(err.footprint.keys)) {
          return done(new Error('Malformed E_UNIQUE error sent back from adapter: Should have a `footprint` property, a dictionary which also contains an array of `keys`!  But instead, `.footprint` is: '+util.inspect(err.footprint, {depth:5})+''));
        }

        err.attrNames = _.reduce(err.footprint.keys, function(memo, key){

          // Find matching attr name.
          var matchingAttrName;
          _.any(WLModel.attributes, function(attrDef, attrName) {
            if (!attrDef.columnName) {
              console.warn(
                'Warning: Malformed ontology: Model `'+modelIdentity+'` has an attribute (`'+attrName+'`) '+
                'with no `columnName`.  But at this point, every attribute should have a column name!  '+
                '(If you are seeing this error, the model definition may have been corrupted in-memory-- '+
                'or there might be a bug in WL schema.)'
              );
            }

            if (attrDef.columnName === key) {
          ...
          // Otherwise log a warning and silently ignore this item.
            ...
            console.warn(
              ...
              'violated references a key which cannot be matched up with any attribute: '+key+'.  This probably '+
