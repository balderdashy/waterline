...
module.exports = function createEach( /* newRecords?, done?, meta? */ ) {
  ...
  adapter.createEach(datastoreName, stageThreeQuery, function(err, records) {
    ...
    // as well as any issues related to stale data that hasn't been migrated
    // to match the new logical schema (`type`, etc. in attribute definitions).
    try {
    ...
    _.each(records, function(record, idx) {
      ...
      // Otherwise, for each reset key, build an array containing the first three arguments
      // that need to be passed in to `replaceCollection()`.
      var targetIds = [record[self.primaryKey]];
      _.each(_.keys(reset), function(collectionAttrName) {

        // (targetId, collectionAttrName, associatedPrimaryKeys)
        argsForEachReplaceOp.push([targetIds, collectionAttrName, reset[collectionAttrName]]);

      });// </ each key in "reset" >
    });//</ each record>

    async.each(argsForEachReplaceOp, function(argsForReplace, next) {
