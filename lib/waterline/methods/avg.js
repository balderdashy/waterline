...
module.exports = function avg( /* numericAttrName?, criteria?, moreQueryKeys?, done?, meta? */ ) {
  ...
  var datastoreName = WLModel.adapterDictionary.avg;
  if (!datastoreName) {
    return done(new Error('The adapter used in the ' + modelIdentity + ' model doesn\'t support the `avg` method.)'));
  }
  ...
  // Run the operation
  adapter.avg(datastoreName, stageThreeQuery, function _afterTalkingToAdapter(err, arithmeticMean) {
    if (err) {
