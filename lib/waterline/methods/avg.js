...
module.exports = function avg( /* numericAttrName?, criteria?, moreQueryKeys?, done?, meta? */ ) {
    ...
    switch (e.code) {
      ...
      case 'E_NOOP':
        return done(flaverr({ name: 'UsageError' }, new Error(
          ...
          'Details:\n'+
          '  ' + e.message + '\n'
          // ^^ Note that we use "message" instead of "details" for this one.
          // (That's because E_NOOP does not come from the `buildUsageError` helper.)
        )));
  ...
  var datastoreName = this.adapterDictionary.avg;
  if (!datastoreName) {
  ...
  // Run the operation
  adapter.avg(datastoreName, stageThreeQuery, function avgCb(err, value) {
    if (err) {
      return done(err);
    }
