...
module.exports = function sum( /* numericAttrName?, criteria?, moreQueryKeys?, done?, meta? */ ) {
  ...
  var datastoreName = WLModel.adapterDictionary.sum;
  if (!datastoreName) {
    return done(new Error('The adapter used in the ' + modelIdentity + ' model doesn\'t support the `sum` method.)'));
  }
  ...
  // Run the operation
  adapter.sum(datastoreName, stageThreeQuery, function _afterTalkingToAdapter(err, value) {
    if (err) {
