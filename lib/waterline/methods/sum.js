...
module.exports = function sum( /* numericAttrName?, criteria?, moreQueryKeys?, done?, meta? */ ) {
  ...
  var connectionName = this.adapterDictionary.sum;
  if (!connectionName) {
    return done(new Error('The adapter used in the ' + this.identity + ' model doesn\'t support the `sum` method.)'));
  }
  ...
  // Run the operation
  adapter.sum(connectionName, stageThreeQuery.using, stageThreeQuery, function(err, value) {
    if (err) {
      return done(err);
    }
