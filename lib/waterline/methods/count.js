...
module.exports = function count( /* criteria?, moreQueryKeys?, done?, meta? */ ) {
  ...
  var datastoreName = WLModel.adapterDictionary.count;
  if (!datastoreName) {
    return done(new Error('The adapter used in the ' + modelIdentity + ' model doesn\'t support the `count` method.)'));
  }
  ...
  // Run the operation
  adapter.count(datastoreName, query, function _afterTalkingToAdapter(err, numRecords) {
    if (err) {
