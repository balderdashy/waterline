 ...
 *
 * ```
 * // The number of bank accounts with more than $32,000 in it.
...
module.exports = function count( /* criteria?, moreQueryKeys?, done?, meta? */ ) {
  ...
  if (!done) {
    return new Deferred(this, count, query);
  } // --•
  ...
  var connectionName = this.adapterDictionary.count;
  if (!connectionName) {
    return done(new Error('The adapter used in the ' + this.identity + ' model doesn\'t support the `count` method.)'));
  }
  ...
  // Run the operation
  adapter.count(connectionName, stageThreeQuery.using, stageThreeQuery, function(err, value) {
    if (err) {
      return done(err);
    }
