...
module.exports = function archive(/* criteria, explicitCbMaybe, metaContainer */) {
  ...
  return parley(
    ...
    function (done){
      ...
      (function (proceed) {
        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        // FUTURE: `beforeArchive` lifecycle callback?
        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        return proceed();
      })(function (err, query) {
        ...
        // Verify the adapter has `destroy` & `find` methods.
        if (!adapter.destroy || !adapter.find) {
          return done(new Error('The adapter used by this model (`' + modelIdentity + '`) doesn\'t support the `destroy`+`find` methods.'));
        }
              ...
              (function _maybeTransformRecords(proceed){
                ...
                if (!_.isArray(rawAdapterResult)) {
                  return proceed(new Error(
                    ...
                    'the callback from its `archive` method.  But instead, got: '+util.inspect(rawAdapterResult, {depth:5})+''
                  ));
                }//-•
                ...
                (function (proceed){
                  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                  // FUTURE: `afterArchive` lifecycle callback?
                  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
                  return proceed();
                })(function(err){
                  if (err){ return done(err); }
