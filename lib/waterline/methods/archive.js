...
module.exports = function archive(/* criteria, explicitCbMaybe, metaContainer */) {
  ...
  return parley(
    ...
    function (done){
      ...
      // Look up the Archived model.
      var Archived;// TODO

      // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      // TODO: provide a way of choosing which datastore is "dominant"
      // as far as the Archived model is concerned (e.g. where does it live).
      //
      // TODO: provide a way of choosing the `tableName` and `columnName`s
      // for the Archived model
      //
      // TODO: provide a way of disabling the Archived model (and thus
      // disabling support for the `.archive()` model method)
      //
      // TODO: automigrate the built-in Archived model
      //
      // TODO: inject the built-in Archived model into the ORM's ontology
      //   • id              (pk-- string or number, depending on where the Archived model is being stored)
      ...
      // using .stream().  (This would allow you to potentially archive
      // millions of records at a time without overflowing RAM.)
      // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
      ...
      WLModel.find(query.criteria, function _afterFinding(err, foundRecords) {
        ...
        //  ║╣ ╔╩╦╝║╣ ║  ║ ║ ║ ║╣   │  ├┬┘├┤ ├─┤ │ ├┤ ├┤ ├─┤│  ├─┤  │─┼┐│ │├┤ ├┬┘└┬┘
        //  ╚═╝╩ ╚═╚═╝╚═╝╚═╝ ╩ ╚═╝  └─┘┴└─└─┘┴ ┴ ┴ └─┘└─┘┴ ┴└─┘┴ ┴  └─┘└└─┘└─┘┴└─ ┴
        Archived.createEach(itemsToArchive, function _afterCreatingEach(err) {
          if (err) { return done(err); }
