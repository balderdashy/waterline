  ...
  return parley(
    ...
    function (done){
      ...
      (function (proceed){
        ...
        //
        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        // FUTURE: Only do this .count() if the foreign key attribute is required
        // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
        WLChild.count(nullOutCriteria, function(err, total) {
          ...
          // (But note: there should only ever be zero or one target record.)
          if (query.targetRecordIds.length >= 2 && query.associatedIds.length >= 1) { return proceed(new Error('Consistency violation: Too many target record ids and associated ids-- should never have been possible, because this query should have been halted when it was being forged at stage 2.')); }
          _.each(query.targetRecordIds, function(targetId) {
            _.each(query.associatedIds, function(associatedId) {
          ...
          WLChild.update(nullOutCriteria, valuesToUpdate, function(err) {
            ...
            async.each(updateQueries, function(updateQuery, next) {
              // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
              // FUTURE: remove this unnecessary `async.each()`-- there should only
              // ever be exactly zero or exactly one query!  Even though there could
              // be multiple associated ids, there is never more than one target id,
              // so it's fine to just use an `in` query here.
              // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
