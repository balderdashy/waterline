  ...
  return parley(
    ...
    function (done){
      ...
      (function (proceed){
        ...
        // First, check if the foreign key attribute is required so that we know
        // whether it's safe to null things out without checking for collisions
        // beforehand.
        ...
        (function(proceed){
          ...
          if (!isFkAttributeOptional && numCollisions > 0) {
            return proceed(flaverr({
              ...
              'whose `'+schemaDef.via+'` cannot be set to `null` (because that attribute is required.)'
          ...
          if (query.associatedIds.length > 0) {
            ...
            // If there are no collisions, then we skip past this first "null out" query
            // altogether. (There's nothing to "null out"!)
            if (numCollisions === 0) {
          ...
          // Alternatively, we'll go with scenario B, where we potentially null all the fks out.
            ...
            // If there are no collisions, then we skip past the "null out" query altogether.
            // (There's nothing to "null out"!)
            if (numCollisions === 0) {
              return proceed();
            }//•
