  ...
  return parley(
    ...
    function (done){
      ...
      (function (proceed){
        ...
        //  ├─┤││││ │ │ ├─┤├┤ ├┬┘  │─┼┐│ │├┤ ├┬┘└┬┘   │ │ │  └─┐├┤  │   ├┤ │ │├┬┘├┤ ││ ┬│││  ├┴┐├┤ └┬┘└─┐
        //  ┴ ┴┘└┘└─┘ ┴ ┴ ┴└─┘┴└─  └─┘└└─┘└─┘┴└─ ┴    ┴ └─┘  └─┘└─┘ ┴   └  └─┘┴└─└─┘┴└─┘┘└┘  ┴ ┴└─┘ ┴ └─┘
        // We'll start with scenario A, where we simultaneously update all the foreign key
        // values for new associated records to point to one particular parent record (aka
        // target record) and null out the fk on any other existing records.
        if (query.associatedIds.length > 0) {

          async.parallel([

            // TODO: make sure it's actually safe to do these in parallel
            ...
            if (err) { return proceed(err); }
            return proceed();
          });//_∏_  </simultaneously running both queries>
