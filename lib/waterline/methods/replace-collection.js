    ...
    function (done){
      ...
      (function (proceed){
        ...
        WLChild.count(nullOutCriteria, function(err, total) {
          ...
          if (query.targetRecordIds.length >= 2) { return proceed(new Error('Consistency violation: Too many target record ids-- should never have been possible, because this query should have been halted when it was being forged at stage 2.')); }
          _.each(query.targetRecordIds, function(targetId) {
            _.each(query.associatedIds, function(associatedId) {
