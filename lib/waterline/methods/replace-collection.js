...
var replaceCollectionHelper = require('../utils/collection-operations/replace-collection');
...
module.exports = function replaceCollection(/* targetRecordIds?, collectionAttrName?, associatedIds?, done?, meta? */) {
  ...
  // Handle first few arguments:
  // (each of these always has exactly one meaning)
  query.targetRecordIds = arguments[0];
  query.collectionAttrName = arguments[1];
  query.associatedIds = arguments[2];

  // Handle 4th and 5th argument:
  // (both of which have exactly one meaning here)
  done = arguments[3];
  query.meta = arguments[4];
  ...
  if (!done) {
    return new Deferred(this, replaceCollection, query);
  } // --•
      ...
      case 'E_INVALID_ASSOCIATED_IDS':
        return done(
          flaverr(
            { name: 'UsageError' },
            new Error(
              'The associated ids (i.e. third argument) passed to `.replaceCollection()` should be '+
  ...
  replaceCollectionHelper(query, this.waterline, done);
};
