...
module.exports = function destroy(criteria, cb, metaContainer) {
  ...
  (function _runBeforeLC(proceed) {
    ...
    (function _handleCascadeOnDestroy(proceed) {
      ...
      // Run the operation
      adapter.destroy(datastoreName, stageThreeQuery, function destroyCb(err, rawAdapterResult) {
        if (err) {
          return cb(err);
        }
        ...
        (function _runCascadeOnDestroy(proceed) {
          ...
          (function _determineResult(proceed){
            ...
            if (!_.has(stageThreeQuery.meta, 'fetch') || stageThreeQuery.meta.fetch === false) {
              ...
              if (!_.isUndefined(rawAdapterResult) && _.isArray(rawAdapterResult)) {
                console.warn(
                  ...
                  'should NOT have sent back anything as the 2nd argument when triggering the callback from its `destroy` method.  '+
                  'But it did!  And since it\'s an array, displaying this warning to help avoid confusion and draw attention to '+
                  'the bug.  Specifically, got: '+util.inspect(rawAdapterResult, {depth:5})+'\n'+
            ...
            (function _runAfterLC(proceed) {
              ...
              if (doRunAfterLC) {
                return self._callbacks.afterDestroy(transformedRecords, proceed);
              }
