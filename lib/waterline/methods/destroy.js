...
module.exports = function destroy(criteria, done, metaContainer) {
  ...
  (function _runBeforeLC(proceed) {
    ...
    if (query.meta && (query.meta.cascade || query.meta.fetch)){
      ...
      if (!adapter.find) {
        return done(new Error('The adapter used by this model (`' + modelIdentity + '`) doesn\'t support the `find` method, but that method is mandatory to be able to use `cascade: true` or `fetch: true`.'));
      }
