...
module.exports = function destroy(criteria, done, metaContainer) {
  ...
  (function _runBeforeLC(proceed) {
    ...
    if (WLModel.adapterDictionary.find !== WLModel.adapterDictionary.destroy) {
      // ^^Sanity check to ensure we never allow any remnants of the old adapter-per-method approach from Sails v0.10.x.
    ...
    if (!datastoreName) {
      // ^^Another sanity check to ensure the adapter has both `find` and `destroy` methods.
      return done(new Error('Cannot complete query: The adapter used by this model (`' + modelIdentity + '`) doesn\'t support the `find` method.'));
    }
    ...
    (function _maybeFindIdsToDestroy(proceed) {
      ...
      var pkColumnName = getAttribute(WLModel.primaryKey, modelIdentity, orm).columnName;
