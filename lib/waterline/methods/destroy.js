  ...
  (function _runBeforeLC(proceed) {
    ...
    if (_.has(query.meta, 'skipAllLifecycleCallbacks') && query.meta.skipAllLifecycleCallbacks) {
      ...
      if (_.has(WLModel._callbacks, 'beforeDestroy')) {
        return WLModel._callbacks.beforeDestroy(query.criteria, proceed);
      }
    ...
    (function _handleCascadeOnDestroy(proceed) {
    ...
    })(function _afterPotentiallyLookingUpRecordsToCascade(err, cascadePrimaryKeysMaybe) {
      if (err) {
        return done(err);
      }
      ...
      adapter.destroy(datastoreName, query, function _afterTalkingToAdapter(err, rawAdapterResult) {
        ...
        (function _runCascadeOnDestroy(proceed) {
          ...
          if (!_.isArray(cascadePrimaryKeysMaybe)) {
            return done(new Error('Consistency violation: Should have an array of records looked up before!  But instead, got: '+util.inspect(cascadePrimaryKeysMaybe, {depth: 5})+''));
          }

          // Run the cascade which will handle all the `replaceCollection` calls.
          cascadeOnDestroy(cascadePrimaryKeysMaybe, WLModel, function (err) {
          ...
          (function _determineResult(proceed){
            ...
            (function _runAfterLC(proceed) {
              ...
              if (doRunAfterLC) {
                return WLModel._callbacks.afterDestroy(transformedRecordsMaybe, proceed);
              }
