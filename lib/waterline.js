...
function Waterline() {
  ...
  orm.initialize = function initialize(options, done) {
    ...
    // archiveModelInfoByIdentity «« TODO
    var archiveModelIdentities = [];

    // `wmd` stands for "weird intermediate model def thing".
    var wmds = modelDefs;
    // - - - - - - - - - - - - - - - - - - - - - - - -
    // FUTURE: make this less weird.
    // - - - - - - - - - - - - - - - - - - - - - - - -
    _.each(wmds, function(wmd){
      ...
      // Keep track of the model identities of all archive models.
      if (modelDef.archiveModelIdentity !== false) {
        if (!_.contains(archiveModelIdentities, modelDef.archiveModelIdentity)) {
          archiveModelIdentities.push(modelDef.archiveModelIdentity);
        }
      }//ﬁ
    ...
    if (_.contains(archiveModelIdentities, DEFAULT_ARCHIVE_MODEL_IDENTITY)) {
        ...
        var newWmd = Waterline.Model.extend({
          ...
          attributes: {
            id: {
              ...
              autoMigrations: {
                autoIncrement: true,// TODO: infer proper autoIncrement, if possible
              }
            },
            createdAt: { type: 'number', autoCreatedAt: true, autoMigrations: {} },
            originalRecord: { type: 'json', required: true, autoMigrations: {} },
            originalRecordId: { type: 'number', autoMigrations: {} },// TODO: infer proper type, if possible
            fromModel: { type: 'string', required: true, autoMigrations: {} }
          }
        });
        wmds.push(newWmd);
      }//ﬁ
    ...
    _.each(archiveModelIdentities, function(archiveIdentity) {
      var archiveWmd = _.find(wmds, function(wmd){ return wmd.prototype.identity === archiveIdentity; });
      if (!archiveWmd) {
        throw new Error('Invalid `archiveModelIdentity` setting.  A model declares `archiveModelIdentity: \''+archiveIdentity+'\'`, but there\'s no other model actually registered with that identity!');
      }
      // - - - - - - - - - - - - - - - - - - - - -
      // FUTURE: additional validation here
      // - - - - - - - - - - - - - - - - - - - - -
    });//∞
    ...
    // to make sure it's actually usable as the orm's Archive.  If not,
    // throw a descriptive error on ORM initialization.
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -



    // First, verify traditional settings, check compat., and build live models, etc.:
    // =============================================================================================
