  ...
  orm.initialize = function initialize(options, done) {
    ...
    async.each(_.keys(datastoreMap), function(datastoreName, next) {
      ...
      if (_.isFunction(datastore.adapter.registerConnection)) {
        throw new Error('The adapter for datastore `' + datastoreName + '` is invalid: the `registerConnection` method must be renamed to `registerDatastore`.');
      }
      ...
      if (!_.has(datastore.adapter, 'registerDatastore')) {
        // TODO: get rid of this `setImmediate` (or if it's serving a purpose, document what that is)
      ...
      return done(undefined, ontology);
