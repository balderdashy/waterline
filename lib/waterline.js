...
Waterline.prototype.initialize = function(config, cb) {
  ...
  function buildJunctionTable(item, next) {
    new item(schema.schema, config, next);
  }
  ...
  async.each(this._collections, loadCollection, function(err) {
    ...
    Object.keys(schema.schema).forEach(function(table) {
      if(!schema.schema[table].junctionTable) return;
      junctionTables.push(Waterline.Collection.extend(schema.schema[table]));
    });

    async.each(junctionTables, buildJunctionTable, function(err) {
      if(err) return cb(err);
      cb(null, collections);
    });
  });
