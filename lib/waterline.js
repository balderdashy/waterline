...
module.exports = function ORM() {
  ...
  orm.initialize = function initialize(options, cb) {
    ...
    if (!_.isUndefined(options.connections)){
      ...
      // Sanity check
      if (!_.isUndefined(options.datastores)) {
        throw new Error('Consistency violation: Attempted to provide backwards-compatibility for `connections`, but `datastores` was ALSO defined!  This should never happen.');
      }
...
// Expose the stateless Waterline `Model` constructor for direct access from your application.
module.exports.Model = WLModelConstructor;
//(note that we also expose `Collection` as an alias, but only for backwards compatibility)
