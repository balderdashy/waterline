...
function Waterline() {
  ...
  orm.initialize = function initialize(options, done) {
    ...
    // First, set up support for the default archive, and validate related model settings:
    // =============================================================================================
    var DEFAULT_ARCHIVE_MODEL_IDENTITY = 'archive';
    var areAnyUsingDefaultArchiveModel;
    ...
    _.each(modelDefs, function(modelDef){
      ...
      var isUsingDefaultArchiveModel = modelDef.archiveModelIdentity === DEFAULT_ARCHIVE_MODEL_IDENTITY;
      // If this is using the default archive, track that the default archive is in use.
      if (isUsingDefaultArchiveModel) {
        areAnyUsingDefaultArchiveModel = true;
      }
      // Otherwise, if this is a custom archive model, then make sure it's actually a real thing.
      else if (modelDef.archiveModelIdentity !== false) {
        var archiveModelDef = _.find(modelDefs, {identity: modelDef.archiveModelIdentity});
        if (!archiveModelDef) {
          throw new Error('Invalid `archiveModelIdentity` setting.  Model `'+modelDef.identity+'` has `archiveModelIdentity: \''+modelDef.archiveModelIdentity+'\'`, but there\'s no model registered with that identity!');
        }
    ...
    // If any models are using the default archive, then register the default archive model if it isn't already.
    if (areAnyUsingDefaultArchiveModel) {
      ...
      var existingDefaultArchiveModel = _.find(modelDefs, {identity: DEFAULT_ARCHIVE_MODEL_IDENTITY});
      modelDefs.push({
    ...
    // In `../waterline.js`:
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    // TODO: default the `archiveModelIdentity` orm setting to "archive"
    //
    ...
    //  > Note there's no updatedAt!
    //
    // TODO: validate the configured model's attributes and so forth,
    // to make sure it's actually usable as the orm's Archive.
    //
    // TODO: if there is an existing archive model with a conflicting
    // identity, throw a descriptive error on ORM initialization
    // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -



    // Now validate the rest of the settings and build the rest of the ORM:
    // =============================================================================================
