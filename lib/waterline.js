...
function Waterline() {
  ...
  orm.initialize = function initialize(options, done) {
    ...
    if (_.contains(archiveModelIdentities, DEFAULT_ARCHIVE_MODEL_IDENTITY)) {
      ...
      if (!existingDefaultArchiveModel) {
        modelDefs.push({
          identity: DEFAULT_ARCHIVE_MODEL_IDENTITY,
          datastore: 'default',
          primaryKey: 'id',
          attributes: {
            id: { type: 'number' },// TODO: infer proper type, if possible
            createdAt: { type: 'number', autoCreatedAt: true },
            originalRecord: { type: 'json', required: true },
            originalRecordId: { type: 'number' },// TODO: infer proper type, if possible
    ...
    _.each(archiveModelIdentities, function(archiveIdentity) {
      var archiveModelDef = _.find(modelDefs, {identity: archiveIdentity});
      if (!archiveModelDef) {
