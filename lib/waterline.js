...
var Waterline = module.exports = function ORM() {
  ...
  ORM.initialize = function initialize(options, cb) {
    // Ensure the ORM hasn't already been initialized. This causes all sorts of
    // issues because collection instances are modified in place.
    if (_.keys(modelMap).length) {
      throw new Error('A Waterline ORM instance can not be initialized more than once. To reset the ORM create a new instance of it by running `new Waterline()`.');
    }
    ...
    if (_.isUndefined(options.connections) || !_.isPlainObject(options.connections)) {
      throw new Error('Options must contain a `connections` dictionary');
    }
